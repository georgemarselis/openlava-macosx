...
usage (char *cmd)
{
  ...
  fprintf (stderr, ": %s [-h] [-V] [-w |-l] [-a] [-d] [-p] [-s] [-r]", cmd);
  ...
  if (lsbMode_ & LSB_MODE_BATCH)
    fprintf (stderr, "             [-P project_name] [-N host_spec]\n");
...
main (int argc, char **argv)
{
  ...
  TIMEIT (0,
	  do_options (argc, argv, &options, &user, &queue, &host, &jobName,
		      &cpuFactor, &format, &projectName), "do_options");
  ...
  if (jInfoH == NULL)
      ...
      else
	{
	  ...
	  jobInfoErr (LSB_ARRAY_JOBID (jobId),
		      jobName, user, queue, host, options);
	  exit (-2);
	}
  ...
  for (i = 0; i < jInfoH->numJobs; i++)
	...
	if (skip_job (job))
	  ...
	  (strstr (securebjobsParams[LSB_SECURE_JOBINFO_USERS].paramValue,
		   lsfUserName) == NULL)
	  && (strcmp (lsfUserName, job->user) != 0))
	{
	  if (numJids > 0)
	    {
	      lsberrno = LSBE_NO_JOB;
	      jobInfoErr (LSB_ARRAY_JOBID (job->jobId), jobName, user, queue,
			  host, options);
	    }
	  continue;
	}
  ...
  if (numJids > 1)
      ...
      for (i = 0; i < numJids; i++)
	{
	  if (numJobs[i] <= 0)
	    {
	      errCount = TRUE;
	      jobInfoErr (usrJids[i], jobName, realUser, queue, host,
			  options);
	    }
	}
      if (errCount == TRUE)
...
do_options (int argc, char **argv, int *options, char **user, char **queue,
	    char **host, char **jobName, float *cpuFactor, int *format,
	    char **projectName)
{
  int cc, Nflag = 0;
  char *norOp = NULL;
  ...
  *format = 0;
  ...
  TIMEIT (1, (numJids = getSpecJobIds (argc, argv, &usrJids, NULL)),
	  "getSpecJobIds");
  ...
  if (job->jType == JGRP_NODE_ARRAY)
    {
      if (format != WIDE_FORMAT)
	{
	  printf ("%-7d  %-8.8s ", LSB_ARRAY_JOBID (job->jobId),
		  job->submit.jobName);
	  printf ("%8.8s ", osUserName);
	}
      else
	{
	  printf ("%-7d  %s ", LSB_ARRAY_JOBID (job->jobId),
		  job->submit.jobName);
	  printf ("%s ", job->user);
	}
  ...
  if (options == PEND_JOB)
    {
      ...
      printf ("%-7d %-7.7s %-5.5s %-11.11s %-14.14s %-18.18s %s\n",
	      ...
	      submitInfo->queue, job->fromHost, jobName, subtime);
    }
  else if (format != WIDE_FORMAT)
    {
      ...
      printf ("%-7d %-7.7s %-5.5s %-10.10s %-11.11s %-11.11s %-10.10s %s\n",
	      LSB_ARRAY_JOBID (job->jobId), osUserName, status,
	      submitInfo->queue, job->fromHost, exec_host, jobName, subtime);
    }
  else
    {
      if (IS_PEND (job->status))
      ...
      if (Wflag == TRUE)
	{
	  printf ("%-7d %-7s %-5.5s %-10s %-11s %-11s %-10s %-14.14s",
		  ...
		  submitInfo->queue,
		  job->fromHost,
		  exec_host, jobName, Time2String (job->submitTime));
	}
      else
	{
	  printf ("%-7d %-7s %-5.5s %-10s %-11s %-11s %-10s %s",
		  ...
		  status,
		  submitInfo->queue,
		  job->fromHost, exec_host, jobName, subtime);
	}
...
Timer2String (float timer)
{
  ...
  Minute = (Time % 3600) / 60;
  Second = (Time % 3600) % 60;
  sprintf (TimerStr, "%03d:%02d:%02d.%02d", Hour, Minute, Second, Point);
  return (TimerStr);
}
