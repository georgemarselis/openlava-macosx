...
displayEvent (struct eventRec *log, struct histReq *req)
{
  ...
  if (req->eventTime[1] != 0 && req->eventTime[0] < req->eventTime[1])
    {
  ...
  switch (log->type)
      ...
      if (req->opCode == MBD_HIST || req->opCode == SYS_HIST)
	{
	  ...
	  sprintf (prline, (_i18n_msg_get (ls_catd, NL_SETN, 1057, "%s: mbatchd on host <%s> died: ")),	/* catgets  1057  */
		   localTimeStr, log->eventLog.mbdStartLog.master);
	  prtLine (prline);
	      ...
	      && isRequested (log->eventLog.queueCtrlLog.queue, req->names)))
	{
	  ...
	  sprintf (prline, "%s: %s <%s> ",
		   localTimeStr,
		   I18N_Queue, log->eventLog.queueCtrlLog.queue);
	  prtLine (prline);
	  switch (log->eventLog.queueCtrlLog.opCode)
	    ...
	    case QUEUE_INACTIVATE:
	      ...
	      break;
	    default:
	      sprintf (prline, "%s <%d>", I18N (1069, "unknown operation code"),	/* catgets  1069  */
	      ...
	      && isRequested (log->eventLog.hostCtrlLog.host, req->names)))
	{
	  ...
	  strcpy (localTimeStr,
		  _i18n_ctime (ls_catd, CTIME_FORMAT_a_b_d_T,
			       &log->eventTime));
	  sprintf (prline, "%s: %s <%s> ", localTimeStr, I18N_Host,
