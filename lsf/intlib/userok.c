...
// #if 0
  ...
  if (logclass & LC_AUTH)
    ls_syslog (LOG_DEBUG, "%s Authentication buffer (rbuf=<%s>)", fname,
	       rbuf);

  if (sscanf (rbuf, "%hd,%hd: USERID :%*[^:]:%s",
	      &rremote, &rlocal, ruser) < 3)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5806, "%s: Authentication data format error (rbuf=<%s>) from %s"),	/* catgets 5806 */
		 fname, rbuf, sockAdd2Str_ (&saddr));
      return 0;
    }
  if ((remote != rremote) || (local != rlocal))
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5807, "%s: Authentication port mismatch (remote=%d, local=%d, rremote=%d, rlocal=%d) from %s"),	/* catgets 5807 */
		 fname, remote, local, rlocal, rremote,
		 sockAdd2Str_ (&saddr));
      return 0;
    }
	...
	struct sockaddr_in *localAddr, struct lsfAuth *auth, int debug)
{
  ...
  if (debug)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5809, "%s: %s rejected in single user mode"), fname, auth->lsfUserName);	/* catgets 5809 */
	  return (FALSE);
	}
    }
...
%s: auth_user %s returned NULL, retrying"),	/*catgets 5824 */
		      ...
		      if (!user)
			{
			  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5815, "%s: auth_user %s retry failed: %m"),	/* catgets 5815 */
				     fname, sockAdd2Str_ (from));
			  return (FALSE);
			}
		      else
			{
			  ls_syslog (LOG_INFO, I18N (5825, "%s: auth_user %s retry succeeded"),	/*catgets 5825 */
				     fname, sockAdd2Str_ (from));
			}
		    }
		  else
	      ...
	      else
		{
		  if (strcmp (user, auth->lsfUserName) != 0)
		    {
		      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5816, "%s: Forged username suspected from %s: %s/%s"),	/* catgets 5816 */
				 fname, sockAdd2Str_ (from),
				 auth->lsfUserName, user);
		      return (FALSE);
		    }
		}
	    }
      ...
      else
	{
	  if (!strcmp (authKind, AUTH_PARAM_EAUTH))
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5817, "%s: Client %s is not using <%d/eauth> authentication"),	/* catgets 5817 */
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5818, "%s: %s authentication failed for %s/%s"),	/* catgets 5818 */
			     fname, "eauth", auth->lsfUserName,
			     sockAdd2Str_ (from));
		  return (FALSE);
		}
	    }
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5819, "%s: Unkown authentication type <%d> from %s/%s; denied"),	/* catgets 5819 */
			 fname, auth->kind, auth->lsfUserName,
			 sockAdd2Str_ (from));
	      return (FALSE);
	    }
