...
// #if 0
    ...
    if (id_port == 0) {
        ...
        if (authd_port != NULL) {
            ...
            /* catgets 5801 */
                ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5801, "%s: LSF_ID_PORT in lsf.conf must be positive number"), __func__);
                return NULL;
            }
            ...
            if (!svp) {
                /* catgets 5802 */
                ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5802, "%s: %s(%s/tcp) failed: %m"), __func__, "getservbyname", "ident");
                return NULL;
            }
            id_port = svp->s_port;
        }
    ...
    ls_syslog (LOG_DEBUG, "%s: Calling for authentication at <%s>, port:<%d>",  __func__, inet_ntoa (saddr.sin_addr), id_port);

    if (b_connect_ (s, (struct sockaddr *) &saddr, sizeof (struct sockaddr_in), TIMEOUT) == -1) {
        int realerrno = errno;
        close (s);
        ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_M, __func__, "b_connetc_", sockAdd2Str_ (&saddr));
        errno = realerrno;
        return 0;
    }
    ...
    while ((n = write (s, bp, bufsize)) < (ssize_t) bufsize) {
        ...
        if (n <= 0) {
            ...
            close (s);
            ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_M, __func__, "write", sockAdd2Str_ (&saddr));
            if (errno == EINTR) {
                errno = ETIMEDOUT;
            }
            alarm (oldTimer);
    ...
    if (n <= 0) {
        ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_M, __func__, "read", sockAdd2Str_ (&saddr));
        if (errno == EINTR) {
            errno = ETIMEDOUT;
        }
        alarm (oldTimer);
    ...
    if (logclass & LC_AUTH) {
        ls_syslog (LOG_DEBUG, "%s Authentication buffer (rbuf=<%s>)", __func__, rbuf);
    }

    if (sscanf (rbuf, "%hu,%hu: USERID :%*[^:]:%s",     &rremote, &rlocal, ruser) < 3) {
        /* catgets 5806 */
        ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5806, "%s: Authentication data format error (rbuf=<%s>) from %s"), __func__, rbuf, sockAdd2Str_ (&saddr));
        return 0;
    }
    if ((remote != rremote) || (local != rlocal)) {
        /* catgets 5807 */
        ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5807, "%s: Authentication port mismatch (remote=%d, local=%d, rremote=%d, rlocal=%d) from %s"),  __func__, remote, local, rlocal, rremote, sockAdd2Str_ (&saddr));
        return 0;
    }
...
userok (int s, struct sockaddr_in *from, char *hostname, struct sockaddr_in *localAddr, struct lsfAuth *auth, int debug)
{
    ...
    if (debug)
    {
        ...
        {
            ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5809, "%s: %s rejected in single user mode"), __func__, auth->lsfUserName);   /* catgets 5809 */
            return FALSE;
        }
    }
    ...
    {
        ls_syslog (LOG_ERR, "%s: sin_family != AF_INET", __func__);
        return FALSE;
    }
    ...
    else
    {
        if (strcmp (authKind, AUTH_IDENT) == 0)
        {
            ...
            if (remote >= IPPORT_RESERVED || remote < IPPORT_RESERVED / 2)
            {
                ...
                if (!user)
                {
                    ...
                    {
                        /*catgets 5824 */
                        ls_syslog (LOG_INFO, I18N (5824, "%s: auth_user %s returned NULL, retrying"), __func__, sockAdd2Str_ (from));
                        ...
                        {
                            /* catgets 5815 */
                            ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5815, "%s: auth_user %s retry failed: %m"), __func__, sockAdd2Str_ (from));
                            return FALSE;
                        }
                        else
                        {
                            /*catgets 5825 */
                            ls_syslog (LOG_INFO, I18N (5825, "%s: auth_user %s retry succeeded"), __func__, sockAdd2Str_ (from));
                        }
                    }
                    else
                    {
                        /*catgets 5826 */
                        ls_syslog (LOG_INFO, I18N (5826, "%s: auth_user %s returned NULL"), __func__, sockAdd2Str_ (from));
                        return FALSE;
                    }
                }
                else
                {
                    ...
                    {
                        /* catgets 5816 */
                        ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5816, "%s: Forged username suspected from %s: %s/%s"),   __func__, sockAdd2Str_ (from), auth->lsfUserName, user);
                        return FALSE;
                    }
                }
            }
        ...
        else
        {
            if (!strcmp (authKind, AUTH_PARAM_EAUTH))
            {
                ...
                {
                    /* catgets 5817 */
                    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5817, "%s: Client %s is not using <%d/eauth> authentication"), __func__, sockAdd2Str_ (from), (int) auth->kind);
                    return FALSE;
                }
                ...
                {
                    /* catgets 5818 */
                    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5818, "%s: %s authentication failed for %s/%s"), __func__, "eauth", auth->lsfUserName, sockAdd2Str_ (from));
                    return FALSE;
                }
            }
            else
            {
                /* catgets 5819 */
                ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5819, "%s: Unkown authentication type <%d> from %s/%s; denied"), __func__, auth->kind, auth->lsfUserName,  sockAdd2Str_ (from));
                return FALSE;
            }
