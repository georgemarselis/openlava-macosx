...
isSectionEnd (char *linep, char *lsfile, int *LineNum, char *sectionName)
{
  ...
  if (!word)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5400, "%s(%d): section %s ended without section name, ignored"), /* catgets 5400 */
     lsfile, *LineNum, sectionName);
      return TRUE;
    }

  if (strcasecmp (word, sectionName) != 0)
    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5401, "%s(%d): section %s ended with wrong section name %s, ignored"), /* catgets 5401 */
       ...
       int *LineNum, int exact, char *section)
{
  ...
  if (!value)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5402, "%s: %s(%d): missing '=' after keyword %s, section %s ignored"), /* catgets 5402  */
  ...
  if (value[0] == '\0')
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5403, "%s: %s(%d): nul value after keyword %s, section %s ignored"), /* catgets 5403  */
     fname, lsfile, *LineNum, key, section);
      return -1;
    }
  ...
  *sp1 = '\0';
  ...
  if (putValue (keyList, key, value) < 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5404, "%s: %s(%d): bad keyword %s in section %s, ignoring the section"), /* catgets 5404  */
     fname, lsfile, *LineNum, key, section);
      doSkipSection (fp, LineNum, lsfile, section);
      return -1;
    }
      ...
      if (isSectionEnd (linep, lsfile, LineNum, section))
  {
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5405, "%s: %s(%d): required keyword %s is missing in section %s, ignoring the section"), /* catgets 5405  */
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5406, "%s: %s(%d): Premature EOF in section %s"),  /* catgets 5406  */
...
doSkipSection (FILE * fp, int *LineNum, char *lsfile, char *sectionName)
{
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5409, "%s: %s(%d): premature EOF in section"), /* catgets 5409  */
...
%s: <%s> is not a valid user name; ignored", fname, word);
  ...
  if (isInlist (admins->adminNames, pw->pw_name, admins->nAdmins))
    {
      ls_syslog (LOG_WARNING, _i18n_msg_get (ls_catd, NL_SETN, 5411, "%s: Duplicate user name <%s> %s; ignored"), /* catgets 5411  */
     fname, word, forWhat);
      return (0);
    }
