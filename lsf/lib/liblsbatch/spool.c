...
getLocalHostOfficialName (void)
{
    ...
    memset( localHost, '\0', strlen( localHost ) );
    ...
    hp = Gethostbyname_ (localHost);
    if ( hp ) {
        strcpy (localHost, hp->h_name);
        returnLocalHostPtr = &localHost;
    }
...
copySpoolFile (const char *srcFilePath, spoolOptions_t option)
{
    ...
    const char *startFileName         = NULL;
    const char *officialHostNamePtr   = NULL;
    const char *spoolDirPtr           = NULL;
    const char dirSeparator[]         = SPOOL_DIR_SEPARATOR; // FIXME FIXME FIXME FIXME set SPOOL_DIR_SEPARATOR as const char in configure.ac or config.h, not here.
    ...
    char destinationDir    [MAX_FILENAME_LEN ];
    char destinationFile   [MAX_FILENAME_LEN ];
    char srcFileName       [MAX_FILENAME_LEN ];
    char srcFileFullPath   [MAX_FILENAME_LEN ];

    memset( spoolHost,         '\0', strlen( spoolHost         ) );
    memset( localHost,         '\0', strlen( localHost         ) );
    memset( spoolFileFullPath, '\0', strlen( spoolFileFullPath ) );
    memset( pidString,         '\0', strlen( pidString         ) );
    memset( spoolDir,          '\0', strlen( spoolDir          ) );
    memset( destinationDir,    '\0', strlen( destinationDir    ) );
    memset( destinationFile,   '\0', strlen( destinationFile   ) );
    memset( srcFileName,       '\0', strlen( srcFileName       ) );
    memset( srcFileFullPath,   '\0', strlen( srcFileFullPath   ) );

    spoolFileInfoStruct.srcFile = NULL;
    spoolFileInfoStruct.spoolFile = NULL;
...
getTrimmedString (const char *stringToTrim)
{
    ...
    returnStringPtr = stringToTrim;


    advancePtr = returnStringPtr;

    for ( unsigned long i = 0; (unsigned long)i < strlen (returnStringPtr); i++) {
    ...
    for ( unsigned long i = strlen (returnStringPtr) - 1; i >= 0; i--) {
        if ((returnStringPtr[i] != ' ') && (returnStringPtr[i] != '\t')) {
            break;
        }
        returnStringPtr[i] = '\0';
...
findSpoolDir (const char *spoolHost)
{
    ...
    parameterInfoPtr = lsb_parameterinfo (NULL, NULL, 0); // get the parameters under which the local mbd daemon is running
...
createSpoolHostsList ()
{
    ...
    memset( localHost,  '\0', strlen( localHost  ) );
    memset( returnHost, '\0', strlen( returnHost ) );
