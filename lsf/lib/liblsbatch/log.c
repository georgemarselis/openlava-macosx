...
lsb_geteventrec_ex (FILE *log_fp,  size_t *lineNum, char *usedLine)
{
	...
	if ((ccount = stripQStr (line, namebuf)) < 0 || (long) strlen (line) == ccount || (long) strlen (namebuf) >= MAX_VERSION_LEN)
		{
			lsberrno = LSBE_EVENT_FORMAT;
			free (namebuf);
			return NULL;
	...
	if ((version = atof (logRec->version)) <= 0.00000000000000)
		{
			lsberrno = LSBE_EVENT_FORMAT;
			free (namebuf);
			return NULL;
	...
	cc = sscanf (line, "%d%ld", &tempTimeStamp, &ccount);
...
freeLogRec (struct eventRec *logRec)
{
		...
		case EVENT_JOB_FINISH:
			...
			{
				for ( size_t i = 0; i < logRec->eventLog.jobFinishLog.numAskedHosts; i++) {
			...
			{
				for (size_t  i = 0; i < logRec->eventLog.jobFinishLog.numExHosts; i++) {
...
readJobNew (char *line, struct jobNewLog *jobNewLog)
{
	...
	cc = sscanf (line, "%ld%d%d%d%d%d%d%d%ld%d%n",
					 &(jobNewLog->jobId),
					 &(jobNewLog->userId),
					 &(jobNewLog->options),
	...
	line += ccount + 1;
		...
		if (jobNewLog->nxf > 0)
		{
				assert( jobNewLog->nxf >= 0 );
				jobNewLog->xf = (struct xFile *)calloc ((unsigned long)jobNewLog->nxf, sizeof (struct xFile));
...
int readJobMod (char *line, struct jobModLog *jobModLog)
{
		...
		if (jobModLog->options & SUB_HOST)
		{
				...
				{
						assert( jobModLog->numAskedHosts >= 0 );
	...
	if (jobModLog->options & SUB_OTHER_FILES)
		...
		return LSBE_EVENT_FORMAT;
			...
			if (jobModLog->nxf > 0)
		{
			assert( jobModLog->nxf >= 0 );
			jobModLog->xf = (struct xFile *)calloc ((unsigned long)jobModLog->nxf, sizeof (struct xFile));
...
int readJobStart (char *line, struct jobStartLog *jobStartLog)
{
	...
	jobStartLog->execHosts = (char **)calloc( (unsigned long)jobStartLog->numExHosts, sizeof (char *));
...
int readMig (char *line, struct migLog *migLog)
{
	...
	char hName[MAXHOSTNAMELEN];
		...
		if (migLog->numAskedHosts)
		{
				...
				assert( migLog->numAskedHosts >=0 );
				migLog->askedHosts = (char **)calloc ((unsigned long)migLog->numAskedHosts, sizeof (char *));
...
int readLoadIndex (char *line, struct loadIndexLog *loadIndexLog)
{
		...
		loadIndexLog->name = (char **) calloc ((unsigned long)loadIndexLog->nIdx, sizeof (char *));
...
int readJobFinish (char *line, struct jobFinishLog *jobFinishLog, time_t eventTime)
{
	...
	if (jobFinishLog->numAskedHosts > 0) {
		...
		assert( jobFinishLog->numAskedHosts >= 0 );
		jobFinishLog->askedHosts = calloc( jobFinishLog->numAskedHosts, sizeof( jobFinishLog->numAskedHosts ) );
	...
	for ( size_t i = 0; i < jobFinishLog->numAskedHosts; i++)
	{
		if ((ccount = stripQStr (line, hName)) < 0) {
	...
	cc = sscanf (line, "%lu%n", &jobFinishLog->numExHosts, &ccount);
	if (cc != 1) {
		return LSBE_EVENT_FORMAT;
	}
	line += ccount + 1;
	...
	if (jobFinishLog->numExHosts > 0) {
		...
		assert( jobFinishLog->numExHosts >= 0 );
		jobFinishLog->execHosts = calloc( jobFinishLog->numExHosts, sizeof (jobFinishLog->numExHosts)); // FIXME FIXME FIXME sure buffer overflow
	...
	for ( size_t i = 0; i < jobFinishLog->numExHosts; i++) {
	...
	cc = sscanf (line, "%d%f%n", &jobFinishLog->jStatus, &jobFinishLog->hostFactor, &ccount);
	if (cc != 2) {
		return LSBE_EVENT_FORMAT;
	}
	line += ccount + 1;
	...
	jobFinishLog->cpuTime = (jobFinishLog->lsfRusage.ru_utime + jobFinishLog->lsfRusage.ru_stime);
	...
	if ((cc = sscanf (line, "%lu%n", &jobFinishLog->maxNumProcessors, &ccount)) != 1) {
		return LSBE_EVENT_FORMAT;
	}
	line += ccount + 1;
	...
	line += ccount + 1;
	cc = sscanf (line, "%d%d%n", &(jobFinishLog->maxRMem), &(jobFinishLog->maxRSwap), &ccount);
	if (cc != 2) {
		return LSBE_EVENT_FORMAT;
	}
	line += ccount + 1;
...
lsb_puteventrec (FILE * log_fp, struct eventRec *logPtr)
{
	...
	switch (logPtr->type)
		...
		case EVENT_SBD_JOB_STATUS:
			lsberrno =
		writeSbdJobStatus (log_fp, &(logPtr->eventLog.sbdJobStatusLog));
			break;
		case EVENT_JOB_SWITCH:
		...
		case EVENT_JOB_FORCE:
			lsberrno =
		writeJobForce (log_fp, &(logPtr->eventLog.jobForceRequestLog));
			break;
		case EVENT_LOG_SWITCH:
...
int writeJobFinish (FILE * log_fp, struct jobFinishLog *jobFinishLog)
{
	...
	if (fprintf (log_fp, " %lu", jobFinishLog->numAskedHosts) < 0) {
		return LSBE_SYS_CALL;
	}
	...
	if (fprintf (log_fp, " %lu", jobFinishLog->numExHosts) < 0) {
		return LSBE_SYS_CALL;
	}
	...
	if (fprintf (log_fp, " %lu", jobFinishLog->maxNumProcessors) < 0) {
		return LSBE_SYS_CALL;
	}
...
void
countLineNum (FILE * fp, long pos, size_t *lineNum)
{
		int ch = 0;
		long curPos;
						...
						struct jobIdIndexS *indexS)
{
	...
	{
			if (ePtr->fp != NULL) {
				logRec = lsbGetNextJobRecFromFile (ePtr->fp, lineNum, numJobIds, jobIds);
			}
			else {
				lsberrno = LSBE_EOF;
			}
			...
			if (logRec == NULL &&
				ePtr->curOpenFile > 0 &&
				lsberrno == LSBE_EOF && ePtr->curOpenFile > ePtr->lastOpenFile)
			{
				...
				else
				{
					...
					if (indexS != NULL)
					{
						nextFileNumber = getNextFileNumFromIndexS (indexS, numJobIds, jobIds);
					}

					if ((indexS == NULL) || (nextFileNumber == -1))
					{
						sprintf (eventFile, "%s/lsb.events.%d", ePtr->openEventFile, --(ePtr->curOpenFile));
					}
					else
					{
						if (nextFileNumber == 0)
						...
						{
							sprintf (eventFile, "%s/lsb.events.%d", ePtr->openEventFile, nextFileNumber);
...
checkJobEventAndJobId (char *line, int eventType, size_t numJobIds, size_t * jobIds)
{
	...
	switch (eventType)
		...
		cc = sscanf (line, "%d %d", &tempUserId, &eventJobId);
		if (cc != 2)
			{
				return 1;
			}
...
long lsb_array_jobid( long jobId ) {
	if( -1 == jobId ) {
		...
		return ( jobId & LSB_MAX_ARRAY_JOBID );
	}
}
...
int
getJobIdIndexFromEventFile (char *eventFile, struct sortIntList *header, time_t * timeStamp)
{
	FILE *eventFp = NULL;
	char ch;
	long jobId = 0;
	...
	*timeStamp = tempTimeStamp;
	...
	while (TRUE) // FIXME FIXME FIXME create terminating condition.
		{
			...
			if ((line = getNextLine_ (eventFp, FALSE)) == NULL)
		{
			...
			if ((ccount = stripQStr (line, nameBuf)) < 0 ||  strlen (nameBuf) >= MAX_LSB_NAME_LEN)
		{
			// FIXME FIXME FIXME FIXME why is this empty? maybe line below?
			line += ccount + 1;
		}
			// line += ccount + 1;
...
updateJobIdIndexFile (char *indexFile, char *eventFile, int totalEventFile)
{
	...
	if ((stat (indexFile, &st) != 0) || (indexFp == NULL))
		{
	...
	while (TRUE)
			...
			if (fscanf (indexS->fp, "#%ld %d %ld %ld\n", &(indexS->timeStamp), &(indexS->totalJobIds), &(indexS->minJobId), &(indexS->maxJobId)) != 4)
		{
			...
			return -1;
				...
				size = (unsigned long) indexS->totalJobIds * sizeof (int);
				indexS->jobIds = (int *) malloc (size);
			...
			for (i = 0; i < numJobIds; i++)
		{
			...
			if ((jobId > indexS->maxJobId) || (jobId < indexS->minJobId)) {
				continue;
			}
...
size_t saveQStr( char *line, char *destStr)  {
	...
	size_t ccounter = 0;
	int temp        = 0;
	char *tmpLine   = malloc( strlen( line ) );
	if ( NULL == tmpLine && ENOMEM == errno ) {
			return LSBE_NO_MEM;
	}
