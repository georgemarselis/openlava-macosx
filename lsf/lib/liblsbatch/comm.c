...
{
	int cc = 0;
	struct Buffer *sndBuf = NULL;
	struct Buffer reqbuf;
	struct Buffer reqbuf2;
	struct Buffer replybuf;
	struct Buffer *replyBufPtr = NULL;
	int serverSock = 0;
	...
	*rep_buf = NULL; // FIXME FIXME FIXME there could be some memory allocation issues here
	...
	if (!(flags & CALL_SERVER_USE_SOCKET)) {
		...
		if (connectedSock == NULL) {
			/* catgets 5000 */
			ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5000, "%s: CALL_SERVER_USE_SOCKET defined, but %s is NULL"), __func__, "connectedSock");
	...
	if (logclass & LC_COMM) {
		ls_syslog (LOG_DEBUG1, "%s: serv_connect() get server sock <%d>", __func__, serverSock);
	}
	...
	if (!(flags & CALL_SERVER_NO_HANDSHAKE)) {
		...
		if (handShake_ (serverSock, TRUE, conn_timeout) < 0) {
			CLOSECD (serverSock);
			if (logclass & LC_COMM) {
				ls_syslog (LOG_DEBUG, "%s: handShake_(socket=%d, conn_timeout=%d) failed", __func__, serverSock, conn_timeout);
			}
			return -2;
		}

		if (logclass & LC_COMM) {
			ls_syslog (LOG_DEBUG1, "%s: handShake_() succeeded", __func__);
		}
	}
	...
	if (flags & CALL_SERVER_ENQUEUE_ONLY) {
		...
		if (chanAllocBuf_ (&sndBuf, (int)tsize) < 0)    {
			ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "chanAllocBuf_",  tsize);
			CLOSECD (serverSock);
			return -2;
		}
...
getServerMsg (int serverSock, struct LSFHeader *replyHdr, char **rep_buf)
{
	...
	if (readDecodeHdr_ (serverSock, (char *) &hdrBuf, b_read_fix, &xdrs, replyHdr) < 0) {
		if (LSE_SYSCALL (lserrno))  {
			lsberrno = LSBE_SYS_CALL;
			if (logclass & LC_COMM) {
				ls_syslog (LOG_DEBUG1, "%s: readDecodeHdr_() failed for read reply header from mbatchd: %m", __func__, replyHdr ); // FIXME FIXME is replyHdr correct?
			}
		}
		else {
			lsberrno = LSBE_XDR;
		...
		*rep_buf = malloc( len + 1 );
		if ( NULL == *rep_buf && ENOMEM == errno ) {
			...
			if (logclass & LC_TRACE) {
				...
				ls_syslog (LOG_DEBUG1, "call_server: malloc (%d) failed", len );
			}
		...
		if (b_read_fix (serverSock, *rep_buf, len) == -1) {
			...
			*rep_buf = NULL;
			lsberrno = LSBE_SYS_CALL;
			if (logclass & LC_COMM) {
				// ls_syslog (LOG_DEBUG1, "%s: b_read_fix() failed for read message from mbatchd: %m", __func__);
				ls_syslog (LOG_DEBUG1, "%s: b_read_fix() failed for read message from mbatchd: ", __func__);
			}
...
get_mbd_port (void)
{
	...
	return mbd_port = (unsigned short)sv->s_port;		// FIXME FIXME is cast correct here?
}
...
callmbd (char *clusterName, char *request_buf, int requestlen, char **reply_buf, struct LSFHeader *replyHdr, int *serverSock, int (*postSndFunc) (), int *postSndFuncArg)
{
	...
	do { //  do .. while ( );
		...
		if( NULL == clusterName && NULL == masterHost ) {
			if (logclass & LC_TRACE) {
				ls_syslog (LOG_DEBUG1, "%s: getMasterName() failed", __func__);
			}
			return -1;
		}
		else {
			...
			if( NULL == clusterInfo ) {
				if (logclass & LC_TRACE) {
					ls_syslog (LOG_DEBUG1, "%s: ls_clusterinfo() failed", __func__);
				}
			...
			if (clusterInfo[0].status & CLUST_STAT_OK) {
				...
				if (logclass & LC_TRACE) {
					ls_syslog (LOG_DEBUG1, "%s: master host identified", __func__);
				}
			}
			else {
				lsberrno = LSBE_BAD_CLUSTER;
				if (logclass & LC_TRACE) {
					ls_syslog (LOG_DEBUG1, "%s: Remote cluster not OK", __func__);
				}
				return -1;
			}
		}
		...
		if (logclass & LC_TRACE) {
			ls_syslog (LOG_DEBUG1, "%s: masterHost=%s", __func__, masterHost);
		}
		...
		xdr_destroy (&xdrs);
		if (logclass & LC_TRACE) {
			ls_syslog (LOG_DEBUG1, "%s: mbd_port=%d", __func__, ntohs (mbd_port));
		}
		   ...
		   )
		{
			...
			if (logclass & LC_TRACE) {
				ls_syslog (LOG_DEBUG1, "%s: callmbd() failed: %M", __func__);
			}
...
{
	unsigned short sbdPort = 0;
	int cc = 0;

	if (logclass & LC_COMM) {
		ls_syslog (LOG_DEBUG, "%s: Entering this routine... host=<%s>", __func__, sbdHost);
	}
	...
	if (cc < 0) {
		if (logclass & LC_COMM) {
			ls_syslog (LOG_DEBUG, "%s: cc=%d lsberrno=%d lserrno=%d", __func__, cc, lsberrno, lserrno);
		}
		return -1;
	}
...
handShake_ (int s, char client, int timeout)
{
	...
	if (client) {
		...
		memset ((char *) &hdr, 0, sizeof (struct LSFHeader)); // FIXME FIXME cast seems suspicious
		...
		cc = chanRpc_(s, &reqbuf, &replybuf, &hdr, timeout * 1000); // FIXME FIXME 1000 seems awfully specific
		if (cc < 0) {
			lsberrno = LSBE_LSLIB;
			return -1;
		}
		if (hdr.opCode != READY_FOR_OP) {
			...
			lsberrno = hdr.opCode;
			if (logclass & LC_TRACE) {
				ls_syslog (LOG_DEBUG1, "%s: mbatchd returned error code <%d>", __func__, hdr.opCode);
			}
			return -1;
		}
...
authTicketTokens_ (struct lsfAuth *auth, char *toHost)
{
	if (toHost == NULL) {
		...
		char buf[1024];				// FIXME FIXME FIXME buf[1024] seems awufly particular
