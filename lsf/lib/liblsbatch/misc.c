...
// NOFIX wrapper function, let it sleep.
// void
...
{
    struct sortIntList *headerPtr = NULL;
...
copyJUsage (struct jRusage *to, struct jRusage *from)
{
    ...
    if (from->npgids)
    {
            ...
            newPgid = calloc ( from->npgids, sizeof( gid_t ) );
            ...
            assert (from->npgids >= 0);
            memcpy ( to->pgid, from->pgid, from->npgids * sizeof ( from->npgids ) );
    }
    else if (to->npgids) {
            FREEUP (to->pgid);
            to->npgids = 0;
...
convertRLimit (int *pRLimits, int toKb)
{
    ...
    for ( size_t i = 0; i < LSF_RLIM_NLIMITS; i++) {
        switch (i) {
            ...
            break;
            default:
                sprintf( stderr, "%s: you are not supposed to be here.", __func__ );
            break;
        }
    }
