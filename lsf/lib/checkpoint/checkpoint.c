...
char *getEchkpntMethodDir (char *pChkpnt_Cmd, const char *pMethodDir, const char *pProgramName, const char *pMethodName)
{
    ...
    if (pConfigPath == NULL) {
        pConfigPath = "/etc";   // FIXME FIXME FIXME the value here should
                                //  be set by the autoconf toolset
    }
    ...
    if ((pMethodName != NULL) && (strcmp (pMethodName, ECHKPNT_DEFAULT_METHOD) != 0) && (pMethodDir != NULL)) {
