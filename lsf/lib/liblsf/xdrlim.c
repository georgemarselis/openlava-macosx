...
static bool_t
xdr_hostLoad (XDR * xdrs, struct hostLoad *loadVec, struct LSFHeader *hdr,
	      char *cp)
{
  char *sp;
  int i;
...
bool_t
xdr_placeReply (XDR * xdrs, struct placeReply * placeRepPtr,
		struct LSFHeader * hdr)
{
  int i, status = FALSE;
  static char *memp;
...
bool_t
xdr_loadReply (XDR * xdrs, struct loadReply * loadReplyPtr,
	       struct LSFHeader * hdr)
{
  char *sp;
  int i, status = TRUE;
		   ...
		   struct LSFHeader * hdr)
{
  ...
  if (xdrs->x_op == XDR_DECODE)
	...
	FREEUP (memp);
      ...
      shISize =
	ALIGNWORD_ (hostInfoReply->nHost * sizeof (struct shortHInfo));
      ...
      hostInfoReply->hostMatrix =
	(struct shortHInfo *) malloc (shISize + matSize);
...
bool_t
xdr_shortHInfo (XDR * xdrs, struct shortHInfo * shortHInfo,
		struct LSFHeader * hdr, char *nIndex)
{
  ...
  if (xdrs->x_op == XDR_ENCODE)
	...
	ls_syslog (LOG_DEBUG2,
		   "xdr_shortHInfo: host <%s>  type = %d, model = %d",
		   shortHInfo->hostName, tIndx, mIndx);
    }

  if (!xdr_string (xdrs, &sp, MAXHOSTNAMELEN) ||
      !xdr_u_int (xdrs, &a) ||
      !xdr_u_int (xdrs, &b) || !xdr_int (xdrs, &shortHInfo->resClass))
    return FALSE;
...
static bool_t
xdr_shortLsInfo (XDR * xdrs, struct shortLsInfo *shortLInfo,
		 struct LSFHeader *hdr)
{
  int i;
...
bool_t
xdr_masterInfo (XDR * xdrs, struct masterInfo * mInfoPtr,
		struct LSFHeader * hdr)
{
  char *sp;
...
bool_t
xdr_shortCInfo (XDR * xdrs, struct shortCInfo *clustInfoPtr,
		struct LSFHeader *hdr)
{
  char *sp1, *sp2, *sp3;
  int i;
  ...
  if (xdrs->x_op == XDR_DECODE && clustInfoPtr->nAdmins > 0)
	...
	= (int *) malloc (clustInfoPtr->nAdmins * sizeof (int));
      if (clustInfoPtr->admins == NULL || clustInfoPtr->adminIds == NULL)
	{
  ...
  for (i = 0; (i < clustInfoPtr->nRes && i < GET_INTNUM (clustInfoPtr->nRes));
       i++)
    {
  ...
  if (xdrs->x_op == XDR_DECODE && cInfo->nAdmins > 0)
    {
      ...
      if (cInfo->admins == NULL || cInfo->adminIds == NULL)
	{
  ...
  if (xdrs->x_op == XDR_DECODE && resourceInfoReq->numResourceNames > 0)
    {
      if ((resourceInfoReq->resourceNames =
	   (char **) malloc (resourceInfoReq->numResourceNames *
			     sizeof (char *))) == NULL)
	{
	  lserrno = LSE_MALLOC;
	  return FALSE;
...
bool_t
xdr_resourceInfoReply (XDR * xdrs,
