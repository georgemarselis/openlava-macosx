...
int
mychdir_(  const char *path, struct hostent *hp )
{
    ...
    if (hp != NULL) {
        if (netHostChdir (goodpath, hp) == 0) {
            return 0;
        }
    }

    if (strstr (goodpath, "/tmp_mnt") == goodpath) { // FIXME FIXME FIXME FIXME replace fixed strings with autoconf settings
        sp = goodpath + strlen ("/tmp_mnt");     // FIXME FIXME FIXME FIXME replace fixed strings with autoconf settings
...
try:  // FIXME FIXME FIXME FIXME FIXME remove label:
    ...
    if ((goodpath = strstr (goodpath, "/exp/lsf")) != NULL) { // FIXME FIXME FIXME FIXME replace fixed strings with autoconf settings
        if (chdir (usePath (goodpath)) == 0) {
            return 0;
        }
    }
    ...
    if (strstr (goodpath, "/tmp_mnt") == goodpath) { // FIXME FIXME FIXME FIXME replace fixed strings with autoconf settings
        goodpath = goodpath + strlen ("/tmp_mnt"); // FIXME FIXME FIXME FIXME replace fixed strings with autoconf settings
    }
    else {
    ...
    // memset( goodpath, '\0', strlen( goodpath ) ) ; // FIXME FIXME FIXME FIXME FIXME this outa be a free or something.
    strcat (filename, sp);
...
int
tryPwd ( const char *path, const char *pwdpath)
{
    char *PA   = NULL;
    char *PAPB = NULL;
    char *pa   = NULL;
...
int
netHostChdir (const char *path, struct hostent *hp)
{
...
int
mystat_ ( const char *filename, struct stat *sbuf, struct hostent *hp)
{
...
int
mychmod_ ( const char *filename, mode_t mode, struct hostent *hp)
{
...
void
myexecv_ ( const char *filename, char **argv, struct hostent *hp)
{
  char fnamebuf[MAX_FILENAME_LEN];
  char *mp;

  memset( fnamebuf, '\0', MAX_FILENAME_LEN );
...
int
myunlink_ ( const char *filename, struct hostent *hp, int doMount)
{
    char fnamebuf[MAX_FILENAME_LEN];
    char *mp = NULL;
    int i    = 0;
...
int
mymkdir_ ( const char *filename, mode_t mode, struct hostent *hp)
{
    char fnamebuf[MAX_FILENAME_LEN];
    char *mp = NULL;
    int i    = 0;
...
int
myrename_ ( const char *from, const char *to, struct hostent *hp)
{
    char fnamebuf[MAX_FILENAME_LEN];
