...
CreateSock_ (int protocol)
{
  ...
  if ((s = Socket_ (AF_INET, protocol, 0)) < 0)
    {
      if (logclass & LC_COMM)
	ls_syslog (LOG_DEBUG, "%s: Socket_ failed, %s", fname,
		   strerror (errno));
  ...
  for (i = 0; i < IPPORT_RESERVED / 2; i++)
	...
	{
	  if (logclass & LC_COMM)
	    ls_syslog (LOG_DEBUG, "%s: bind failed, %s", fname,
		       strerror (errno));
	  closesocket (s);
	  lserrno = LSE_SOCK_SYS;
	  return (-1);
  ...
  if (isroot && i == IPPORT_RESERVED / 2)
    {
      if (logclass & LC_COMM)
	ls_syslog (LOG_DEBUG, "%s: went through all , %s", fname,
		   strerror (errno));
      closesocket (s);
      lserrno = LSE_SOCK_SYS;
      return (-1);
...
svrsockCreate_ (u_short port,
		int backlog, struct sockaddr_in * addr, int options)
{
