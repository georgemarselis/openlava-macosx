...
writeAddHost (FILE * fp, struct lsEventRec *ev)
{
  ...
  fprintf (fp, "\
\"%s\" \"%s\" \"%s\" %d %d %4.2f %d ", hPtr->hostName, hPtr->hostModel, hPtr->hostType, hPtr->rcv, hPtr->nDisks, hPtr->cpuFactor, hPtr->numIndx);
 ...
 */
static int
readAddHost (char *buf, struct lsEventRec *ev)
{
  int cc;
  int n;
  int i;
   ...
   */
  cc = sscanf (p, "\
%s%s%s%d%d%f%d%n", name, model, type, &hPtr->rcv, &hPtr->nDisks, &hPtr->cpuFactor, &hPtr->numIndx, &n);
  if (cc != 7)
	{
	  free (hPtr);
	  return -1;
