	       ...
	       int rpid, int exitStatus, int terWhiPendStatus)
{
  ...
  if (b_connect_ (s, (struct sockaddr *) from,
		  sizeof (struct sockaddr_in), resTimeout) < 0)
    {
      if (logclass & LC_EXEC)
	ls_syslog (LOG_DEBUG, "\
		      %s: connect(s=%d,%s,len=%d) failed: %m", fname, s, sockAdd2Str_ (from), sizeof (struct sockaddr_in));
      closesocket (s);
      return (-1);
    }
  ...
  if (writeEncodeMsg_ (s, (char *) &reqBuf, sizeof (reqBuf), &reqHdr,
    ...
    {
      if (logclass & LC_EXEC)
	ls_syslog (LOG_ERR, I18N (6201, "%s: writeEncodeMsg_(%d,%d) RES2NIOS_connect failed: %M"),	/* catgets 6201 */
		   fname, s, rpid);
      closesocket (s);
      return (-1);
    }
