...
chanSendDgram_ (int chfd, char *buf, int len, struct sockaddr_in *peer)
{
  ...
  if (logclass & (LC_COMM | LC_TRACE))
    ls_syslog (LOG_DEBUG3,
	       "chanSendDgram_: Sending message size=%d peer=%s chan=%d", len,
	       sockAdd2Str_ (peer), chfd);
  ...
  if (channels[chfd].state == CH_CONN)
    ...
    {
      cc =
	sendto (s, buf, len, 0, (struct sockaddr *) peer,
		sizeof (struct sockaddr_in));
...
chanRcvDgram_ (int chfd,
	       char *buf, int len, struct sockaddr_in *peer, int timeout)
{
  int sock;
...
chanRcvDgram_: Receive on chan %d timeout=%d", chfd, timeout);
  ...
  if (timeout < 0)
	...
	cc = recvfrom (sock,
		       buf, len, 0, (struct sockaddr *) peer, &peersize);
      if (SOCK_CALL_FAIL (cc))
	{
	  lserrno = LSE_MSG_SYS;
	  return -1;
...
chanSelect_ (struct Masks *sockmask,
	     struct Masks *chanmask, struct timeval *timeout)
{
  int i;
  int nReady;
  int maxfds;
	  ...
	  struct LSFHeader *outhdr, int timeout)
{
  ...
  xdrmem_create (&xdrs, (char *) &hdrBuf, sizeof (struct LSFHeader),
		 XDR_DECODE);
...
dowrite (int chfd, struct Masks *chanmask)
{
  ...
  cc = write (channels[chfd].handle,
	      sendbuf->data + sendbuf->pos, sendbuf->len - sendbuf->pos);
  if (cc < 0 && BAD_IO_ERR (errno))
    {
      FD_SET (chfd, &(chanmask->emask));
