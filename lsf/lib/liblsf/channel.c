...
chanInit_ (void)
{
    ...
    channels = calloc ((size_t) chanMaxSize, (size_t) sizeof (struct chanData));
    if (channels == NULL)
        return -1;
...
chanConnect_ (int chfd, struct sockaddr_in *peer, int timeout)
{
    ...
    if (timeout >= 0)
    {
        if (b_connect_ (channels[chfd].handle, (struct sockaddr *) peer, sizeof (struct sockaddr_in), (uint)timeout / 1000) < 0)
        {
            if (errno == ETIMEDOUT) {
...
chanOpen_ (u_int iaddr, u_short port, int options)
{
    ...
    if (cc < 0)
    {
        ...
        if (errno != EINPROGRESS)
        {
            ...
            ls_syslog (LOG_ERR, I18N (5002, "chanOpen: connect() failed, cc=%d, errno=%d"), cc, errno); /*catgets 5002 */
...
chanSelect_ (struct Masks *sockmask, struct Masks *chanmask, struct timeval *timeout)
{
    ...
    for (i = 0; i < chanIndex; i++)
    {
        ...
        {
            ls_syslog (LOG_ERR, "\
                %s: channel %d has socket %d but in %s state", __func__, i, channels[i].handle, "CH_FREE");
            continue;
        }
        ...
        if (logclass & LC_COMM)
            ls_syslog (LOG_DEBUG3, "\
                %s: Considering channel %d handle %d state %d type %d", __func__, i, channels[i].handle, (int) channels[i].state, (int) channels[i].type);

        if (channels[i].type == CH_TYPE_TCP
            && channels[i].state != CH_PRECONN
            && !channels[i].recv && !channels[i].send)
            continue;
        ...
        if (logclass & LC_COMM)
            ls_syslog (LOG_DEBUG3, "\
                %s: Adding channel %d handle %d ", __func__, i, channels[i].handle);
    ...
    nReady = select (maxfds,
        &(sockmask->rmask),
        &(sockmask->wmask), &(sockmask->emask), timeout);
    if (nReady <= 0)
    {
        return nReady;
    }
...
chanWrite_ (int chfd, char *buf, size_t len)
{
    ...
    // FIXME function should return size_t. after fixing, remove cast
    return (long) b_write_fix (channels[chfd].handle, buf, len);
...
doread (int chfd, struct Masks *chanmask)
{
    ...
    cc = read (channels[chfd].handle, rcvbuf->data + rcvbuf->pos, (rcvbuf->len - (size_t) rcvbuf->pos));
    if (cc == 0 && errno == EINTR)
    {
        ls_syslog (LOG_ERR, "\
            %s: looks like read() has returned EOF when interrupted by a signal", __func__);
        return;
    }
