...
ls_connect (char *host)
{
  ...
  memcpy ((char *) &res_addr_.sin_addr, (char *) hp->h_addr,
	  (int) hp->h_length);
  ...
  size = sizeof (struct LSFHeader) + sizeof (connReq) +
    ...
    ALIGNWORD_ (connReq.eexec.len) + sizeof (int) * 5;
...
lsReqHandCreate_ (int tid,
		  ...
		  appCompletionHandler appHandler, void *appExtra)
{
...
callRes_ (int s,
	  ...
	  int reqLen,
	  bool_t (*xdrFunc) (),
	  int *rd, struct timeval *timeout, struct lsfAuth *auth)
{
  int cc;
  int t;
  ...
  if ((cc = lsSendMsg_ (s,
			...
			0,
			data,
			reqBuf, reqLen, xdrFunc, b_write_fix, auth)) < 0)
    {
...
lsIRGetRusage_ (int rpid,
		struct jRusage * ru,
		appCompletionHandler appHandler, void *appExtra, int options)
{
  struct
  {
    struct LSFHeader hdr;
...
lsGetRProcRusage (char *host, int pid, struct jRusage *ru, int options)
{
  ...
  request = lsReqHandCreate_ (pid,
			      ...
			      s,
			      (void *) ru,
			      rgetRusageCompletionHandler_, NULL, NULL);
...
lsRSig_ (char *host, int rid, int sig, int options)
{
  ...
  if ((!(options & (RSIG_ID_ISTID | RSIG_ID_ISPID))) || rid < 0)
    {
	    ...
	    struct timeval *timeout, int options)
{
  ...
  if ((!rdyTidCnt && !rdyFdCnt) ||
      (!tidArray && !fdArray) || (!inTidCnt && !inFdCnt))
    return (0);
...
Again:
  ...
  if (rdyTidCnt)
      ...
      for (i = 0; i < inTidCnt; i++)
	{
	  ...
	  xdr_setpos (&xdrs, 0);
	  rc =
	    readDecodeHdr_ (taskEnt->sock, hdrBuf, b_read_fix, &xdrs,
			    &msgHdr);
	  if (rc < 0)
	    {
	      int nTids;
