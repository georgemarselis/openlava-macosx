...
//      so, rename.
int
getAskedHosts_ ( const char *optarg_, char ***askedHosts, unsigned int *numAskedHosts, unsigned long *badIdx, int checkHost)
{
    ...
    static unsigned int nhlist = 0;
    char host[MAXHOSTNAMELEN];

    memset( host, '\0', strlen( host ) );

    if (hlist) {
        for ( unsigned int i = 0; i < nhlist; i++) {
                free (hlist[i]);
        }
        free (hlist);
        hlist = NULL;
