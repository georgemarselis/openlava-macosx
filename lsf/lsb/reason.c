...
lsb_suspreason (int reasons, int subreasons, struct loadIndexLog *ld)
{
  ...
  if (reasons & SUSP_USER_STOP)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 500, " The job was suspended by user;\n"));	/* catgets 500 */
  else if (reasons & SUSP_ADMIN_STOP)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 501, " The job was suspended by LSF admin or root;\n"));	/* catgets 501 */
  else if (reasons & SUSP_QUEUE_WINDOW)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 502, " The run windows of the queue are closed;\n"));	/* catgets 502 */
  else if (reasons & SUSP_HOST_LOCK)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 506, " The execution host is locked by LSF administrator now;\n"));	/* catgets 506 */
  else if (reasons & SUSP_HOST_LOCK_MASTER)
    {
      sprintf (msgbuf, I18N (531, " The execution host is locked by master LIM now;\n"));	/* catgets 531 */
    }
  else if (reasons & SUSP_USER_RESUME)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 507, " Waiting for re-scheduling after being resumed by user;\n"));	/* catgets 507 */
  else if (reasons & SUSP_QUE_STOP_COND)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 508, " STOP_COND is true with current host load;\n"));	/* catgets 508 */
  else if (reasons & SUSP_QUE_RESUME_COND)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 509, " RESUME_COND is false with current host load;\n"));	/* catgets 509 */
  else if (reasons & SUSP_RES_RESERVE)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 510, " Job's requirements for resource reservation not satisfied;\n"));	/* catgets 510 */
  else if (reasons & SUSP_PG_IT)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 511, " Job was suspended due to paging rate (pg) and the host is not idle yet\n"));	/* catgets 511 */

  else if (reasons & SUSP_LOAD_UNAVAIL)
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 512, " Load information on execution host(s) is unavailable\n"));	/* catgets 512 */
  else if (reasons & SUSP_LOAD_REASON)
    {
      strcpy (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 513, " Host load exceeded threshold: "));	/* catgets 513 */
      if (subreasons == R15S)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 514, "%s 15-second CPU run queue length (r15s)\n"), msgbuf);	/* catgets 514 */
      else if (subreasons == R1M)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 515, "%s 1-minute CPU run queue length (r1m)\n"), msgbuf);	/* catgets 515 */
      else if (subreasons == R15M)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 516, "%s 15-minute CPU run queue length (r15m)\n"), msgbuf);	/* catgets 516 */
      else if (subreasons == UT)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 517, "%s 1-minute CPU utilization (ut)\n"), msgbuf);	/* catgets 517 */
      else if (subreasons == IO)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 518, "%s Disk IO rate (io)\n"), msgbuf);	/* catgets 518 */
      else if (subreasons == PG)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 519, "%s Paging rate (pg)\n"), msgbuf);	/* catgets 519 */
      else if (subreasons == IT)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 520, "%s Idle time (it)\n"), msgbuf);	/* catgets 520 */
      else if (subreasons == MEM)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 521, "%s Available memory (mem)\n"), msgbuf);	/* catgets 521 */
      else if (subreasons == SWP)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 522, "%s Available swap space (swp)\n"), msgbuf);	/* catgets 522 */
      else if (subreasons == TMP)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 523, "%s Available /tmp space (tmp)\n"), msgbuf);	/* catgets 523 */
      else if (subreasons == LS)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 524, "%s Number of login sessions (ls)\n"), msgbuf);	/* catgets 524 */
      else
	{
	...
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 525, " RUNLIMIT was reached;\n"));	/* catgets 525 */
      else if (subreasons & SUB_REASON_DEADLINE)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 526, " DEADLINE was reached;\n"));	/* catgets 526 */
      else if (subreasons & SUB_REASON_PROCESSLIMIT)
	sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 527, " PROCESSLIMIT was reached;\n"));	/* catgets 527 */
      else if (subreasons & SUB_REASON_CPULIMIT)
	sprintf (msgbuf, I18N (529, " CPULIMIT was reached;\n"));	/* catgets 529 */
      else if (subreasons & SUB_REASON_MEMLIMIT)
	sprintf (msgbuf, I18N (530, " MEMLIMIT was reached;\n"));	/* catgets 530 */
    }
  else
    sprintf (msgbuf, _i18n_msg_get (ls_catd, NL_SETN, 528, " Unknown suspending reason code: %d\n"), reasons);	/* catgets 528 */
...
userIndexReasons (char *msgline,
		  int resource, int reason, struct loadIndexLog *ld)
{
