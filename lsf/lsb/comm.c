    ...
    *rep_buf = NULL;
    ...
    if (flags & CALL_SERVER_ENQUEUE_ONLY)
        ...
        cc = chanRpc_ (serverSock,
                       &reqbuf, replyBufPtr, replyHdr, recv_timeout * 1000);
        if (cc < 0)
            {
            lsberrno = LSBE_LSLIB;
            CLOSECD (serverSock);
        ...
        if (LSE_SYSCALL (lserrno))
            ...
            if (logclass & LC_COMM)
                ls_syslog (LOG_DEBUG1,
                           "%s: readDecodeHdr_() failed for read reply header from mbatchd: %m",
                           fname);
            }
        else
            lsberrno = LSBE_XDR;
...
Retry:
    ...
    if (clusterName == NULL)
        ...
        {
        clusterInfo = ls_clusterinfo (NULL, (unsigned int *)&num, &clusterName, 1, 0);
    ...
    cc = call_server (masterHost,
                      ...
                      _lsb_recvtimeout,
                      serverSock,
                      postSndFunc, postSndFuncArg, CALL_SERVER_NO_HANDSHAKE);
            ...
            && (lsberrno == LSBE_CONN_TIMEOUT ||
                ...
                (lsberrno == LSBE_LSLIB &&
                 (lserrno == LSE_TIME_OUT ||
                  ...
                  lserrno == LSE_MSG_SYS))) && try < mbdTries ())
            {
            fprintf (stderr, _i18n_msg_get (ls_catd, NL_SETN, 1, "batch system daemon not responding ... still trying\n"));	/* catgets 1 */
            if (logclass & LC_TRACE)
...
Retry:
    ...
    if (tempPtr == NULL)
        ...
        if (lserrno == LSE_TIME_OUT || lserrno == LSE_LIM_DOWN ||
            ...
            {
            fprintf (stderr,
                     _i18n_msg_get (ls_catd, NL_SETN, 2,
