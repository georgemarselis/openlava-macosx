...
logProcessInfo (void)
{
	...
	if (unlink (infofile) < 0 && errno != ENOENT) {
		ls_syslog (LOG_DEBUG, "%s: unlink(%s) failed: %m.", __func__, infofile);
	}

	if (rename (wfile, infofile) < 0) {
		ls_syslog (LOG_ERR, "%s: rename() %s to %s failed: %m.", __func__, wfile, infofile);
	}
	unlink (wfile);

	ls_syslog (LOG_DEBUG, "%s: process table updated.", __func__);
