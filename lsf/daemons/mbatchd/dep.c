...
struct dptNode *
parseDepCond (char *dependCond, struct lsfAuth *auth, int *replyCode,
	      char **badName, int *jFlags, int flags)
{
  ...
  while ((token = getToken (&dependCond, &tokenType)) != NULL)
	...
	case DPT_NOT:
	  ...
	  if ((prev = popStackDep (operatorStack)) != NULL)
	    ...
	    PUSH_STACK (operatorStack, node) break;
	case DPT_LEFT_:
	  if ((node = newNode (tokenType, NULL)) == NULL)
	    goto Error;
	  PUSH_STACK (operatorStack, node) break;
	...
	case DPT_ENDED:
	  ...
	  if ((jobRec = matchJobs (tempToken,
	      ...
	      else if (badName && (*badName) && notExistArrayJob)
		{
		  ...
		  sprintf (jobIdStr, "%d[%d]",
			   LSB_ARRAY_JOBID (notExistArrayJob),
			   LSB_ARRAY_IDX (notExistArrayJob));
	  ...
	  if (numJob > 1)
		...
		my_calloc (numJob, sizeof (struct dptNode *), "parseDepCond");
		...
		{
		  if ((node =
		       newNode (tokenType, (void *) jobRec[0])) == NULL)
		    goto Error;
	...
	case DPT_NAME:
	  ...
	  if (numJob > 1)
	    {
	      if (createMoreNodes (DPT_DONE, numJob, jobRec, jobIdx, NULL) <
		  0)
		goto Error;
	    }
	  else
	...
	case DPT_NUMENDED:
	  {
		...
		*replyCode = LSBE_ARRAY_NULL;
		...
		node =
		  newNode (tokenType, (void *) (ARRAY_DATA (jpbw->jgrpNode)));
	      }
	    else
	      node = newNode (tokenType, NULL);
...
%s: job %s has one to one dependency but dptJobIdx is NULL", __func__, lsb_jobid2str (jobRec->jobId));
		    ...
		    for (ptr1 = listSetIteratorBegin (&iter);
			 ptr1 != NULL; ptr1 = listSetIteratorGetNext (&iter))
		      {
	...
	while (jpbw)
	  {
		...
		break;
	      default:
		ls_syslog (LOG_ERR, I18N (5502, "At line %d of file %s, should never be here"),	/*catgets 5502 */
			   __LINE__, __FILE__);
	      }
	...
	if (opExpr (node->dptUnion.jgrp.opType,
		    getCounterOfDep (node->type, node->dptJgrp->counts), num))
	  node->value = DP_TRUE;
	else
	  node->value = DP_FALSE;
	break;
...
newNode (dptType nodeType, void *data)
{
  ...
  node = (struct dptNode *) my_calloc (1, sizeof (struct dptNode), "newNode");
  node->type = nodeType;
  node->value = DP_FALSE;
  switch (nodeType)
    {
...
initStackDep (void)
{
  ...
  stack->top = -1;
  stack->nodes =
    (struct dptNode **) my_malloc (5 * sizeof (struct dptNode *),
				   "initStackDep");
  stack->size = 5;
  return (stack);
}
  ...
  *element = 0;
  ...
  if (jobId)
      ...
      if (foundJobRec[0])
	{
	  ...
	  if (idxList == NULL && foundJobRec[0]->nodeType == JGRP_NODE_ARRAY)
	    {
	      idxList =
		parseJobArrayIndex (foundJobRec[0]->shared->jobBill.jobName,
				    &error, &maxJLimit);

	      if (idxList)
		{
		  (*jobIdx) =
		    (struct jobIdx *) my_malloc (sizeof (struct jobIdx),
						 "matchJobs");
		  (*jobIdx)->idxList = idxList;
		  (*jobIdx)->depJobList = NULL;
		  (*jobIdx)->numRef = 0;
...
checkIdx:
  ...
  if (flag != NULL && *flag == ARRAY_DEP_ONE_TO_ONE && foundJobRec[0] != NULL)
    {
      ...
      else
	{
	  ...
	  for (i = depArrayIdx->start;
	       i <= depArrayIdx->end; i += depArrayIdx->step)
	    {
	      struct jData *jPtr;
...
evalJobDep (struct dptNode *node, struct jData *jpbw, struct jData *depJob)
{
  ...
  switch (node->type)
    ...
    case DPT_POST_DONE:
      if ((depJob == NULL) || (depJob->endTime < jpbw->endTime))
	{
	  if (IS_POST_DONE (jpbw->jStatus) && (jpbw->jStatus & JOB_STAT_DONE))
	    {
