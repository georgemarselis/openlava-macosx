...
static sbdReplyType callSBD (char *,
			     ...
			     char *,
			     char *,
			     int *, int *, int, struct sbdNode *, int *);
...
sbdReplyType
start_job (struct jData *jDataPtr, struct qData *qp,
	   struct jobReply *jobReply)
{
...
msg_job (struct jData * jp, struct Buffer * mbuf, struct jobReply * jobReply)
{
  ...
  switch (reply)
    ...
    default:
      ls_syslog (LOG_ERR, "\
%s: Job %s invalid reply code %d from sbatchd on host %s", __func__, lsb_jobid2str (jp->jobId), reply, toHost);
      reply = ERR_BAD_REPLY;
    }
	 ...
	 int *cc, int callServerFlags, struct sbdNode *sbdPtr, int *sockPtr)
{
  ...
  *cc = call_server (toHost, sbd_port, request_buf, len, reply_buf, replyHdr, connTimeout,	/* 100 millisec */
		     60 * 30,
		     sockPtr, postSndFunc, postSndFuncArg, callServerFlags);
  if (*cc < 0)
    {
      if (*cc == -2)
	{
	  ...
	  if (equalHost_ (toHost, lastHost))
	    {
	      *cnt = *cnt + 1;
	      if (*cnt >= 5)
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5419, "%s: Failed to call sbatchd on host <%s>: %s"), caller, toHost, lsb_sysmsg ());	/* catgets 5419 */
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5420, "The above failure repeated %d times"), *cnt);	/* catgets 5420 */
		  *cnt = 0;
		}
	    }
	  else if (*cnt >= 0)
	    {
	      ...
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5421, "%s: Failed to call sbatchd on host <%s>: %s"),	/* catgets 5421 */
			 caller, toHost, lsb_sysmsg ());
	      *cnt = 1;
	    }
	  if (*cnt >= 0)
	    {
...
callSbdDebug (struct debugReq * pdebug)
{
  ...
  if (reply != ERR_NO_ERROR)
    ls_syslog (LOG_ERR, "\
%s: Invalid reply code %d from sbatchd on host %s", __func__, reply, pdebug->hostName);
  return reply;
}
