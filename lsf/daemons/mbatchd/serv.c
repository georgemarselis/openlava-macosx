...
int packJgrpInfo (struct jgTreeNode *, int, char **, int, int);
int packJobInfo (struct jData *, int, char **, int, int, int);
void initSubmit (int *, struct submitReq *, struct submitMbdReply *);
int sendBack (int, struct submitReq *, struct submitMbdReply *, int);
void addPendSigEvent (struct sbdNode *sbdPtr);
void freeJobHead (struct jobInfoHead *);
void freeJobInfoReply (struct jobInfoReply *);
void freeShareResourceInfoReply (struct lsbShareResourceInfoReply *);
int xdrsize_QueueInfoReply (struct queueInfoReply *);
	      ...
	      int *schedule, int dispatch, struct jData **jobData)
{
  ...
  if (logclass & (LC_TRACE | LC_EXEC | LC_COMM))
    ls_syslog (LOG_DEBUG, "%s: Entering this routine...; host=%s, socket=%d", __func__, hostName, chanSock_ (chfd));
...
sendback:
  if (reply != 0 || submitReply.jobId <= 0)
	...
	{
	  ls_syslog (LOG_DEBUG, "Job submission failed: <%d> job <%d > JobName<%s> queue <%s> reqReq <%s> hostSpec <%s>  subHomeDir <%s> inFile <%s>  outFile <%s> errFile<%s> command <%s> inFileSpool <%s> commandSpool <%s> chkpntDir <%s> jobFile <%s> fromHost <%s>  cwd <%s> preExecCmd <%s>  mailUser <%s> projectName <%s> loginShell <%s> schedHostType <%s> numAskedHosts <%d> nxf <%d>  ",
		     reply, (nextJobId - 1), subReq.jobName, subReq.queue,
		     subReq.resReq, subReq.hostSpec, subReq.subHomeDir,
		     subReq.inFile, subReq.outFile, subReq.errFile,
		     subReq.command, subReq.inFileSpool, subReq.commandSpool,
		     subReq.chkpntDir, subReq.jobFile, subReq.fromHost,
		     subReq.cwd, subReq.preExecCmd, subReq.mailUser,
		     subReq.projectName, subReq.loginShell,
  ...
  if (logclass & (LC_TRACE | LC_EXEC))
    {
      ls_syslog (LOG_DEBUG,
		 "Job submission before sendBack reply <%d> job <%d > JobName <%s> queue <%s> reqReq <%s> hostSpec <%s> subHomeDir <%s> inFile <%s>  outFile <%s> errFile<%s> command <%s> inFileSpool <%s> commandSpool <%s> chkpntDir <%s>  jobFile <%s> fromHost <%s>  cwd <%s> preExecCmd <%s>  mailUser <%s> projectName <%s> loginShell <%s> schedHostType <%s>  numAskedHosts <%d> nxf <%d>  ",
		 reply, (nextJobId - 1), subReq.jobName, subReq.queue,
		 subReq.resReq, subReq.hostSpec, subReq.subHomeDir,
		 subReq.inFile, subReq.outFile, subReq.errFile,
		 subReq.command, subReq.inFileSpool, subReq.commandSpool,
	       ...
	       struct LSFHeader *reqHdr, int schedule)
{
  ...
  if (logclass & (LC_TRACE | LC_COMM))
    ls_syslog (LOG_DEBUG, "%s: Entering this routine...; channel=%d", __func__,
	       chfd);
  ...
  if (!xdr_jobInfoReq (xdrs, &jobInfoReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_jobInfoReq");
    }
  ...
  if (jobInfoReq.options & HOST_NAME)
    {
      ...
      for (hPtr = (struct hData *) hostList->back;
	   hPtr != (void *) hostList; hPtr = (struct hData *) hPtr->back)
	{
  ...
  for (i = 0; i < listSize; i++)
	...
	{
	  ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "packJobInfo");
	  FREEUP (jgrplist);
	  return -1;
	}
      if (!jgrplist[i].isJData &&
	  ((len = packJgrpInfo ((struct jgTreeNode *) jgrplist[i].info,
				...
				reqHdr->version)) < 0))
	{
	  ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "packJgrpInfo");
	  FREEUP (jgrplist);
	  return -1;
	}
	     ...
	     char **replyBuf, int schedule, int options, int version)
{
  ...
  if (logclass & LC_PEND)
    ls_syslog (LOG_DEBUG3,
	       ...
	       __func__, lsb_jobid2str (jobData->jobId), jobData->oldReason,
  ...
  if (IS_PEND (jobData->jStatus) && !(options & NO_PEND_REASONS))
      ...
      else
	{
	  ...
	  if (svReason == 0)
	    {
	      if (logclass & LC_PEND)
		ls_syslog (LOG_DEBUG2, "%s: Get h/u/q reasons", __func__);
	      k = 0;
		   ...
		   hPtr = (struct hData *) hPtr->back)
		{
		  ...
		  if (pkQReasonTb[i] == PEND_HOST_QUEUE_MEMB)
		    continue;
  ...
  if ((jobData->numHostPtr > 0) && (jobData->hPtr[0] != NULL)
      ...
      if (getModelFactor_r (jobInfoReply.jobBill->hostSpec, &cpuF) < 0)
	{
	  ...
	  if (cpuFactor == NULL)
		...
		{
		  ls_syslog (LOG_ERR, "\
%s: Cannot find cpu factor for hostSpec %s; cpuFactor is set to 1.0", __func__, jobInfoReply.jobBill->fromHost);
		  cpuFactor = &one;
		}
	    }
	  else
	      ...
	      struct LSFHeader *reqHdr)
{
  ...
  if (!portok (from))
    {
      ls_syslog (LOG_ERR, "\
%s: Received status report from bad port %s", __func__, sockAdd2Str_ (from));
      if (reqHdr->opCode != BATCH_RUSAGE_JOB)
	errorBack (chfd, LSBE_PORT, from);
      return -1;
    }
  hp = Gethostbyaddr_ (&from->sin_addr.s_addr, sizeof (in_addr_t), AF_INET);
  if (hp == NULL)
    {
      ls_syslog (LOG_ERR, "\
%s: gethostbyaddr() failed %s", __func__, sockAdd2Str_ (from));
      if (reqHdr->opCode != BATCH_RUSAGE_JOB)
  ...
  if (!xdr_statusReq (xdrs, &statusReq, reqHdr))
	...
	case BATCH_RUSAGE_JOB:
	  ...
	  break;
	default:
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7827, "%s: Unknown request %d"),	/* catgets 7827 */
		     __func__, reqHdr->opCode);
	  reply = LSBE_PROTOCOL;
	}
    }
  ...
  if (!xdr_LSFHeader (&xdrs2, &replyHdr))
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL, __func__, "xdr_LSFHeader", reply);
      xdr_destroy (&xdrs2);
      return -1;
    }
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
...
{
  char __func__] = "do_chunkStatusReq()";
  char reply_buf[MSGSIZE];
  XDR xdrs2;
...
%s: gethostbyaddr() failed %s", __func__, sockAdd2Str_ (from));
      ...
      ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_MM, __func__, "getHostEntryByAddr_",
		 sockAdd2Str_ (from));
      errorBack (chfd, LSBE_BAD_HOST, from);
      return -1;
    }
	       ...
	       struct LSFHeader *reqHdr)
{
  ...
  if (!portok (from))
    {
      ls_syslog (LOG_ERR, "\
%s: Received request from bad port %s", __func__, sockAdd2Str_ (from));
      errorBack (chfd, LSBE_PORT, from);
      return -1;
    }
  ...
  if ((hData = getHostData (hp->h_name)) == NULL)
    {
       ...
       */
      ls_syslog (LOG_ERR, "\
%s: Got registration request from unknown host %s at %s", __func__, hp->h_name, sockAdd2Str_ (from));
      errorBack (chfd, LSBE_BAD_HOST, from);
      return -1;
    }
  hStatChange (hData, 0);
  ...
  if (chanWrite_ (chfd, reply_buf, cc) <= 0)
    ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "chanWrite_", cc);
...
do_hostInfoReq (XDR * xdrs,
		int chfd, struct sockaddr_in *from, struct LSFHeader *reqHdr)
{
  char *reply_buf;
  XDR xdrs2;
  struct LSFHeader replyHdr;
...
out:
  ...
  if (!xdr_encodeMsg (&xdrs2,
		      replyStruct, &replyHdr, xdr_hostDataReply, 0, NULL))
    {
      ls_syslog (LOG_ERR, "\
%s: failed encode %dbytes reply to %s", __func__, XDR_GETPOS (&xdrs2), sockAdd2Str_ (from));
  ...
  if (!xdr_infoReq (xdrs, &userInfoReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, "\
%s: failed decoding userInfoReq from %s %M", __func__, sockAdd2Str_ (from));
    }
  else
    {
	...
	: userInfoReq.numNames;
      userInfoReply.users = my_calloc (count,
				       sizeof (struct userInfoEnt), __func__);
  ...
  if (!xdr_encodeMsg (&xdrs2, replyStruct, &hdr, xdr_userInfoReply, 0, NULL))
    {
      ls_syslog (LOG_ERR, "\
%s: xdr_encodeMsg() to %s failed", __func__, sockAdd2Str_ (from));
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    {
      ls_syslog (LOG_ERR, "\
%s: chanWrite_() %dbytes to %s failed", __func__, XDR_GETPOS (&xdrs2), sockAdd2Str_ (from));
      xdr_destroy (&xdrs2);
      FREEUP (reply_buf);
      FREEUP (userInfoReply.users);
      return -1;
		 ...
		 int chfd, struct sockaddr_in *from, struct LSFHeader *reqHdr)
{
  ...
  qInfoReply.queues = my_calloc (numofqueues,
				 sizeof (struct queueInfoEnt), __func__);
...
%s: failed decode data from %s", __func__, sockAdd2Str_ (from));
  ...
  if (!xdr_encodeMsg (&xdrs2, replyStruct, &hdr, xdr_queueInfoReply, 0, NULL))
    {
      ls_syslog (LOG_ERR, "\
%s: failed encode data to %s", __func__, sockAdd2Str_ (from));
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    {
      ls_syslog (LOG_ERR, "\
%s: failed write() %d bytes to %s", __func__, XDR_GETPOS (&xdrs2), sockAdd2Str_ (from));
      freeQueueInfoReply (&qInfoReply, replyStruct);
      FREEUP (reply_buf);
      xdr_destroy (&xdrs2);
      return -1;
...
%s: failed decode data from %s", __func__, sockAdd2Str_ (from));
      ...
      else
	{
	  ...
	  groupInfoReply.groups = (struct groupInfoEnt *)
	    my_calloc (numofhgroups,
		       sizeof (struct groupInfoEnt), "do_groupInfoReq");
  ...
  if (!xdr_encodeMsg (&xdrs2, replyStruct, &hdr, xdr_groupInfoReply, 0, NULL))
    {
      ls_syslog (LOG_ERR, "\
%s: failed encode data to %s", __func__, sockAdd2Str_ (from));
      FREEUP (reply_buf);
		 ...
		 struct LSFHeader *reqHdr)
{
  ...
  if (!xdr_infoReq (xdrs, &infoReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_infoReq");
    }
  else
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "chanWrite_",
		 XDR_GETPOS (&xdrs2));
      xdr_destroy (&xdrs2);
      FREEUP (reply_buf);
      return -1;
...
do_queueControlReq (XDR * xdrs, int chfd, struct sockaddr_in *from,
		    ...
		    struct lsfAuth *auth)
{
  char __func__] = "do_queueControlReq()";
  ...
  if (!xdr_controlReq (xdrs, &bqcReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_controlReq");
    }
  else
    {
  ...
  if (!xdr_LSFHeader (&xdrs2, &replyHdr))
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "xdr_LSFHeader", reply);
      xdr_destroy (&xdrs2);
      return -1;
    }
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
		...
		struct LSFHeader *reqHdr)
{
  ...
  if (!xdr_LSFHeader (&xdrs2, &replyHdr))
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "xdr_LSFHeader",
		 replyHdr.opCode);
    }
  if (b_write_fix (s, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "b_write_fix",
	       XDR_GETPOS (&xdrs2));
  xdr_destroy (&xdrs2);
  millisleep_ (3000);
		...
		char *hostName, struct LSFHeader *reqHdr)
{
  ...
  if (!xdr_LSFHeader (&xdrs2, &replyHdr))
    {
      ls_syslog (LOG_ERR, "\
%s: decode header for operation %d from %s failed", __func__, replyHdr.opCode, sockAdd2Str_ (from));
    }
...
int
do_hostControlReq (XDR * xdrs, int chfd, struct sockaddr_in *from,
		   char *hostName, struct LSFHeader *reqHdr,
		   struct lsfAuth *auth)
{
  char __func__] = "do_hostControlReq()";
...
int
do_jobSwitchReq (XDR * xdrs, int chfd, struct sockaddr_in *from,
		 char *hostName, struct LSFHeader *reqHdr,
		 struct lsfAuth *auth)
{
  char __func__] = "do_jobSwitchReq()";
  char reply_buf[MSGSIZE];
  XDR xdrs2;
  ...
  if (!xdr_jobSwitchReq (xdrs, &jobSwitchReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_jobSwitchReq");
    }
  else
    {
	       ...
	       struct LSFHeader *reqHdr, struct lsfAuth *auth)
{
  ...
  if (!xdr_jobMoveReq (xdrs, &jobMoveReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_jobMoveReq");
    }
  else
    {
	      ...
	      struct LSFHeader *reqHdr, struct lsfAuth *auth)
{
  ...
  if (!xdr_modifyReq (xdrs, &modifyReq, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_modifyReq");
      goto sendback;
    }
	  ...
	  struct submitMbdReply *submitReply, int chfd)
{
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) !=
      XDR_GETPOS (&xdrs2))
    ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M, __func__, "chanWrite_",
	       XDR_GETPOS (&xdrs2));
  xdr_destroy (&xdrs2);
...
doNewJobReply (struct sbdNode *sbdPtr, int exception)
{
  ...
  if (exception == TRUE || chanRecv_ (sbdPtr->chanfd, &buf) < 0)
    {
      if (exception == TRUE)
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7887, "%s: Exception bit of <%d> is set for job <%s>"),	/* catgets 7887 */
		   __func__, sbdPtr->chanfd, lsb_jobid2str (jData->jobId));
      else
	ls_syslog (LOG_ERR, I18N_JOB_FAIL_S,
		   __func__, lsb_jobid2str (jData->jobId), "chanRecv_");
  ...
  if (!xdr_LSFHeader (&xdrs, &replyHdr))
    {
      ls_syslog (LOG_ERR, I18N_JOB_FAIL_S,
		 __func__, lsb_jobid2str (jData->jobId), "xdr_LSFHeader");
  ...
  if (replyHdr.opCode != ERR_NO_ERROR)
      ...
      if (IS_START (jData->jStatus))
	{
	  ...
	  replayReason =
	    jobStartError (jData, (sbdReplyType) replyHdr.opCode);
  ...
  if (!xdr_jobReply (&xdrs, &jobReply, &replyHdr))
    {
      ls_syslog (LOG_ERR, I18N_JOB_FAIL_S,
		 __func__, lsb_jobid2str (jData->jobId), "xdr_jobReply");
      if (IS_START (jData->jStatus))
	{
	  jData->newReason = PEND_JOB_START_FAIL;
	  jStatusChange (jData, JOB_STAT_PEND, LOG_IT, __func__);
  ...
  if (IS_START (jData->jStatus))
      ...
      if (daemonParams[LSB_MBD_BLOCK_SEND].paramValue == NULL)
	{
	  ...
	  if (chanAllocBuf_ (&replyBuf, sizeof (struct LSFHeader)) < 0)
	    {
	      ls_syslog (LOG_ERR, I18N_FUNC_FAIL_M, __func__, "chanAllocBuf_");
	      goto Leave;
	    }
	  ...
	  if (chanEnqueue_ (sbdPtr->chanfd, replyBuf) < 0)
	    {
	      ls_syslog (LOG_ERR, I18N_FUNC_FAIL_M, __func__, "chanEnqueue");
  ...
  if (exception == TRUE || chanRecv_ (sbdPtr->chanfd, &buf) < 0)
    {
      ...
      if (exception == TRUE)
	ls_syslog (LOG_ERR, "\
%s: exception bit %d is set for host %s", __func__, sbdPtr->chanfd, toHost);
      else
	ls_syslog (LOG_ERR, "\
...
%s: xdr_LSFHeader() failed calling host %s", __func__, toHost);
  ...
  if (replyHdr.opCode != ERR_NO_ERROR)
    {
      ls_syslog (LOG_ERR, "\
%s: sbatchd replied %d from host %s", __func__, replyHdr.opCode, toHost);
      goto hout;
    }
...
{
  char __func__] = "doSwitchJobReply";
  struct LSFHeader replyHdr;
  XDR xdrs;
  struct jData *jData = sbdPtr->jData;
  char *toHost = sbdPtr->hData->host;
  ...
  if (exception == TRUE || chanRecv_ (sbdPtr->chanfd, &buf) < 0)
    {
      if (exception == TRUE)
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7898, "%s: Exception bit of <%d> is set for job <%s>"),	/* catgets 7898 */
		   __func__, sbdPtr->chanfd, lsb_jobid2str (jData->jobId));
      else
	ls_syslog (LOG_ERR, I18N_JOB_FAIL_S,
		   __func__, lsb_jobid2str (jData->jobId), "chanRecv_");
  ...
  switch (replyHdr.opCode)
	  ...
	  || jData->jobId != jobReply.jobId)
	{
	  if (jData->jobId != jobReply.jobId)
	    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7901, "%s: Got bad jobId <%s> for job <%s>"),	/* catgets 7901 */
		       __func__,
    ...
    case ERR_JOB_FINISH:
      break;
    case ERR_BAD_REQ:
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7903, "%s: Job <%s>: sbatchd on host <%s> complained of bad request"), __func__, lsb_jobid2str (jData->jobId), toHost);	/* catgets 7903 */
      break;
    default:
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7904, "%s: Job <%s>: Illegal reply code <%d> from sbatchd on host <%s>"), __func__, lsb_jobid2str (jData->jobId), replyHdr.opCode, toHost);	/* catgets 7904 */
    }
...
{
  char __func__] = "doSignalJobReply";
  struct LSFHeader replyHdr;
  XDR xdrs;
  struct jData *jData = sbdPtr->jData;
  ...
  if (exception == TRUE || chanRecv_ (sbdPtr->chanfd, &buf) < 0)
    {
      if (exception == TRUE)
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7905, "%s: Exception bit of <%d> is set for job <%s>"),	/* catgets 7905 */
		   __func__, sbdPtr->chanfd, lsb_jobid2str (jData->jobId));
      else
  ...
  switch (replyHdr.opCode)
	...
	{
	  if (jData->jobId != jobReply.jobId)
	    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7908, "%s: Got bad jobId <%s> for job <%s>"),	/* catgets 7908 */
		       __func__, lsb_jobid2str (jobReply.jobId), jData->jobId);
	  else
	    ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_M,
		       __func__, lsb_jobid2str (jData->jobId), "xdr_jobReply");
	  addPendSigEvent (sbdPtr);
...
ctrlMbdDebug (struct debugReq *pdebug, struct lsfAuth *auth)
{
  ...
  if (!isAuthManager (auth) && auth->uid != 0)
    {
      ls_syslog (LOG_CRIT, I18N (7916, "ctrlMbdDebug: uid <%d> not allowed to perform control operation"),	/*catgets 7916 */
		 auth->uid);
  ...
  if (opCode == MBD_DEBUG)
	...
	ls_syslog (LOG_DEBUG,
      ...
      if (logclass & LC_TRACE)
	ls_syslog (LOG_DEBUG, "%s: LSF_LOG_MASK =%s", __func__,
		   daemonParams[LSF_LOG_MASK].paramValue);
	     ...
	     char *hostName, struct LSFHeader *reqHdr, struct lsfAuth *auth)
{
  ...
  if (!xdr_debugReq (xdrs, &debugReq, reqHdr))
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7912, "%s: Bad opCode <%d>"),	/* catgets 7912 */
		     __func__, debugReq.opCode);
	  return -1;
	}
    }
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M,
		 __func__, "chanWrite_", XDR_GETPOS (&xdrs2));
      xdr_destroy (&xdrs2);
      return -1;
    }
  xdr_destroy (&xdrs2);
...
do_resourceInfoReq (XDR * xdrs, int chfd, struct sockaddr_in *from,
		    struct LSFHeader *reqHdr)
{
  char __func__] = "do_resourceInfoReq";
  XDR xdrs2;
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&xdrs2)) <= 0)
    {
      ls_syslog (LOG_ERR, I18N_FUNC_D_FAIL_M,
		 __func__, "b_write_fix", XDR_GETPOS (&xdrs2));
      xdr_destroy (&xdrs2);
      FREEUP (reply_buf);
      freeShareResourceInfoReply (&resInfoReply);
      return -1;
	      ...
	      struct lsfAuth *auth, struct LSFHeader *reqHeader)
{
  ...
  if (!xdr_runJobReq (xdrs, &runJobRequest, reqHeader))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_runJobReq");
      goto Reply;
    }
...
Reply:
  ...
  if (chanWrite_ (chfd, reply_buf, XDR_GETPOS (&replyXdr)) <= 0)
    {
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL_MM, __func__, "chanWrite_");
      xdr_destroy (&replyXdr);
      return -1;
    }
	       ...
	       struct LSFHeader *reqHdr, struct lsfAuth *auth)
{
  ...
  if (!xdr_jobAttrReq (xdrs, &jobAttr, reqHdr))
    {
      reply = LSBE_XDR;
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "xdr_jobAttrReq");
    }
  else
    {
  ...
  if (chanWrite_ (s, reply_buf, XDR_GETPOS (&xdrs2)) != XDR_GETPOS (&xdrs2))
    {
      ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__, "chanWrite_");
      xdr_destroy (&xdrs2);
      return -1;
    }
  xdr_destroy (&xdrs2);
