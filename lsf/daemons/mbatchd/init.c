...
%s: Ohmygosh ls_gethostinfo() failed just now... %M", __func__);
  ...
  if ((hPtr = getHostData (masterHost)) == NULL)
    {
      ls_syslog (LOG_ERR, "\
%s: Master host %s is not defined in the lsb.hosts file", __func__, masterHost);
      mbdDie (MASTER_FATAL);
    }
  if (defaultHostSpec != NULL && !validHostSpec (defaultHostSpec))
    {
      ls_syslog (LOG_ERR, "\
%s: Invalid system defined DEFAULT_HOST_SPEC %s; ignored", __func__, defaultHostSpec);
...
readHostConf (int mbdInitFlags)
{
  ...
  if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF)
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: lsb.hosts not found %M, all hosts known by LSF will be used", __FUNCTION__);
	  addDefaultHost ();
	  return 0;
	}
    }
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: lsb.hosts not found, all hosts known by LSF will be used", __FUNCTION__);
	  addDefaultHost ();
	  return 0;
	}
    }
...
%s: lsb.users not found %M, default user will be used", __FUNCTION__);
	  userConf = my_calloc (1, sizeof (struct userConf), __FUNCTION__);
	  goto defaultUser;
	}
...
readQueueConf (int mbdInitFlags)
{
  ...
  if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF)
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: lsb.queues not found %M, using default queue", __FUNCTION__);
	  createDefQueue ();
	  return;
	}
    }
  ...
  if (numQueues && defaultQueues)
      ...
      while ((word = getNextWord_ (&cp)))
	{
	  if ((qp = getQueueData (word)) == NULL
	    ...
	    {
	      ls_syslog (LOG_WARNING, "\
%s: Invalid queue name specified by parameter DEFAULT_QUEUE; ignoring <%s>", __FUNCTION__, word);
	      lsb_CheckError = WARNING_ERR;
	    }
	  else
	    {
  ...
  if (!new)
    {
       ...
       */
      ls_syslog (LOG_WARNING, "\
%s: Host %s appears to be multiply defined nth instance ignored", __func__, thPtr->host);
      return;
    }
  ...
  if (lsf)
	...
	{
	  ls_syslog (LOG_DEBUG, "\
%s: numCPUs <%d> of host <%s> is not greater than 0; assuming as 1", __func__, lsf->maxCpus, lsf->hostName);
	  hPtr->numCPUs = 1;
	}
...
addGroup (struct gData **groups, char *gname, int *ngroups)
{
  ...
  groups[*ngroups] = my_calloc (1, sizeof (struct gData), "addGroup");
...
readParamConf (int mbdInitFlags)
{
  ...
  if (mbdInitFlags == FIRST_START || mbdInitFlags == RECONFIG_CONF)
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: lsb.params can not be found %M, using default parameters", __FUNCTION__);
	  return;
	}
    }
...
lostFoundQueue (void)
{
  ...
  qPtr->description =
    safeSave
    ("This queue is created by the system to hold the jobs that were in the queues being removed from the configuration by the LSF Administrator.  Jobs in this queue will not be started unless they are switched to other queues.");
	    ...
	    char *filename, struct gData *groups[], int *ngroups)
{
  ...
  while (unixGrp->gr_mem[++i] != NULL)
	...
	{
	  ls_syslog (LOG_WARNING, _i18n_msg_get (ls_catd, NL_SETN, 6179, "%s: Unknown user <%s> in group <%s>"),	/* catgets 6179 */
...
%s: No valid queue in queueConf structure", __func__);
  ...
  for (i = 0; i < queueConf->numQueues; i++)
	...
	parseGroups (USER_GRP,
		     &qPtr->uGPtr, queue->userList, (char *) __func__);
	...
	if (qPtr->uGPtr->memberTab.numEnts == 0
	    && qPtr->uGPtr->numGroups == 0)
	  {
	    ...
	    ls_syslog (LOG_ERR, "\
%s: No valid value for key USERS in the queue <%s>; ignoring the queue", __func__, qPtr->queue);
	    lsb_CheckError = WARNING_ERR;
	    badqueue = TRUE;
	  }
      if (qPtr->hostList != NULL
	  && strcmp (qPtr->hostList, "none")
	  && qPtr->numAskedPtr <= 0 && qPtr->askedOthPrio <= 0)
	{
	  ls_syslog (LOG_ERR, "\
%s: No valid value for key HOSTS in the queue <%s>; ignoring the queue", __func__, qPtr->queue);
	  lsb_CheckError = WARNING_ERR;
	  badqueue = TRUE;
	}
      ...
      if (queue->windows != NULL)
	{
	  ...
	  *(qPtr->windows) = '\0';
	  ...
	  while ((word = getNextWord_ (&sp)) != NULL)
		...
		{
		  ls_syslog (LOG_ERR, "\
%s: Bad time expression %s/%s in queue window %s; ignored", __func__, queue->windows, save, qPtr->queue);
		  lsb_CheckError = WARNING_ERR;
		  freeWeek (qPtr->weekR);
		  free (save);
		  continue;
      ...
      if (queue->windowsD)
	{
	  ...
	  *(qPtr->windowsD) = '\0';
	  ...
	  while ((word = getNextWord_ (&sp)) != NULL)
		...
		{
		  ls_syslog (LOG_ERR, "\
%s: Bad time expression %s/%s in queue dispatch windows %s; ignored", __func__, queue->windowsD, save, qPtr->queue);
		  lsb_CheckError = WARNING_ERR;
		  freeWeek (qPtr->week);
		  free (save);
		  continue;
      ...
      if (queue->defaultHostSpec)
	{
	  ...
	  if (cpuFactor == NULL)
	    {
	      ls_syslog (LOG_ERR, "\
%s: Invalid hostspec %s for %s in queue <%s>; ignored", __func__, queue->defaultHostSpec, "DEFAULT_HOST_SPEC", qPtr->queue);
	      lsb_CheckError = WARNING_ERR;
	    }
	  if (cpuFactor != NULL)
      ...
      if (queue->resReq)
	{
	  qPtr->resValPtr = checkResReq (queue->resReq,
					 USE_LOCAL
					 | PARSE_XOR | CHK_TCL_SYNTAX);
	  if (qPtr->resValPtr == NULL)
	    {
	      ls_syslog (LOG_ERR, "\
%s: invalid RES_REQ %s in queues %s; ignoring", __func__, queue->resReq, qPtr->queue);
	      lsb_CheckError = WARNING_ERR;
	    }
	  else
      ...
      if (queue->resumeCond)
	{
	  ...
	  if (resValPtr == NULL)
	    {
	      ls_syslog (LOG_ERR, "\
%s: invalid RESUME_COND %s in queue %s; ignoring", __func__, queue->resumeCond, qPtr->queue);
	      lsb_CheckError = WARNING_ERR;
	    }
	  else
	    {
      ...
      if (queue->stopCond)
	{
	  ...
	  if (resValPtr == NULL)
	    {
	      ls_syslog (LOG_ERR, "\
%s: invalid STOP_COND %s in queue %s; ignoring", __func__, queue->stopCond, qPtr->queue);
	      lsb_CheckError = WARNING_ERR;
	    }
	  else
	    {
...
updAllConfCond (void)
{
  ...
  if (userFileConf != NULL && updCondData (userFileConf, USER_FILE) == TRUE)
    needReadAgain = TRUE;
  if (queueFileConf != NULL
      && updCondData (queueFileConf, QUEUE_FILE) == TRUE)
    needReadAgain = TRUE;
  if (paramFileConf != NULL
      && updCondData (paramFileConf, PARAM_FILE) == TRUE)
    needReadAgain = TRUE;
...
initConfData (void)
{
  ...
  cData =
    (struct condData *) my_malloc (sizeof (struct condData), "initConfData");
  cData->name = NULL;
  cData->status = FALSE;
...
parseQHosts (struct qData *qp, char *hosts)
{
  ...
  returnErr = chkAskedHosts (numAskedHosts, askedHosts, numofprocs,
			     &numReturnHosts, &returnHosts, &badReqIndx,
			     &others, 0);
