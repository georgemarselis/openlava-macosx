...
main (int argc, char **argv)
{
  ...
  if (daemonParams[LSB_HJOB_PER_SESSION].paramValue != NULL)
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: Invalid LSB_HJOB_PER_SESSION %s ignored", __func__, daemonParams[LSB_HJOB_PER_SESSION].paramValue);
	}
    }

  if ((daemonParams[LSB_MOD_ALL_JOBS].paramValue != NULL)
      && (strcasecmp (daemonParams[LSB_MOD_ALL_JOBS].paramValue, "y") == 0
	  || strcasecmp (daemonParams[LSB_MOD_ALL_JOBS].paramValue,
			 "yes") == 0))
    {
...
%s: Omygosh... cannot resolve my own name %s", __func__, myhostnm);
  ...
  for (;;)
      ...
      if (now - lastElockTouch >= msleeptime)
	{
      ...
      if (now - lastPeriodicCheckTime > 5 * 60 && lastPeriodicCheckTime != 0)
	{
	  hsKeeping = FALSE;
	  timeout.tv_sec = 0;
	}
...
%s: gethostbyaddr() failed for %s", __func__, sockAdd2Str_ (&from));
  ...
  ls_syslog (LOG_DEBUG, "\
%s: Received request from host %s %s on socket %d", __func__, hp->h_name, sockAdd2Str_ (&from), chanSock_ (s));
...
%s: Received request <%d> from host <%s/%s> on channel <%d>", __func__, mbdReqtype, client->fromHost, sockAdd2Str_ (&from), s);
  ...
  switch (mbdReqtype)
    ...
    case BATCH_JOB_SUB:
      jobData = NULL;
      TIMEIT (0,
	      do_submitReq (&xdrs, s, &from, client->fromHost, &reqHdr,
			    &laddr, &auth, &schedule1, dispatch, &jobData),
	      "do_submitReq()");
    ...
    case BATCH_JOB_SIG:
      TIMEIT (0,
	      do_signalReq (&xdrs, s, &from, client->fromHost, &reqHdr,
			    &auth), "do_signalReq()");
      break;
    case BATCH_JOB_MSG:
	...
	{
	  TIMEIT (0,
		  do_jobMsg (bucket, &xdrs, s, &from, client->fromHost,
			     &reqHdr, &auth), "do_jobMsg()");
	}
      else
	{
    ...
    case BATCH_JOB_MIG:
      TIMEIT (0,
	      do_migReq (&xdrs, s, &from, client->fromHost, &reqHdr, &auth),
	      "do_migReq()");
      if (mSchedStage == 0)
	{
    ...
    case BATCH_HOST_CTRL:
      TIMEIT (0,
	      do_hostControlReq (&xdrs, s, &from, client->fromHost, &reqHdr,
				 &auth), "do_hostControlReq()");
      break;
    case BATCH_JOB_SWITCH:
      TIMEIT (3,
	      do_jobSwitchReq (&xdrs, s, &from, client->fromHost, &reqHdr,
			       &auth), "do_jobSwitchReq()");
      break;
    case BATCH_JOB_MOVE:
      TIMEIT (3,
	      do_jobMoveReq (&xdrs, s, &from, client->fromHost, &reqHdr,
			     &auth), "do_jobMoveReq()");
      break;
    ...
    case BATCH_JOB_PEEK:
      TIMEIT (0,
	      do_jobPeekReq (&xdrs, s, &from, client->fromHost, &reqHdr,
			     &auth), "do_jobPeekReq()");
      break;
    case BATCH_USER_INFO:
      TIMEIT (0, do_userInfoReq (&xdrs, s, &from, &reqHdr),
	      "do_userInfoReq()");
      break;
    ...
    case BATCH_HOST_INFO:
      TIMEIT (3, do_hostInfoReq (&xdrs, s, &from, &reqHdr),
	      "do_hostInfoReq()");
      break;
...
endLoop:
  ...
  if ((reqHdr.opCode != PREPARE_FOR_OP &&
       ...
       reqHdr.opCode != BATCH_STATUS_CHUNK) || statusReqCC < 0)
    {
      shutDownClient (client);
      return (-1);
    }
...
shutDownClient (struct clientNode *client)
{
  if ((client->reqType == BATCH_STATUS_JOB
       ...
       || client->reqType == BATCH_STATUS_CHUNK) && client->lastTime)
    nSbdConnections--;
...
#define T15MIN (60*15)
  ...
  ls_syslog (LOG_DEBUG, "\
%s: mSchedStage=%x schedule=%d eventPending=%d now=%d lastSchedTime=%d nextSchedTime=%d", __func__, mSchedStage, schedule, eventPending, (int) now, (int) lastSchedTime, (int) nextSchedTime);
	     ...
	     struct sockaddr_in *local, char *hostName, int s)
{
  ...
  if (!xdr_lsfAuth (xdrs, auth, reqHdr))
    {
      ls_syslog (LOG_ERR, "\
%s: Ohmygosh failed to decode auth from %s", __func__, sockAdd2Str_ (from));
      return LSBE_XDR;
    }
...
%s: Root user's job submission rejected", __func__);
	...
	{
	  ls_syslog (LOG_CRIT, "\
%s: uid %d not allowed to perform control operation", __func__, auth->uid);
	  return LSBE_PERMISSION;
	}
      break;
    default:
...
shutdownSbdConnections (void)
{
  ...
  ls_syslog (LOG_DEBUG, "\
%s: nSbdConnections=%d maxSbdConnections=%d", __func__, nSbdConnections, maxSbdConnections);
