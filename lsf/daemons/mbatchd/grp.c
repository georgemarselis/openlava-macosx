...
void catMembers (struct gData *, char *, char);
void traverseGroupTree (struct gData *);
void checkuDataSet (struct uData *);
struct groupInfoEnt *getGroupInfoEnt (char **, int *, char);
struct groupInfoEnt *getGroupInfoEntFromUdata (struct uData *, char);
void getAllHostGroup (struct gData **, int, int,
				 struct groupInfoEnt *, int *);
void getSpecifiedHostGroup (char **, int,
				   struct gData **, int, int,
				   struct groupInfoEnt *, int *);
void copyHostGroup (struct gData *, int, struct groupInfoEnt *);
...
expandGrp (struct gData *gp, char *gName, int *num)
{
  ...
  *num = countEntries (gp, TRUE);
  ...
  memberTab = my_calloc (*num, sizeof (char *), "expandGrp");
...
uDataGroupCreate ()
{
  char __func__] = "uDataGroupCreate()";
  int i;
	  ...
	  if (setIsMember (uGrpAllAncestorSet, (void *) ancestor) == FALSE)
		{
		  ...
		  sprintf (strBuf,
			   "User <%s>'s all user set observer", ancestor->user);
...
{
  char __func__] = "traverseGroupTree";
  ...
  if (u->children == NULL)
	{
	  ...
	  u->children = setCreate (MAX_GROUPS,
				   getIndexByuData, getuDataByIndex, strBuf);
	}
...
uDataTableCreate ()
{
  char __func__] = "uDataTableCreate()";
  UDATA_TABLE_T *this;
...
uDataTableFree (UDATA_TABLE_T * uTab)
{
  ...
  if (!uTab)
	return;
  FREEUP (uTab->_base_) FREEUP (uTab);
  return;
...
{
  char __func__] = "uDataTableAddEntry()";
...
{
  char __func__] = "userSetOnNewUser";
  LS_BITSET_T *set;
