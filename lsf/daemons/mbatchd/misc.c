...
updCounters (struct jData *jData, int oldStatus, time_t eventTime)
{
 char __func__] = "updCounters";
  ...
  switch (MASK_STATUS (jData->jStatus & ~JOB_STAT_UNKWN))
	...
	{
	  ls_syslog (LOG_DEBUG2, I18N (7014, "%s: Job %s RWAIT to RUN"),	/* catgets 7014 */
		     __func__, lsb_jobid2str (jData->jobId));
	}
      else
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7000, "%s: Job <%s> transited from %d to JOB_STAT_RUN"),	/* catgets 7000 */
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7001, "%s: Job <%s> transited from %d to JOB_STAT_SSUSP"),	/* catgets 7001 */
		     __func__, lsb_jobid2str (jData->jobId), oldStatus);
	}
      break;
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7002, "%s: Job <%s> transited from %d to JOB_STAT_USUSP"),	/* catgets 7002 */
		     __func__, lsb_jobid2str (jData->jobId), oldStatus);
	}
      break;
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7003, "%s: Job <%s> transited from %x to %x"),	/* catgets 7003 */
		     __func__, lsb_jobid2str (jData->jobId), oldStatus,
		     jData->jStatus);
	}
      break;
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7004, "%s: Job <%s> transited from %d to %d"),	/* catgets 7004 */
		     __func__, lsb_jobid2str (jData->jobId),
    ...
    case JOB_STAT_RUN | JOB_STAT_WAIT:
      if (oldStatus & JOB_STAT_RUN)
	{
	  ls_syslog (LOG_DEBUG2, I18N (7019, "%s: Job %s RUN to RWAIT"),	/* catgets 7019 */
		     __func__, lsb_jobid2str (jData->jobId));
	}
      else if (oldStatus & JOB_STAT_PEND)
	{
	  ls_syslog (LOG_DEBUG2, I18N (7020, "%s: Job %s PEND to RWAIT"),	/* catgets 7020 */
		     __func__, lsb_jobid2str (jData->jobId));
	}
      else
	{
	  ls_syslog (LOG_ERR, I18N (7021, "%s: Job %s transited from %d to %d"),	/* catgets 7021 */
		     __func__, lsb_jobid2str (jData->jobId), oldStatus,
		     jData->jStatus);
	}
      break;
 ...
 char __func__] = "updQaccount";
  ...
  if (logclass & LC_JLIMIT)
    ls_syslog (LOG_DEBUG1,
	       "%s: Entering with job=%s queue=%s numJobs=%d numPEND=%d numRUN=%d numSSUSP=%d numUSUSP=%d numRESERVE=%d",
	       __func__, lsb_jobid2str (jp->jobId), qp->queue, numJobs, numPEND,
	       numRUN, numSSUSP, numUSUSP, numRESERVE);


  addValue (&qp->numJobs, numJobs, jp, __func__, "numJobs");
  addValue (&qp->numPEND, numPEND, jp, __func__, "numPEND");
  addValue (&qp->numRUN, numRUN, jp, __func__, "numRUN");
  addValue (&qp->numSSUSP, numSSUSP, jp, __func__, "numSSUSP");
  addValue (&qp->numUSUSP, numUSUSP, jp, __func__, "numUSUSP");
  addValue (&qp->numRESERVE, numRESERVE, jp, __func__, "numRESERVE");

  if (logclass & LC_JLIMIT)
    ls_syslog (LOG_DEBUG2,
	       "%s: job=%s queue=%s numJobs=%d numPEND=%d numRUN=%d numSSUSP=%d numUSUSP=%d numRESERVE=%d",
	       __func__, lsb_jobid2str (jp->jobId), qp->queue, qp->numJobs,
	       qp->numPEND, qp->numRUN, qp->numSSUSP, qp->numUSUSP,
	       qp->numRESERVE);
  ...
  updUAcct (jp, jp->uPtr, &(qp->uAcct), numRUN, numSSUSP, numUSUSP,
	    numRESERVE, numPEND, (struct hData *) NULL, NULL, (void *) qp);
...
void
updUAcct (struct jData *jData, struct uData *up, struct hTab **uAcct, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE, int numPEND, struct hData *hp, void (*onNewEntry) (struct userAcct *, void *), void *extra)
{
	struct userAcct *foundU = NULL;
	struct qData *qp = jData->qPtr;
	int numSlots = 0;

	if (*uAcct == NULL) {
		*uAcct = my_malloc (sizeof (struct hTab), __func__);
		h_initTab_ (*uAcct, 4);
	}

	if ((foundU = getUAcct (*uAcct, up)) != NULL) {
		addValue (&foundU->numRUN, numRUN, jData, __func__, "numRUN");
		addValue (&foundU->numSSUSP, numSSUSP, jData, __func__, "numSSUSP");
		addValue (&foundU->numUSUSP, numUSUSP, jData, __func__, "numUSUSP");
		addValue (&foundU->numRESERVE, numRESERVE, jData, __func__, "numRESERVE");
		addValue (&foundU->numPEND, numPEND, jData, __func__, "numPEND");
		if (numRESERVE != 0 && !(jData->jStatus & JOB_STAT_PEND)) {
			addValue (&foundU->numNonPrmptRsv, numRESERVE, jData, __func__,   "numNonPrmptRsv");
		}
		if (logclass & LC_JLIMIT) {
      		/* catgets NULL */
			ls_syslog (LOG_DEBUG3, "%s: job=%s host/queue=%s user=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d PEND=%d numNonPrmptRsv=%d", __func__, lsb_jobid2str (jData->jobId), ((hp == NULL) ? jData->qPtr->queue : hp->host), jData->uPtr->user, foundU->numRUN, foundU->numSSUSP, foundU->numUSUSP, foundU->numRESERVE, foundU->numPEND, foundU->numNonPrmptRsv);
		}
	}

	else if (numRUN != 0 || numSSUSP != 0 || numUSUSP != 0 || numRESERVE != 0 || numPEND != 0) {
		foundU = addUAcct (uAcct, up, numRUN, numSSUSP, numUSUSP, numRESERVE, numPEND);
		...
		if (logclass & LC_JLIMIT) {
      		/* catgets NULL */
			ls_syslog (LOG_DEBUG3, "%s: New uAcct for job=%s host/queue=%s user=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d, PEND=%d", __func__, lsb_jobid2str (jData->jobId), ((hp == NULL) ? jData->qPtr->queue : hp->host), ((jData->uPtr == NULL) ? jData->userName : jData->uPtr->user), foundU->numRUN, foundU->numSSUSP, foundU->numUSUSP, foundU->numRESERVE, foundU->numPEND);
		}
	}
	else
	{
 	/* catgets 7006 */
		ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7006, "%s: expected one of (numRUN, numSSUSP, numUSUSP, numRESERVE and numPEND) to be non-ZERO, but they are all ZERO"),	/__func__);
		return;
	}
	...
	if (hp == NULL)
	{
		...
		numSlots = qp->uJobLimit - foundU->numRUN - foundU->numSSUSP - foundU->numUSUSP - foundU->numRESERVE;
		SET_REASON (numSlots <= 0, foundU->reason, PEND_QUEUE_USR_JLIMIT);
		if (numSlots <= 0 && (logclass & LC_JLIMIT))
			ls_syslog (LOG_DEBUG3,  "%s: Q's JL/U reached; job=%s queue=%s user=%s", __func__, lsb_jobid2str (jData->jobId), jData->qPtr->queue, jData->uPtr->user);

	}
	else
	{

		numSlots = hp->uJobLimit - foundU->numRUN - foundU->numSSUSP - foundU->numUSUSP - foundU->numRESERVE;
		SET_REASON (numSlots <= 0, up->reasonTb[1][hp->hostId], PEND_HOST_USR_JLIMIT);
		if (numSlots <= 0 && (logclass & LC_JLIMIT))
			ls_syslog (LOG_DEBUG3, "%s: H's JL/U reached; job=%s host=%s user=%s", __func__, lsb_jobid2str (jData->jobId), hp->host, jData->uPtr->user);
	}
 ...
 */
struct userAcct *
addUAcct (struct hTab **uAcct, struct uData *up, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE, int numPEND)
{
	struct hEnt *ent;
	struct userAcct *acct;

	if (logclass & (LC_JLIMIT))
		ls_syslog (LOG_DEBUG2, "%s: New userAcct for user %s", __func__, up->user);

	if (*uAcct == NULL)
	{
		*uAcct = my_calloc (1, sizeof (struct hTab), __func__ );
		h_initTab_ (*uAcct, 11);
	}

	acct = my_calloc (1, sizeof (struct userAcct), __func__ );
...
void
updHAcct (struct jData *jData, struct qData *qp, struct uData *up, struct hTab **hAcct, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE, void (*onNewEntry) (struct hostAcct *, void *), void *extra)
{

	struct hostAcct *foundH = NULL;
	struct hData *hp = NULL;
	...
	if (*hAcct == NULL) {
		*hAcct = my_malloc (sizeof (struct hTab), __func__ );
		h_initTab_ (*hAcct, 4);
	}
	...
	for (i = 0; i < jData->numHostPtr; i++)
	{
		...
		if ((foundH = getHAcct (*hAcct, hp)) != NULL)
		{
			...
			if (logclass & LC_JLIMIT) {
				ls_syslog (LOG_DEBUG3, "%s: job=%s user/queue=%s host=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d NonPrmptRsv=%d", __func__, lsb_jobid2str (jData->jobId), ((qp != NULL) ? qp->queue : up->user), hp->host, foundH->numRUN, foundH->numSSUSP, foundH->numUSUSP, foundH->numRESERVE, foundH->numNonPrmptRsv);
			}
		}

		else if (numRUN > 0 || numSSUSP > 0 || numUSUSP > 0 || numRESERVE > 0)
		{
			foundH = addHAcct (hAcct, hp, numRUN, numSSUSP, numUSUSP, numRESERVE);
			...
			if (logclass & LC_JLIMIT) {
				ls_syslog (LOG_DEBUG2, "%s: New hAcct: job=%s user/queue=%s host=%s RUN=%d SSUSP=%d USUSP=%d RESERVE=%d", __func__, lsb_jobid2str (jData->jobId), ((qp != NULL) ? qp->queue : up->user), hp->host, foundH->numRUN, foundH->numSSUSP, foundH->numUSUSP, foundH->numRESERVE);
			}
		...
		if (qp != NULL)	{
				...
				&& !LS_ISUNAVAIL (hp->limStatus))
			{
				...
				SET_REASON (numSlots <= 0, qp->reasonTb[1][hp->hostId], PEND_QUEUE_PROC_JLIMIT);
				CHECKQUSABLE (qp, svReason, qp->reasonTb[1][hp->hostId]);
				if (numSlots <= 0 && (logclass & (LC_PEND | LC_JLIMIT))) {
					ls_syslog (LOG_DEBUG2, "%s: Q's JL/P reached. Set reason <%d>; job=%s host=%s queue=%s", __func__, qp->reasonTb[1][hp->hostId], lsb_jobid2str (jData->jobId), hp->host, qp->queue);
				}
			}
			else
			{
				numSlots = hJobLimitOk (hp, foundH, qp->hJobLimit);
				SET_REASON (numSlots <= 0, qp->reasonTb[1][hp->hostId], PEND_QUEUE_HOST_JLIMIT);
				CHECKQUSABLE (qp, svReason, qp->reasonTb[1][hp->hostId]);
				if (numSlots <= 0 && (logclass & (LC_PEND | LC_JLIMIT))) {
					ls_syslog (LOG_DEBUG2, "%s: Q's JL/H reached. Set reason <%d>; job=%s host=%s queue=%s", __func__, qp->reasonTb[1][hp->hostId], lsb_jobid2str (jData->jobId), hp->host, qp->queue);
				}
			}
			if (numSlots > 0 && (svReason == PEND_QUEUE_PROC_JLIMIT || svReason == PEND_QUEUE_HOST_JLIMIT))
			{
				CLEAR_REASON (qp->reasonTb[1][hp->hostId], svReason);
				CHECKQUSABLE (qp, svReason, qp->reasonTb[1][hp->hostId]);
				if (logclass & (LC_PEND | LC_JLIMIT)) {
					ls_syslog (LOG_DEBUG2, "%s: Clear reason <%d>; job=%s host=%s queue=%s", __func__, svReason, lsb_jobid2str (jData->jobId), hp->host, qp->queue);
				}
			}

			if (qp->hJobLimit == INFINIT_INT && qp->pJobLimit == INFINIT_FLOAT) {
				...
				pJobLimit = (qp->pJobLimit < (float) qp->hJobLimit) ? qp->pJobLimit : (float) qp->hJobLimit;
			}
		}
		else {
			...
			if (up->flags & USER_GROUP) {
				if (numSlots <= 0
					...
					&& !LS_ISUNAVAIL (hp->limStatus))
				{
					SET_REASON (numSlots <= 0, up->reasonTb[1][hp->hostId], PEND_UGRP_PROC_JLIMIT);

				}
				else if (numSlots > 0) {

					CLEAR_REASON (up->reasonTb[1][hp->hostId], PEND_UGRP_PROC_JLIMIT);
				}
			...
			else
			{
				if (numSlots <= 0
					...
					&& !LS_ISUNAVAIL (hp->limStatus))
				{
					SET_REASON (numSlots <= 0, up->reasonTb[1][hp->hostId], PEND_USER_PROC_JLIMIT);

				}
				else if (numSlots > 0) {

					CLEAR_REASON (up->reasonTb[1][hp->hostId], PEND_USER_PROC_JLIMIT);
			...
			if (numSlots <= 0 && (logclass & LC_JLIMIT)
				...
				&& !(hp->hStatus & HOST_STAT_UNAVAIL)
				&& !LS_ISUNAVAIL (hp->limStatus)) {
				ls_syslog (LOG_DEBUG2, "%s: U/G's JL/P reached; job=%s host=%s user=%s", __func__, lsb_jobid2str (jData->jobId), hp->host, up->user);
			}
			else if (numSlots > 0 && (logclass & LC_JLIMIT)) {
				ls_syslog (LOG_DEBUG, "%s: U/G's JL/P cleared up; job=%s host=%s user=%s", __func__, lsb_jobid2str (jData->jobId), hp->host, up->user);
			}
		}
	}
 ...
 */
struct hostAcct *
addHAcct (struct hTab **hAcct, struct hData *hp, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE)
{
	struct hEnt *ent = NULL;
	int new = 0;
	struct hostAcct *acct = NULL;

	if (logclass & (LC_JLIMIT)) {
		ls_syslog (LOG_DEBUG2, "%s: New hostAcct for host %s", __func__, hp->host);
	}

	if (*hAcct == NULL) {
		*hAcct = my_calloc (1, sizeof (struct hTab), __func__ );
		h_initTab_ (*hAcct, 11);
	}

	acct = my_calloc (1, sizeof (struct hostAcct), __func__ );
...
{
	struct uData *up = NULL;
	struct uData *ugp = NULL;
	...
	{
		grpPtr = my_calloc (numofugroups, sizeof (struct uData *), __func__ );
	}
	...
	up = jData->uPtr;
	up->flags &= ~USER_GROUP;
	updUserData1 (jData, up, numJobs, numPEND, numRUN, numSSUSP, numUSUSP, numRESERVE);

	newJob = (numRUN == 0 && numSSUSP == 0 && numUSUSP == 0 && numRESERVE == 0);
	if (!newJob)
	{
		...
		if (up->maxJobs == INFINIT_INT) {
			...
			numSlots = up->maxJobs - up->numRUN - up->numSSUSP - up->numUSUSP - up->numRESERVE;
		}
		...
		if (logclass & (LC_PEND | LC_JLIMIT))
		{
			if (numSlots <= 0)
			{
				ls_syslog (LOG_DEBUG2, "%s: Set reason <%d> job=%s user=%s numJobs=%d maxJobs=%d numPEND=%d", __func__, up->reasonTb[1][0], lsb_jobid2str (jData->jobId), up->user, up->numJobs, up->maxJobs, up->numPEND);
			}
			else if (svReason == PEND_USER_JOB_LIMIT
				&& (logclass & (LC_PEND | LC_JLIMIT)))
			{
				ls_syslog (LOG_DEBUG2, "%s: Clear reason <%d> job=%s user=%s numJobs=%d maxJobs=%d numPEND=%d", __func__, svReason, lsb_jobid2str (jData->jobId), up->user, up->numJobs, up->maxJobs, up->numPEND);
			}
		}
	}
	...
	if (!(up->flags & USER_INIT))
	{
		...
		FREEUP (up->gPtr);
		if (numNew > 0)	{
			up->gPtr = my_calloc (numNew, sizeof (struct uData *), __func__);
		}
	...
	for (i = 0; i < up->numGrpPtr; i++)
	{
		...
		updUserData1 (jData, ugp, numJobs, numPEND, numRUN, numSSUSP, numUSUSP, numRESERVE);
		...
		if (!newJob)
		{
			...
			if (ugp->maxJobs == INFINIT_INT) {
				...
				num = ugp->maxJobs - ugp->numRUN - ugp->numSSUSP - ugp->numUSUSP - ugp->numRESERVE;
			}
			...
			{
				if (num <= 0) {
					ls_syslog (LOG_DEBUG2, "%s: Set reason <%d>; job=%s group=%s numJobs=%d maxJobs=%d numPEND=%d", __func__, ugp->reasonTb[1][0], lsb_jobid2str (jData->jobId), ugp->user, ugp->numJobs, ugp->maxJobs, ugp->numPEND);
				}
				else if (svReason == PEND_UGRP_JOB_LIMIT && (logclass & (LC_PEND | LC_JLIMIT))) {
					ls_syslog (LOG_DEBUG2, "%s: Clear reason <%d> job=%s group=%s numJobs=%d maxJobs=%d numPEND=%d", __func__, PEND_UGRP_JOB_LIMIT, slsb_jobid2str (jData->jobId), ugp->user, ugp->numJobs, ugp->maxJobs, ugp->numPEND);
				}
			}
		}
	}
...
void
updUserData1 (struct jData *jData, struct uData *up, int numJobs, int numPEND, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE)
{
	bool_t newJob;

	addValue (&up->numJobs, numJobs, jData, __func__, "numJobs");
	addValue (&up->numPEND, numPEND, jData, __func__, "numPEND");
	addValue (&up->numRUN, numRUN, jData, __func__, "numRUN");
	addValue (&up->numSSUSP, numSSUSP, jData, __func__, "numSSUSP");
	addValue (&up->numUSUSP, numUSUSP, jData, __func__, "numUSUSP");
	addValue (&up->numRESERVE, numRESERVE, jData, __func__, "numRESERVE");

	if (logclass & LC_JLIMIT) {
		ls_syslog (LOG_DEBUG3, "%s: job=%s user/group=%s PEND=%d RUN=%d SSUSP=%d USUSP=%d RESERVE=%d", __func__, lsb_jobid2str (jData->jobId), up->user, up->numPEND, up->numRUN, up->numSSUSP, up->numUSUSP, up->numRESERVE);
	}

	newJob = (numRUN == 0 && numSSUSP == 0 && numUSUSP == 0 && numRESERVE == 0);

	if (!newJob && up->pJobLimit < INFINIT_FLOAT) {

		updHAcct (jData, NULL, up, &(up->hAcct), numRUN, numSSUSP, numUSUSP, numRESERVE, (void (*)(struct hostAcct *, void *)) NULL, NULL);
	}

}

void
updHostData (char updHPart, struct jData *jData, int numJobs, int numRUN, int numSSUSP, int numUSUSP, int numRESERVE)
{

	for ( unsigned int i = 0; i < jData->numHostPtr; i++)
	{
		...
		if (hp->uJobLimit < INFINIT_INT && (numRUN != 0 || numSSUSP != 0 || numUSUSP != 0 || numRESERVE != 0))
		{
			updUAcct (jData, jData->uPtr, &(hp->uAcct), numRUN, numSSUSP, numUSUSP, numRESERVE, 0, jData->hPtr[i], (void (*)(struct userAcct *, void *)) NULL, NULL);
		}
		addValue (&hp->numJobs, numJobs, jData, __func__, "numJobs");
		addValue (&hp->numRUN, numRUN, jData, __func__, "numRUN");
		addValue (&hp->numSSUSP, numSSUSP, jData, __func__, "numSSUSP");
		addValue (&hp->numUSUSP, numUSUSP, jData, __func__, "numUSUSP");
		addValue (&hp->numRESERVE, numRESERVE, jData, __func__, "numRESERVE");
		if (logclass & LC_JLIMIT)
		...
		if (hp->numJobs >= hp->maxJobs)
		{
			...
			if (logclass & (LC_PEND | LC_JLIMIT))
			{
				ls_syslog (LOG_DEBUG2, "%s: Set reason <%d> job=%s host=%s numJobs=%d maxJobs=%d",	__func__, hReasonTb[1][hp->hostId],	lsb_jobid2str (jData->jobId), hp->host, hp->numJobs, hp->maxJobs);
			}
		}
		else
		{
			...
			hp->hStatus &= ~HOST_STAT_FULL;
			if ((logclass & (LC_PEND | LC_JLIMIT)) && hReasonTb[1][hp->hostId] == PEND_HOST_JOB_LIMIT)
			{
				ls_syslog (LOG_DEBUG2, "%s: Clear reason <%d>; job=%s host=%s numJobs=%d maxJobs=%d", __func__, hReasonTb[1][hp->hostId], lsb_jobid2str (jData->jobId), hp->host, hp->numJobs, hp->maxJobs);
			}
...
void
addValue (int *currentValue, int num, struct jData *jp, const char *function,  char *counter)
{
	*currentValue += num;
	if (*currentValue >= 0) {
		return;
	...
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7009, "%s: %s is negative; job=%s queue=%s currentValue=%d, num=%d"), function, counter, lsb_jobid2str (jp->jobId), jp->qPtr->queue, *currentValue, num);	/* catgets 7009 */
	*currentValue = 0;
...
getUserData (char *user)
{
	...
	if (userEnt != NULL)
	{
		...
		uData = addUserData (user, defUser->maxJobs, defUser->pJobLimit, "mbatchd/getUserData", FALSE, FALSE);
		if (uData != NULL) {
			return uData;
		}
	...
	uData = addUserData (user, INFINIT_INT, INFINIT_FLOAT,  "mbatchd/getUserData", FALSE, FALSE);
	if (uData != NULL) {
		return uData;
	}
...
checkUsers (struct infoReq *req, struct userInfoReply *reply)
{
  ...
  // int j = 0; 
	...
	for (i = 0; i < req->numNames; i++)
	{
		...
		uInfo->numStartJobs = uData->numJobs - uData->numPEND - uData->numRESERVE;
		uInfo->numJobs = uData->numJobs;
		uInfo->numPEND = uData->numPEND;
		uInfo->numRUN = uData->numRUN;
		uInfo->numSSUSP = uData->numSSUSP;
...
struct uData *
addUserData ( const char *username, int maxjobs, float pJobLimit, const char *filename, int override, int config)
{
	...
	if (new)
	{
		uData = my_calloc (1, sizeof (struct uData), __func__ );
		initUData (uData);
	}
	else if (override)
	{
	...
	else
	{
		if (filename) {
      		/* catgets 7013 */
			ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7013, "%s: %s: User <%s> is multiply defined; retaining old definition"), __func__, filename, username);
		}
		return (struct uData *) userEnt->hData;
	}
	if (config == TRUE) {
	...
	if (!(uData->flags & USER_GROUP) && strstr (uData->user, "others") == NULL)
	{
		...
		if (logclass & (LC_TRACE)) {
			ls_syslog (LOG_DEBUG2, "%s: new user <%s> added to the all user set", __func__, uData->user);
		}
		...
		for (allUserGrp = (struct uData *) setIteratorBegin (&iter); allUserGrp != NULL; allUserGrp = (struct uData *) setIteratorGetNextElement (&iter))
		{
			char strBuf[128];
			if (uData->parents == NULL)
			{
				...
				uData->parents = setCreate (MAX_GROUPS, getIndexByuData, getuDataByIndex, strBuf);
				if (uData->parents == NULL)
				{
					ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_EMSG_S, __func__, "setCreate", strBuf, setPerror (bitseterrno));
					mbdDie (MASTER_MEM);
				}
			}
			...
			if (uData->ancestors == NULL)
			{
				...
				uData->ancestors = setCreate (MAX_GROUPS, getIndexByuData, getuDataByIndex, strBuf);
				if (uData->ancestors == NULL)
				{
					ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_EMSG_S, __func__, "setCreate", strBuf, setPerror (bitseterrno));
					mbdDie (MASTER_MEM);
...
updResCounters (struct jData *jData, int newStatus)
{
	...
	if (logclass & (LC_TRACE | LC_JLIMIT)) {
		ls_syslog (LOG_DEBUG3, "%s:jobId <%s>,jStatus<%x>, num <%d>, numReq <%d>, newStatus <%x>", __func__, lsb_jobid2str (jData->jobId), jData->jStatus, jData->numHostPtr, jData->shared->jobBill.maxNumProcessors, newStatus);
	}
	num = jData->numHostPtr;
	numReq = jData->shared->jobBill.maxNumProcessors;
	...
	{
		case JOB_STAT_SSUSP: {
			if (!(jData->jStatus & JOB_STAT_RESERVE) && (newStatus & JOB_STAT_RESERVE) && IS_START (newStatus))
			{
			...
			else if ((jData->jStatus & JOB_STAT_RESERVE) && !(newStatus & JOB_STAT_RESERVE) && (IS_START (newStatus) || (newStatus & JOB_STAT_PEND)))
			{
			...
			else if ((jData->jStatus & JOB_STAT_RESERVE) && IS_FINISH (newStatus))
			{
			...
			{
			/* catgets 7016 */
				ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7016, "%s: Job <%s> transited from %x to %x"), __func__, lsb_jobid2str (jData->jobId), jData->jStatus, newStatus);
			}
			break;
			case JOB_STAT_USUSP:
			if (!(jData->jStatus & JOB_STAT_RESERVE) && (newStatus & JOB_STAT_RESERVE) && IS_START (newStatus))
		...
		case JOB_STAT_PEND:
		case JOB_STAT_PSUSP: {
			if ((jData->jStatus & JOB_STAT_RESERVE) && !(newStatus & JOB_STAT_RESERVE) && IS_PEND (newStatus))
			{
			...
			else if (!(jData->jStatus & JOB_STAT_RESERVE) && (newStatus & JOB_STAT_RESERVE) && IS_PEND (newStatus))
			{
				updQaccount (jData, 0, -num, 0, 0, 0, num);
				updHostData (TRUE, jData, 1, 0, 0, 0, 1);
				updUserData (jData, 0, -num, 0, 0, 0, num);
			...
			else if (IS_PEND (jData->jStatus) && (jData->jStatus & JOB_STAT_RESERVE) && IS_FINISH (newStatus))
			{
			...
			{
				/* catgets 7016 */
				ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 7016, "%s: Job <%s> transited from %x to %x"), __func__, lsb_jobid2str (jData->jobId), jData->jStatus,
				newStatus);
			}
			break;
		}
...
updHostLeftRusageMem (struct jData *jobP, int order)
{
	...
	if (logclass & (LC_TRACE)) {
		ls_syslog (LOG_DEBUG, "%s: Enter this function ...", __func__);
	}
		...
		if (resMem < INFINIT_LOAD && resMem > -INFINIT_LOAD)
		{
			...
			for (numHost = 0; numHost < jobP->numHostPtr; numHost++)
			{
				...
				if (logclass & (LC_TRACE | LC_SCHED)) {
					ls_syslog (LOG_DEBUG, "%s: job <%s> reserved mem is %f, execution host <%s>, new leftRusageMem is %f, order is %d", __func__, lsb_jobid2str (jobP->jobId), resMem, jobP->hPtr[numHost]->host, jobP->hPtr[numHost]->leftRusageMem, order);
				}
