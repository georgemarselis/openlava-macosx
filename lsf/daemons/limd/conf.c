...
static struct hostNode *addHost_ (struct clusterNode *,
				  struct hostEntry *, char *, char *, int *);
...
static struct sharedResource *addResource (char *, int, char **, char *,
					   char *, int, int);
static void freeSharedRes (struct sharedResource *);
static int addHostInstance (struct sharedResource *, int, char **, char *,
			    int);
...
readShared (void)
{
  ...
  if (!fp)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5200, "%s: Can't open configuration file <%s>: %m"),	/* catgets 5200 */
		 fname, lsfile);
      return (-1);
    }
  ...
  for (;;)
      ...
      if ((cp = getBeginLine (fp, &LineNum)) == NULL)
	{
	  ...
	  if (!modelok)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5201, "%s: HostModel section missing or invalid in %s"),	/* catgets 5201 */
			 fname, lsfile);
	    }
	  if (!resok)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5202, "%s: Warning: Resource section missing or invalid in %s"),	/* catgets 5202 */
			 fname, lsfile);
	    }
	  if (!typeok)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5203, "%s: HostType section missing or invalid"),	/* catgets 5203 */
			 lsfile);
	    }
	  if (!indxok)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5204, "%s: Warning: attempt to define too many new indices"),	/* catgets 5204 */
			 lsfile);
	    }
	  if (!clsok)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5205, "%s: Cluster section missing or invalid"),	/* catgets 5205 */
			 lsfile);
	    }
      ...
      if (!word)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5206, "%s: %s(%d): Section name expected after Begin; ignoring section"),	/* catgets 5206 */
      ...
      else
	{
	  if (strcasecmp (word, "host") == 0)
	    {
	      ls_syslog (LOG_INFO,
			 (_i18n_msg_get
			  (ls_catd, NL_SETN, 5380,
			   "%s: %s(%d): section %s no longer needed in this version, ignored")),
			 /* catgets 5380 */ "readShared", lsfile, LineNum,
			 word);
	      continue;
	    }
	  ...
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5207, "%s: %s(%d): Invalid section name %s; ignoring section"),	/* catgets 5207 */
...
doindex (FILE * fp, int *LineNum, char *lsfile)
{
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5212, "%s: %s(%d): Premature EOF"),	/* catgets 5212 */
	     fname, lsfile, *LineNum);
  lim_CheckError = WARNING_ERR;
  return (TRUE);
...
setIndex (struct keymap *keyList, char *lsfile, int linenum)
{
  ...
  if (strlen (keyList[3].val) >= MAXLSFNAMELEN)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5213, "%s: %s(%d): Name %s is too long (maximum is %d chars); ignoring index"),	/* catgets 5213 */
  ...
  if (strpbrk (keyList[3].val, ILLEGAL_CHARS) != NULL)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5214, "%s: %s(%d): illegal character (one of %s)"),	/* catgets 5214 */
  ...
  if (IS_DIGIT (keyList[3].val[0]))
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5215, "%s: %s(%d): Index name <%s> begun with a digit is illegal; ignored"),	/* catgets 5215 */
  ...
  if ((resIdx = resNameDefined (keyList[3].val)) >= 0)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5216, "%s: %s(%d): Name <%s> is not a dynamic resource; ignored"),	/* catgets 5216 */
		     fname, lsfile, linenum, keyList[3].val);
	  return (TRUE);
	}
      if ((allInfo.resTable[resIdx].flags & RESF_BUILTIN))
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5217, "%s: %s(%d): Name <%s> reserved or previously defined;"),	/* catgets 5217 */
		   fname, lsfile, linenum, keyList[3].val);
      else
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5219, "%s: %s(%d): Name <%s> reserved or previously defined;ignoring"),	/* catgets 5219 */
		     fname, lsfile, linenum, keyList[3].val);
	  lim_CheckError = WARNING_ERR;
	  return (TRUE);
	}
...
doclist (FILE * fp, int *LineNum, char *lsfile)
{
  ...
  if (!linep)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5222, "%s: %s(%d): section cluster: Premature EOF"),	/* catgets 5222 */
		 fname, lsfile, *LineNum);
      return FALSE;
    }
  ...
  if (strchr (linep, '=') == NULL)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5223, "%s: %s(%d): keyword line format error for section cluster; ignoring section"), fname, lsfile, *LineNum);	/* catgets 5223 */
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5224, "%s: %s(%d): keyword line: key %s is missing in section cluster; ignoring section"),	/* catgets 5224 */
      ...
      while ((linep = getNextLineC_ (fp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5225, "%s: %s(%d): values do not match keys for section cluster, ignoring line"),	/* catgets 5225 */
			 fname, lsfile, *LineNum);
	      lim_CheckError = WARNING_ERR;
	      continue;
	    }
	  ...
	  if (!clusterAdded && !addCluster (keyList[0].val, servers))
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5226, "%s: Ignoring cluster %s"),	/* catgets 5226 */
			 fname, keyList[0].val);
	      lim_CheckError = WARNING_ERR;
	    }
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5227, "%s: %s(%d): horizontal cluster section not implemented yet, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5227 */
      doSkipSection (fp, LineNum, lsfile, "cluster");
      return FALSE;
    }
  ...
  if (strchr (linep, '=') == NULL)
    {
      if (!keyMatch (keyList, linep, TRUE))
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5230, "%s: %s(%d): keyword line format error for section HostType, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5230 */
      ...
      while ((linep = getNextLineC_ (fp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5231, "%s: %s(%d): values do not match keys for section cluster, ignoring line"), fname, lsfile, *LineNum);	/* catgets 5231 */
	      lim_CheckError = WARNING_ERR;
	      continue;
	    }
	  ...
	  if (strpbrk (keyList[0].val, ILLEGAL_CHARS) != NULL)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5232, "%s: %s(%d): illegal character (one of %s), ignoring type %s"),	/* catgets 5232 */
			 fname, lsfile, *LineNum, ILLEGAL_CHARS,
			 keyList[0].val);
	      lim_CheckError = WARNING_ERR;
	      FREEUP (keyList[0].val);
	  ...
	  if (IS_DIGIT (keyList[0].val[0]))
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5233, "%s: %s(%d): Type name <%s> begun with a digit is illegal; ignored"),	/* catgets 5233 */
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5234, "%s: %s(%d): horizontal HostType section not implemented yet, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5234 */
      doSkipSection (fp, LineNum, lsfile, "HostType");
      return FALSE;
    }
  ...
  if (strchr (linep, '=') == NULL)
    {
      if (!keyMatch (keyList, linep, FALSE))
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5237, "%s: %s(%d): keyword line format error for section hostmodel, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5237 */
      ...
      while ((linep = getNextLineC_ (fp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5238, "%s: %s(%d): values do not match keys for section hostmodel, ignoring line"), fname, lsfile, *LineNum);	/* catgets 5238 */
	      lim_CheckError = WARNING_ERR;
	      continue;
	    }
	  ...
	  if (!isanumber_ (keyList[1].val) || atof (keyList[1].val) <= 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5239, "%s: %s(%d): Bad cpuFactor for host model %s, ignoring line"),	/* catgets 5239 */
			 fname, lsfile, *LineNum, keyList[0].val);
	      lim_CheckError = WARNING_ERR;
	      FREEUP (keyList[0].val);
	      FREEUP (keyList[1].val);
	  ...
	  if (strpbrk (keyList[0].val, ILLEGAL_CHARS) != NULL)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5240, "%s: %s(%d): illegal character (one of %s), ignoring model %s"),	/* catgets 5240 */
			 fname, lsfile, *LineNum, ILLEGAL_CHARS,
			 keyList[0].val);
	      lim_CheckError = WARNING_ERR;
	      FREEUP (keyList[0].val);
	  ...
	  if (IS_DIGIT (keyList[0].val[0]))
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5241, "%s: %s(%d): Model name <%s> begun with a digit is illegal; ignored"), fname, lsfile, *LineNum, keyList[0].val);	/* catgets 5241 */
	      lim_CheckError = WARNING_ERR;
	      FREEUP (keyList[0].val);
	      FREEUP (keyList[1].val);
	      FREEUP (keyList[2].val);
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5244, "%s: %s(%d): horizontal HostModel section not implemented yet, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5244 */
      doSkipSection (fp, LineNum, lsfile, "HostModel");
      return FALSE;
    }
  ...
  if (strchr (linep, '=') == NULL)
    {
      if (!keyMatch (keyList, linep, FALSE))
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5248, "%s: %s(%d): keyword line format error for section resource, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5248 */
      ...
      while ((linep = getNextLineC_ (fp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5249, "%s: %s(%d): values do not match keys for section resource, ignoring line"), fname, lsfile, *LineNum);	/* catgets 5249 */
	      lim_CheckError = WARNING_ERR;
	      continue;
	    }
	  ...
	  if (strlen (keyList[RKEY_RESOURCENAME].val) >= MAXLSFNAMELEN - 1)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5250, "%s: %s(%d): Resource name %s too long in section resource. Should be less than %d characters. Ignoring line"),	/* catgets 5250 */
			 fname, lsfile, *LineNum, keyList[0].val,
			 MAXLSFNAMELEN - 1);
	      lim_CheckError = WARNING_ERR;
	      freeKeyList (keyList);
	  ...
	  if ((resIdx = resNameDefined (keyList[RKEY_RESOURCENAME].val)) >= 0)
		  ...
		  && (allInfo.resTable[resIdx].flags & RESF_DYNAMIC))
		{
		  ...
		  if (keyList[RKEY_TYPE].val && *keyList[RKEY_TYPE].val
		  ...
		  else
		    {
		      ...
		      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5251, "%s: %s(%d): Built-in resource %s can't be overrided with different type or increasing. Ignoring line"), fname, lsfile, *LineNum, keyList[0].val);	/* catgets 5251 */
		      lim_CheckError = WARNING_ERR;
		    }
		}
	      else
		{
		  ...
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5251, "%s: %s(%d): Resource name %s reserved or previously defined. Ignoring line"), fname, lsfile, *LineNum, keyList[0].val);	/* catgets 5251 */
		  lim_CheckError = WARNING_ERR;
		}
	      freeKeyList (keyList);
	      continue;
	  ...
	  if (strpbrk (keyList[RKEY_RESOURCENAME].val, ILLEGAL_CHARS) != NULL)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5252, "%s: %s(%d): illegal character (one of %s): in resource name:%s, section resource, ignoring line"),	/* catgets 5252 */
			 fname, lsfile, *LineNum, ILLEGAL_CHARS,
			 keyList[0].val);
	      lim_CheckError = WARNING_ERR;
	      freeKeyList (keyList);
	  ...
	  if (IS_DIGIT (keyList[RKEY_RESOURCENAME].val[0]))
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5253, "%s: %s(%d): Resource name <%s> begun with a digit is illegal; ignored"), fname, lsfile, *LineNum, keyList[0].val);	/* catgets 5253 */
	      lim_CheckError = WARNING_ERR;
	      freeKeyList (keyList);
	      continue;
	    }
	  ...
	  if (keyList[RKEY_TYPE].val != NULL
	      ...
	      if (strcmp (keyList[RKEY_TYPE].val, LSF_LIM_ERES_TYPE) == 0)
		{
		  if (setExtResourcesDef (keyList[RKEY_RESOURCENAME].val) !=
		      0)
		    {
		      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5256, "%s: Ignoring the external resource <%s>(%d) in section resource of file %s"),	/* catgets 5256 */
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5257, "%s: %s(%d): resource type <%s> for resource <%s> is not valid; ignoring resource <%s> in section resource"),	/* catgets 5257 */
		...
		ls_syslog (LOG_DEBUG3,
			   "doresources: %s(%d): Resource type is not defined for resource <%s>; The resource will be assigned type <boolean>",
	  ...
	  if (keyList[RKEY_INTERVAL].val != NULL
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5258, "%s: %s(%d): INTERVAL <%s> for resource <%s> should be a integer greater than 0; ignoring resource <%s> in section resource"),	/* catgets 5258 */
			     fname,
			     lsfile,
			     *LineNum,
	  ...
	  if (keyList[RKEY_INCREASING].val != NULL
	      ...
	      if (allInfo.resTable[allInfo.nRes].valueType == LS_NUMERIC)
		{
		  if (!strcasecmp (keyList[RKEY_INCREASING].val, "N"))
		    ...
		    {
		      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5259, "%s: %s(%d): INCREASING <%s> for resource <%s> is not valid; ignoring resource <%s> in section resource"),	/* catgets 5259 */
				 fname,
				 lsfile,
				 *LineNum,
		...
		ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5260, "%s: %s(%d): INCREASING <%s> is not used by the resource <%s> with type <%s>; ignoring INCREASING"),	/* catgets 5260 */
			   fname,
			   lsfile,
			   *LineNum,
			   keyList[RKEY_INCREASING].val,
			   keyList[RKEY_RESOURCENAME].val,
			   (allInfo.resTable[allInfo.nRes].orderType ==
			    LS_BOOLEAN) ? "BOOLEAN" : "STRING");
	    }
	  else
	    {
	      if (allInfo.resTable[allInfo.nRes].valueType == LS_NUMERIC)
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5261, "%s: %s(%d): No INCREASING specified for a numeric resource <%s>; ignoring resource <%s> in section resource"),	/* catgets 5261 */
			     fname,
			     lsfile,
			     *LineNum,
			     keyList[RKEY_RESOURCENAME].val,
...
doresourcemap (FILE * fp, char *lsfile, int *LineNum)
{
  ...
  if (isSectionEnd (linep, lsfile, LineNum, "resourceMap"))
    {
      ls_syslog (LOG_WARNING, _i18n_msg_get (ls_catd, NL_SETN, 5477, "%s: %s(%d): Empty resourceMap, no keywords or resources defined."),	/* catgets 5477 */
		 fname, lsfile, *LineNum);
      return (-1);
    }
  ...
  if (strchr (linep, '=') == NULL)
    {
      if (!keyMatch (keyList, linep, TRUE))
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5267, "%s: %s(%d): keyword line format error for section resource, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5267 */
      ...
      while ((linep = getNextLineC_ (fp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5268, "%s: %s(%d): values do not match keys for resourceMap section, ignoring line"), fname, lsfile, *LineNum);	/* catgets 5268 */
	      lim_CheckError = WARNING_ERR;
	      continue;
	    }
	  ...
	  if ((resNo = resNameDefined (keyList[RKEY_RESOURCE_NAME].val)) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5269, "%s: %s(%d): Resource name <%s> is  not defined; ignoring line"),	/* catgets 5269 */
			 fname, lsfile, *LineNum,
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5296, "%s: %s(%d): Built-in resource %s can't be redefined as shared resource here. Ignoring line"),	/* catgets 5296 */
			     fname, lsfile, *LineNum, keyList[0].val);
		  continue;
		}
	    }
	  ...
	  if (keyList[RKEY_LOCATION].val != NULL
	      && strcmp (keyList[RKEY_LOCATION].val, LSF_LIM_ERES_TYPE) == 0)
	    {
		  ...
		  != 0)
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5270, "%s: Ignoring the external resource location <%s>(%d) in section resourceMap of file %s"),	/* catgets 5270 */
			     fname, keyList[RKEY_RESOURCENAME].val,
			     *LineNum, lsfile);
		  lim_CheckError = WARNING_ERR;
		  freeKeyList (keyList);
	  ...
	  if (keyList[RKEY_LOCATION].val != NULL
		  ...
		  strchr (keyList[RKEY_LOCATION].val, '~'))
		{
	      ...
	      if (addResourceMap (keyList[RKEY_RESOURCE_NAME].val,
				  ...
				  *LineNum, &isDefault) < 0)
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5271, "%s: %s(%d): addResourceMap() failed for resource <%s>; ignoring line"),	/* catgets 5271 */
			     fname, lsfile, *LineNum,
			     keyList[RKEY_RESOURCE_NAME].val);
		  lim_CheckError = WARNING_ERR;
		  freeKeyList (keyList);
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5272, "%s: %s(%d): No LOCATION specified for resource <%s>; ignoring the line"),	/* catgets 5272 */
			 fname, lsfile, *LineNum,
			 keyList[RKEY_RESOURCE_NAME].val);
	      lim_CheckError = WARNING_ERR;
	      freeKeyList (keyList);
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5273, "%s: %s(%d): horizontal resource section not implemented yet"), fname, lsfile, *LineNum);	/* catgets 5273 */
      return (-1);
    }
...
addSharedResourceInstance (int nHosts, char **hosts, char *resName)
{
  ...
  if ((resNo = resNameDefined (resName)) < 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5274, "%s: Resource name <%s> not defined"),	/* catgets 5274 */
		 fname, resName);
      return (-1);
    }
  ...
  tmp =
    ...
    malloc (sizeof (struct sharedResourceInstance));
  if (tmp == NULL)
    {
		...
		int *isDefault)
{
  ...
  *isDefault = FALSE;
  ...
  if (resName == NULL || location == NULL)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5382, "%s: %s(%d): Resource name <%s> location <%s>"),	/* catgets 5382 */
		 fname, lsfile, LineNum,
  ...
  if ((resNo = resNameDefined (resName)) < 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5275, "%s: %s(%d): Resource name <%s> not defined"),	/* catgets 5275 */
  ...
  if (i != 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5383, "%s: %s(%d): number of '[' is not match that of ']' in <%s> for resource <%s>; ignoring"),	/* catgets 5383 */
  ...
  while (sp != NULL && sp[0] != '\0')
      ...
      if (cp != sp)
	{
	  ...
	  if (!isdigitstr_ (initValue)
	      && allInfo.resTable[resNo].valueType == LS_NUMERIC)
	    {
	      ...
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5386, "%s: %s(%d): Invalid characters (%s) used as NUMERIC resource value; ignoring"),	/* catgets 5386 */
	...
	sp++;
      ...
      if (*sp != '[' && *sp != '\0')
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5384, "%s: %s(%d): Bad character <%c> in instance; ignoring"),	/* catgets 5384 */
      ...
      if (*sp == '[')
	{
	  ...
	  if (*sp == '\0')
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5385, "%s: %s(%d): Bad format for instance <%s>; ignoring the instance"),	/* catgets 5385 */
	  ...
	  *sp = '\0';
	  ...
	  if (initValue[0] == '\0' && !dynamic)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5276, "%s: %s(%d): Value must be defined for static resource; ignoring resource <%s>, instance <%s>"),	/* catgets 5276 */
	  ...
	  if (initValue[0] != '\0' && dynamic)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5277, "%s: %s(%d): Value <%s> ignored for dynamic resource <%s>, instance <%s>"),	/* catgets 5277 */
	  ...
	  if ((numHosts =
	       parseHostList (cp, lsfile, LineNum, &hosts,
			      &defaultWord)) <= 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5387, "%s: %s(%d): getHostList(%s) failed; ignoring the instance <%s>"),	/* catgets 5387 */
	  ...
	  if (defaultWord == TRUE)
	    {
	      *isDefault = TRUE;
	      ...
	      if (numHosts > 1)
		ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5388, "%s: %s(%d):  Other host is specified with reserved word <default> in the instance <%s> for resource <%s>;ignoring other hosts"),	/* catgets 5388 */
			   fname, lsfile, LineNum, instance, resName);

	      if (resource && resource->numInstances > 1)
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5389, "%s: %s(%d):  Other instances are specified with the instance <%s> for resource <%s>; ignoring the instance"),	/* catgets 5389 */
			     fname, lsfile, LineNum, instance, resName);
		  break;
		}
	    }
		  ...
		  if (!(defaultWord && dynamic &&
			allInfo.resTable[resNo].valueType == LS_NUMERIC) &&
		      (resource = addResource (resName, numHosts,
					       ...
					       LineNum, TRUE)) == NULL)
		    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5390, "%s: %s(%d): addResource() failed; ignoring the instance <%s>"),	/* catgets 5390 */
			       fname, lsfile, LineNum, instance);
		}
	      else
		{
		  if (addHostInstance (resource, numHosts, hosts,
				       initValue, TRUE) < 0)
		    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5391, "%s: %s(%d): addHostInstance() failed; ignoring the instance <%s>"),	/* catgets 5391 */
			       fname, lsfile, LineNum, instance);
		}
	    }
	  defaultWord = FALSE;
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5392, "%s: %s(%d): No <[>  for instance in <%s>; ignoring"), fname, lsfile, LineNum, location);	/* catgets 5392 */
	  while (*sp != ']' && *sp != '\0')
	    sp++;
	  if (*sp == '\0')
	    {
	       ...
	       int *hasDefault)
{
  ...
  while ((host = getNextWord_ (&sp)) != NULL)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5278, "%s: %s(%d): Invalid hostname <%s>;ignoring the host"), fname, lsfile, LineNum, host);	/* catgets 5278 */
	  lim_CheckError = WARNING_ERR;
	  continue;
	}
...
validLocationHost (char *hostName)
{
  ...
  if (!strcmp (hostName, "default")
      || !strcmp (hostName, "others") || !strcmp (hostName, "all"))
    return hostName;
  ...
  if (Gethostbyname_ (hostName))
	...
	return hostName;
      ...
      ls_syslog (LOG_ERR, "\
%s: Host %s is not used by cluster %s; ignored", __func__, hostName, myClusterName);
      return NULL;
    }
...
readCluster (int checkMode)
{
  ...
  if (!myClusterPtr)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5279, "My cluster name %s is not configured in lsf.shared"),	/* catgets 5279 */
		 myClusterName);
  ...
  if (!myHostPtr)
    {
      ...
      if (!myHostPtr)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5281, "%s: Local host %s not configured in Host section of file lsf.cluster.%s"), fname, hname, myClusterName);	/* catgets 5281 */
	  if (checkMode)
	    return -1;
	  else
	    lim_Exit ("readCluster");
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5282, "%s: Local host %s is configured as client-only in file lsf.cluster.%s; LIM will not run on a client-only host"),	/* catgets 5282 */
  ...
  if (nClusAdmins == 0)
    {
      ...
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5396, "%s: No LSF managers are specified in file lsf.cluster.%s, default cluster manager is root."), fname, myClusterName);	/* catgets 5396 */
  ...
  for (;;)
      ...
      if (!cp)
	{
	  ...
	  if (clPtr->hostList)
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5285, "%s: %s(%d): No hosts configured for cluster %s"),	/* catgets 5285 */
      ...
      if (!word)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5286, "%s: %s(%d): Keyword expected after Begin. Ignoring section"),	/* catgets 5286 */
...
domanager (FILE * clfp, char *lsfile, int *LineNum, char *secName)
{
  ...
  if (lim_debug > 0 && lim_debug < 3)
      ...
      if (getLSFUser_ (lsfUserName, sizeof (lsfUserName)) < 0)
	{
      ...
      if (lim_CheckMode > 0)
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5289, "%s: %s(%d): The cluster manager is the invoker <%s> in debug mode"), fname, lsfile, *LineNum, lsfUserName);	/* catgets 5289 */
      return 0;
    }
  ...
  if (strchr (linep, '=') == NULL)
    {
      if (!keyMatch (keyList, linep, TRUE))
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5291, "%s: %s(%d): keyword line format error for section %s, ignoring section"), fname, lsfile, *LineNum, secName);	/* catgets 5291 */
	  doSkipSection (clfp, LineNum, lsfile, secName);
	  return -1;
	}
      ...
      if ((linep = getNextLineC_ (clfp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (mapValues (keyList, linep) < 0)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5292, "%s: %s(%d): values do not match keys for section %s, ignoring section"), fname, lsfile, *LineNum, secName);	/* catgets 5292 */
	      doSkipSection (clfp, LineNum, lsfile, secName);
	      return -1;
	    }
...
getClusAdmins (char *line, char *lsfile, int *LineNum, char *secName)
{
  ...
  if (count > 1)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5293, "%s: %s(%d): More than one %s section defined; ignored."), fname, lsfile, *LineNum, secName);	/* catgets 5293 */
      return -1;
    }
  count++;
  if (strcmp (lastSecName, secName) == 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5294, "%s: %s(%d): section <%s> is multiply specified; ignoring the section"), fname, lsfile, *LineNum, secName);	/* catgets 5294 */
      return (-1);
    }
  lserrno = LSE_NO_ERR;
  ...
  if (admins->nAdmins <= 0)
    {
      ...
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5295, "%s: %s(%d): No valid user for section %s: %s"), fname, lsfile, *LineNum, secName, line);	/* catgets 5295 */
      return -1;
    }
  if (strcmp (secName, "clustermanager") == 0 &&
...
#define LSF_ELIM_RESTARTS 10
  ...
  if (strchr (linep, '=') == NULL)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5298, "%s: %s(%d): vertical section not supported, ignoring section"), fname, lsfile, *LineNum);	/* catgets 5298 */
      ...
      if (keyList[EXINTERVAL].val)
	{
	  if (!isanumber_ (keyList[0].val) || atof (keyList[0].val) < 0.001)
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5299, "%s: %s(%d): Invalid exchange interval in section parameters: %s. Ignoring."),	/* catgets 5299 */
      ...
      if (keyList[PROBE_TIMEOUT].val)
	{
	  if (!isint_ (keyList[PROBE_TIMEOUT].val)
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5301, "%s: %s(%d): Invalid probe timeout value in section parameters: %s. Ignoring."),	/* catgets 5301 */
      ...
      if (keyList[ELIM_POLL_INTERVAL].val)
	{
	  if (!isanumber_ (keyList[ELIM_POLL_INTERVAL].val) ||
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5302, "%s: %s(%d): Invalid sample interval in section parameters: %s. Must be between 0.001 and 5. Ignoring."),	/* catgets 5302 */
			 fname, lsfile, *LineNum,
			 keyList[ELIM_POLL_INTERVAL].val);
	      warning = TRUE;
      ...
      if (keyList[HOST_INACTIVITY_LIMIT].val)
	{
	  if (!isint_ (keyList[HOST_INACTIVITY_LIMIT].val) ||
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5303, "%s: %s(%d): Invalid host inactivity limit in section parameters: %s. Ignoring."), fname, lsfile, *LineNum, keyList[HOST_INACTIVITY_LIMIT].val);	/* catgets 5303 */
      ...
      if (keyList[MASTER_INACTIVITY_LIMIT].val)
	{
	  if (!isint_ (keyList[MASTER_INACTIVITY_LIMIT].val) ||
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5304, "%s: %s(%d): Invalid master inactivity limit in section parameters: %s. Ignoring."), fname, lsfile, *LineNum, keyList[MASTER_INACTIVITY_LIMIT].val);	/* catgets 5304 */
      ...
      if (keyList[ADJUST_DURATION].val)
	{
	  if (!isint_ (keyList[ADJUST_DURATION].val) ||
	    ...
	    {
	      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5306, "%s: %s(%d): Invalid load adjust duration in section parameters: %s. Ignoring."),	/* catgets 5306 */
			 fname, lsfile, *LineNum,
      ...
      if (keyList[LSF_ELIM_DEBUG].val)
	{
	  ...
	  if (strcasecmp (keyList[LSF_ELIM_DEBUG].val, "y") != 0 &&
	      strcasecmp (keyList[LSF_ELIM_DEBUG].val, "n") != 0)
	    {
	      ...
	      ls_syslog (LOG_WARNING, I18N (5316, "LSF_ELIM_DEBUG invalid: %s, not debuging ELIM."),	/* catgets 5316 */
	  ...
	  if (!isint_ (keyList[LSF_ELIM_BLOCKTIME].val) || ELIMblocktime < 0)
	    {
	      ...
	      ls_syslog (LOG_WARNING, I18N (5318, "LSF_ELIM_BLOCKTIME invalid: %s, blocking communication with ELIM."),	/* catgets 5318 */
      ...
      if (ELIMdebug && ELIMblocktime == -1)
	{
	  ...
	  ls_syslog (LOG_WARNING, I18N (5340, "LSF_ELIM_DEBUG=y but LSF_ELIM_BLOCKTIME is not set/valid; LSF_ELIM_BLOCKTIME will be set to 2 seconds "));	/* catgets 5340 */
	  ...
	  if (!isint_ (keyList[LSF_ELIM_RESTARTS].val) || ELIMrestarts < 0)
	    {
	      ...
	      ls_syslog (LOG_WARNING, I18N (5366, "LSF_ELIM_RESTARTS invalid: %s, unlimited ELIM restarts."),	/* catgets 5366 */
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5308, "%s: Exchange interval must be greater than or equal to sampling interval. Setting exchange and sample interval to %f."),	/* catgets 5308 */
		     fname, sampleIntvl);
  ...
  for (;;)
      ...
      if ((lp = getBeginLine (fp, &LineNum)) == NULL)
	{
	  if (!found)
	    {
	      ls_syslog (LOG_DEBUG,
			 "setMyClusterName: Local host %s not defined in cluster file %s",
			 hname, clusterFile);
	    }
	  break;
	}
...
dohosts (FILE * clfp, struct clusterNode *clPtr, char *lsfile, int *LineNum)
{
  ...
  if (isSectionEnd (linep, lsfile, LineNum, "host"))
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5329, "%s: %s(%d): empty host section"),	/* catgets 5329 */
		 fname, lsfile, *LineNum);
      return -1;
    }
  ...
  if (strchr (linep, '=') == NULL)
	...
	{
	  ls_syslog (LOG_WARNING, _i18n_msg_get (ls_catd, NL_SETN, 5332, "%s: %s(%d): keyword line: conflicting keyword definition: you cannot define both 'R' and 'SERVER_'. 'R' ignored"),	/* catgets 5332 */
		     fname, lsfile, *LineNum);
	  lim_CheckError = WARNING_ERR;
	  ignoreR = TRUE;
	}
      ...
      while ((linep = getNextLineC_ (clfp, LineNum, TRUE)) != NULL)
	{
	  ...
	  if (hostEntry.busyThreshold[UT] > 1.0
	      && hostEntry.busyThreshold[UT] < INFINIT_LOAD)
	    {
	      ls_syslog (LOG_INFO,
			 (_i18n_msg_get
			  (ls_catd, NL_SETN, 5476,
			   "%s: %s(%d): value for threshold ut <%2.2f> is greater than 1, assumming <%5.1f%%>")),
			 /* catgets 5476 */ "dohosts", lsfile,
...
addHost_ (struct clusterNode *clPtr,
	  ...
	  char *window, char *fileName, int *LineNumPtr)
{
  struct hostNode *hPtr;
  struct hostent *hp;
  char *word;
  ...
  if ((hp = Gethostbyname_ (hEntPtr->hostName)) == NULL)
    {
      ls_syslog (LOG_ERR, "\
%s: Invalid hostname %s in section host. Ignoring host", __func__, hEntPtr->hostName);
      return NULL;
    }
...
%s: %s(%d): host %s redefined, using previous definition", __func__, fileName, *LineNumPtr, hEntPtr->hostName);
      return hPtr;
    }
...
%s: initHostNode() failed %m", __func__);
  ...
  for (i = 0; i < hEntPtr->nRes; i++)
      ...
      else
	{
	  lim_CheckError = WARNING_ERR;
	  ls_syslog (LOG_ERR, "\
%s: %s(%d): Invalid resource name %s for host %s in section %s; ignored", __func__, fileName, *LineNumPtr, resStr, hEntPtr->hostName, "Host", resStr);
	}
    }
  ...
  if (!hEntPtr->hostModel[0])
    ...
    {
      ls_syslog (LOG_ERR, "\
%s: %s(%d): Unknown host model %s. Ignoring host", __func__, fileName, *LineNumPtr, hEntPtr->hostModel);
  ...
  if (!hEntPtr->hostType[0])
    ...
    {
      ls_syslog (LOG_ERR, "\
%s: %s(%d): Unknown host type %s. Ignoring host", __func__, fileName, *LineNumPtr, hEntPtr->hostType);
      freeHostNodes (hPtr, FALSE);
      return NULL;
    }
  ...
  if (saveHostIPAddr (hPtr, hp) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s Can not save internet address of host %s", __func__, hp->h_name);
      freeHostNodes (hPtr, FALSE);
      return NULL;
    }
  ...
  if (window && hEntPtr->rcv)
      ...
      while ((word = getNextWord_ (&window)) != NULL)
	{
	  if (addWindow (word, hPtr->week, hPtr->hostName) < 0)
	    {
	      ls_syslog (LOG_ERR, "\
%s: %s(%d): Bad time expression %s; ignored", __func__, fileName, *LineNumPtr, word);
	      lim_CheckError = WARNING_ERR;
	      free (hPtr->windows);
	      hPtr->windows = strdup ("-");
	      hPtr->wind_edge = 0;
...
static int
addHostInstance (struct sharedResource *sharedResource, int nHosts,
		 char **hostNames, char *value, int resourceMap)
{
  ...
  if (resourceMap == FALSE)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5405, "%s: More than one instatnce defined for the resource <%s> on host <%s> in host section; ignoring"), fname, sharedResource->resourceName, hostNames[0]);	/* catgets 5405 */
	  return (-1);
	}
	...
	{
	  if ((hostList =
	       getValidHosts (hostNames[i], &numList,
			      sharedResource)) == NULL)
	    continue;
	  for (j = 0; j < numList; j++)
	    {
	      ...
	      for (k = 0; k < numHosts; k++)
		{
	      ...
	      if (duplicated)
		{
		  ls_syslog (LOG_WARNING, I18N (5478, "%s: Host %s is duplicated in resource %s mapping."),	/* catgets 5478 */
			     fname, hostList[j],
			     sharedResource->resourceName);
		  continue;
		}
      ...
      if ((instance =
	   addInstance (sharedResource, numHosts, temp, value)) == NULL)
	{
	  ls_syslog (LOG_ERR, I18N_FUNC_FAIL, fname, "addInstance");
	  return (-1);
	}
	...
	return (-1);
      ...
      if (addSharedResourceInstance
	  (numHosts, temp, sharedResource->resourceName) < 0)
	{
		...
		int nHosts, char **hostNames)
{
  ...
  if ((resNo = resNameDefined (instance->resName)) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: Resource name <%s> is not defined in resource section in lsf.shared", __func__, instance->resName);
      return 0;
    }
...
%s: Invalid hostname %s ", __func__, hostNames[i]);
	...
	{
	  ls_syslog (LOG_WARNING, "\
%s: Host %s is not defined in host sectionin lsf.cluster", __func__, hostNames[i]);
	  continue;
	}

      if (hPtr->numInstances > 0
	  && isInHostNodeIns (instance->resName,
			      hPtr->numInstances, hPtr->instances) != NULL)
	continue;

      if (hPtr->numInstances > 0)
	temp = realloc (hPtr->instances,
			(hPtr->numInstances +
			 1) * sizeof (struct resourceInstance *));
      else
...
static int
addHostList (struct resourceInstance *resourceInstance, int nHosts,
	     char **hostNames)
{
  ...
  for (i = 0; i < nHosts; i++)
	...
	{
	  ls_syslog (LOG_DEBUG3,
		     "addHostList: Host <%s> is not used by cluster <%s> as a server:ignoring",
...
static struct resourceInstance *
addInstance (struct sharedResource *sharedResource, int nHosts,
	     char **hostNames, char *value)
{
  ...
  for (i = 0; i < nHosts; i++)
	...
	continue;
      ...
      if ((hPtr =
	   findHostbyList (myClusterPtr->hostList, hostNames[i])) == NULL)
	{
	  ls_syslog (LOG_DEBUG3,
		     "addInstance: Host <%s> is not used by cluster <%s> as server;ignoring",
		     hostNames[i], myClusterName);
	  continue;
	}
  ...
  if ((insPtr =
       (struct resourceInstance **) myrealloc (sharedResource->instances,
...
isInHostList (struct sharedResource *sharedResource, char *hostName)
{
  ...
  for (i = 0; i < sharedResource->numInstances; i++)
	...
	{
	  if (strcmp
	      (sharedResource->instances[i]->hosts[j]->hostName,
	       hostName) == 0)
...
addHostType (char *type)
{
  ...
  if (allInfo.nTypes == MAXTYPES)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5353, "%s: Too many host types defined in section HostType. You can only define up to %d host types; host type %s ignored"),	/* catgets 5353 */
		 fname, MAXTYPES, type);
      return (FALSE);
    }
  ...
  for (i = 0; i < allInfo.nTypes; i++)
      ...
      if (strcmp (type, "DEFAULT") == 0)
	{
      ...
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5354, "%s: host type %s multiply defined"),	/* catgets 5354 */
		 fname, type);
      return (FALSE);
    }
...
addHostModel (char *model, char *arch, float factor)
{
  ...
  if (allInfo.nModels == MAXMODELS)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5355, "%s: Too many host models defined in section HostModel. You can only define up to %d host models; host model %s ignored"),	/* catgets 5355 */
  ...
  for (i = 0; i < allInfo.nModels; ++i)
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5357, "%s: host model %s multiply defined"),	/* catgets 5357 */
		     fname, model);
	  return (TRUE);
	}
	...
	continue;
      ...
      ls_syslog (LOG_ERR, I18N (5479, "%s: host architecture %s defined multiple times"),	/*catgets 5479 */
		 fname, arch);
...
%s: Ignoring duplicate cluster %s", __func__, clName);
  ...
  while ((word = getNextWord_ (&sp)) != NULL && i < MAXCANDHOSTS)
	...
	{
	  ls_syslog (LOG_ERR, "\
%s: Invalid host %s for cluster %s, ignoring", __func__, word, clName);
	  lim_CheckError = WARNING_ERR;
	  continue;
	}
...
findClusterServers (char *clName)
{
  ...
  for (;;)
      ...
      if (!cp)
	{
      ...
      if (!word)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5360, "%s: %s(%d): Keyword expected after Begin, ignoring section"), fname, fileName, LineNum);	/* catgets 5360 */
	  lim_CheckError = WARNING_ERR;
	  doSkipSection (clfp, &LineNum, fileName, "unknown");
	}
      else if (strcasecmp (word, "host") == 0)
...
reCheckRes (void)
{
  ...
  for (i = 0, j = 0; i < allInfo.nRes; i++)
	...
	{
	  memcpy (&newTable[j],
		  &allInfo.resTable[i], sizeof (struct resItem));
	  j++;
	}
    }
  for (i = 0; i < allInfo.nRes; i++)
...
reCheckClusterClass (struct clusterNode *clPtr)
{
  ...
  if (clPtr->hostTypeBitMaps == NULL)
	...
	calloc (GET_INTNUM (allInfo.nTypes), sizeof (int));
      if (clPtr->hostTypeBitMaps == NULL)
	{
	  ls_syslog (LOG_ERR, I18N_FUNC_FAIL_M, fname, "malloc");
	  return (-1);
...
setExtResourcesDef (char *resName)
{
  ...
  if ((type = validType (extResInfoPtr->type)) >= 0)
    ...
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5371, "%s: type <%s> for external resource <%s> is not valid"),	/* catgets 5371 */
  ...
  if (extResInfoPtr->interval != NULL && extResInfoPtr->interval[0] != '\0')
	...
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5372, "%s: interval <%s> for external resource <%s> should be a integer greater than 0"),	/* catgets 5372 */
  ...
  if (extResInfoPtr->increasing != NULL
      ...
      if (allInfo.resTable[allInfo.nRes].valueType == LS_NUMERIC)
	{
	  if (!strcasecmp (extResInfoPtr->increasing, "N"))
		...
		{
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5373, "%s: increasing <%s> for resource <%s> is not valid"),	/* catgets 5373 */
	...
	ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5374, "%s: increasing <%s> is not used by the resource <%s> with type <%s>; ignoring INCREASING"),	/* catgets 5374 */
  ...
  else
    {
      if (allInfo.resTable[allInfo.nRes].valueType == LS_NUMERIC)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5375, "%s: No increasing specified for a numeric resource <%s>"),	/* catgets 5375 */
		     fname, resName);
	  setExtResourcesDefDefault (resName);
	  return (0);
	}
...
setExtResourcesLoc (char *resName, int resNo)
{
  ...
  if (extResLocPtr == NULL || extResLocPtr[0] == '\0')
    {
      ls_syslog (LOG_INFO, I18N (5379, "%s: Failed to get LOCATION specified for external resource <%s>; Set to default"),	/* catgets 5379 */
		 fname, resName);
...
getExtResourcesDef (char *resName)
{
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5453, "%s: external resource object is current not support in this platform"), fname);	/* catgets 5453 */
  return (NULL);
}
...
getExtResourcesLoc (char *resName)
{
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5454, "%s: external resource object is current not support in this platform"), fname);	/* catgets 5454 */
  return (NULL);
}
...
getExtResourcesVal (char *resName)
{
  ...
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5455, "%s: external resource object is current not support in this platform"), fname);	/* catgets 5455 */
		...
		struct sockaddr_in *from, struct LSFHeader *reqHdr, int chan)
{
  ...
  if (node)
    {
      ls_syslog (LOG_WARNING, "\
%s: trying to add already configured host %s from %s", __func__, hPtr.hostName, sockAdd2Str_ (from));
  ...
  if ((node = addHost_ (myClusterPtr,
			&hPtr, hPtr.window, (char *) __func__, &cc)) == NULL)
    {
      ls_syslog (LOG_ERR, "\
...
%s: failed decoding header from %s", __func__, sockAdd2Str_ (from));
  ...
  if (chanWrite_ (chan, buf, XDR_GETPOS (&xdrs2)) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: Failed replying to %s dbytes %m", __func__, sockAdd2Str_ (from), XDR_GETPOS (&xdrs2));
      xdr_destroy (&xdrs2);
      return;
    }
  ...
  for (e = h_firstEnt_ (tab, &stab); e != NULL; e = h_nextEnt_ (&stab))
    {
      ...
      if ((node = addHost_ (myClusterPtr,
			    &hPtr,
			    hPtr.window, (char *) __func__, &cc)) == NULL)
	{
	  ls_syslog (LOG_ERR, "\
%s: failed adding runtime host %s", __func__, hPtr.hostName);
      ...
      ls_syslog (LOG_DEBUG, "\
%s: runtime host %s model %s type %s added all right", __func__, hPtr.hostName, hPtr.hostModel, hPtr.hostType);
...
rmMigrantHost (XDR * xdrs, struct sockaddr_in *from, struct LSFHeader *reqHdr, unsigned long chan)
{
  ...
  if (hPtr == NULL)
    {
      ls_syslog (LOG_WARNING, "\
%s: trying to remove unknown host %s from %s", __func__, hostName, sockAdd2Str_ (from));
