  ...
  if (s < 0)
    {
      cc = chanSendDgram_ (limSock,
			   buf,
			   XDR_GETPOS (&xdrs), (struct sockaddr_in *) from);
    }
  else
    {
...
%s: chanSendDgram_ info to %s", __func__, sockAdd2Str_ (to));
  ...
  if (chanSendDgram_ (limSock,
		      buf, XDR_GETPOS (&xdrs), (struct sockaddr_in *) to) < 0)
    {
      ls_syslog (LOG_ERR, "\
...
announceMasterToHost (struct hostNode *hPtr, int infoType)
{
  ...
  if (!xdr_LSFHeader (&xdrs, &reqHdr)
    ...
    {
      ls_syslog (LOG_ERR, "\
%s: Error xdr_LSFHeader/xdr_masterReg to %s", __func__, sockAdd2Str_ (&toAddr));
      xdr_destroy (&xdrs);
      return;
    }
  ...
  ls_syslog (LOG_DEBUG, "\
%s: Sending request %d to LIM on %s", __func__, infoType, sockAdd2Str_ (&toAddr));

  if (chanSendDgram_ (limSock,
		      buf,
		      XDR_GETPOS (&xdrs), (struct sockaddr_in *) &toAddr) < 0)
    ls_syslog (LOG_ERR, "\
%s: Failed to send request %d to LIM on %s: %m", __func__, infoType, sockAdd2Str_ (&toAddr));
...
probeMasterTcp (struct clusterNode *clPtr)
{
  ...
  ls_syslog (LOG_ERR, "\
%s: Attempting to probe last known master %s port %d timeout is %d", __func__, hPtr->hostName, ntohs (hPtr->statInfo.portno), probeTimeout);
