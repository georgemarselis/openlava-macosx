...
callLim_ (enum limReqCode reqCode,
	  ...
	  void *drecv,
	  bool_t (*xdr_rfunc) (),
	  char *host, int options, struct LSFHeader *hdr)
{
...
%s: failed in connecting to limChans_[TCP]=<%d> <%s>", __func__, limchans_[TCP], sockAdd2Str_ (&sockIds_[TCP]));
  ...
  cc = chanRpc_ (limchans_[TCP],
		 &sndbuf, &rcvbuf, replyHdr, recvtimeout_ * 1000);
  if (cc < 0)
    {
      CLOSECD (limchans_[TCP]);
      return (-1);
  ...
  switch (replyHdr->opCode)
      ...
      if (!xdr_masterInfo (&xdrs, &masterInfo_, replyHdr))
	{
	  ...
	  || !memcmp (&masterInfo_.addr,
		      &sockIds_[MASTER].sin_addr, sizeof (in_addr_t)))
	{
...
callLimUdp_ (char *reqbuf,
	     char *repbuf,
	     int len, struct LSFHeader *reqHdr, char *host, int options)
{
  struct hostent *hp;
  int retried = 0;
  u_int masterLimAddr;
