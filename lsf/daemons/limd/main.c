...
%s: Checking Done. Warning(s)/error(s) found.", __func__);
  ...
  ls_syslog (LOG_INFO, "\
%s: Daemon running (%d %d %d)", __func__, myClusterPtr->checkSum, ntohs (myHostPtr->statInfo.portno), OPENLAVA_VERSION);
  ls_syslog (LOG_DEBUG, "\
%s: sampleIntvl %f exchIntvl %f hostInactivityLimit %d masterInactivityLimit %d retryLimit %d", __func__, sampleIntvl, exchIntvl, hostInactivityLimit, masterInactivityLimit, retryLimit);
...
processUDPMsg (void)
{
  ...
  if (cc < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: chanRcvDgram() failed limSock %d: %m", __func__, limSock);
      return -1;
    }
...
%s: Received request %d from unresolvable address %s", __func__, limReqCode, sockAdd2Str_ (&from));
  ...
  ls_syslog (LOG_DEBUG, "\
%s: Received request %d from host %s %s", __func__, limReqCode, (fromHost ? fromHost->hostName : hp->h_name), sockAdd2Str_ (&from));
...
%s: failed accept() new connection socket %d: %M", __func__, limTcpSock);
  ...
  if (fromHost == NULL && limParams[LIM_NO_MIGRANT_HOSTS].paramValue)
    {
...
%s: LSF_LIM_PORT <%s> must be a positive number", __func__, limParams[LSF_LIM_PORT].paramValue);
  ...
  if (limSock < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: unable to create datagram socket port %d; another LIM running?: %M ", __func__, lim_port);
      return -1;
    }
