		  ...
		  if (hPtr->numInstances > 0)
		      ...
		      for (i = 0; i < hPtr->numInstances; i++)
			{
			  if (hPtr->instances[i]->updHost == NULL
			    ...
			    continue;
			  resNo =
			    resNameDefined (hPtr->instances[i]->resName);
		  ...
		  if (logclass & LC_COMM)
		    {
		      ls_syslog (LOG_DEBUG3,
				 "%s: Asking %s to send load info %d %d",
...
%s: compute only host will not try to take over mastership", __func__);
  ...
  if (!masterMe)
	...
	+ myLoadVector.numResPairs * sizeof (struct resPair) + 100;
      for (i = 0; i < myLoadVector.numResPairs; i++)
	{
	  bufSize +=
	    ALIGNWORD_ (strlen (myLoadVector.resPairs[i].name) *
			sizeof (char) + 1) + 4;
	  bufSize +=
	    ALIGNWORD_ (strlen (myLoadVector.resPairs[i].value) *
			sizeof (char) + 1) + 4;
	}
...
%s: message bigger then receive buf(%d)", __func__, bufSize);
	...
	{
	  ls_syslog (LOG_ERR, I18N_FUNC_FAIL, __func__,
		     "xdr_enum/xdr_loadvector");
	  xdr_destroy (&xdrs);
	  FREEUP (repBuf);
	  return;
	}
      ...
      memcpy (&toAddr.sin_addr.s_addr,
	      &myClusterPtr->masterPtr->addr[0], sizeof (in_addr_t));

      if (logclass & LC_COMM)
	ls_syslog (LOG_DEBUG, "\
sendLoad: sending to %s (len=%d,port=%d)", sockAdd2Str_ (&toAddr), XDR_GETPOS (&xdrs), ntohs (lim_port));
...
rcvLoad (XDR * xdrs, struct sockaddr_in *from, struct LSFHeader *hdr)
{
  ...
  if (from->sin_port != lim_port)
    {
      ls_syslog (LOG_ERR, "\
%s: Update not from LIM: %s, expected %d", __func__, sockAdd2Str_ (from), ntohs (lim_port));
      return;
    }
  ...
  if (loadType != e_vec)
    {
      ls_syslog (LOG_ERR, "\
%s: Invalid load type %d from host %s", __func__, loadType, sockAdd2Str_ (from));
      return;
    }
...
%s: Error in xdr_loadvector from %s", __func__, sockAdd2Str_ (from));
  ...
  if (!masterMe)
    {
      ls_syslog (LOG_DEBUG, "\
%s: %s thinks I am the master, but I'm not", __func__, sockAdd2Str_ (from));
      return;
    }
...
%s: Sender (%s) may have different config?.", __func__, sockAdd2Str_ (from));
  ...
  if (hPtr == NULL)
    {
      ls_syslog (LOG_ERR, "\
%s: Received load update from unknown host %s", __func__, sockAdd2Str_ (from));
      return;
    }
  ...
  if (findHostbyList (myClusterPtr->hostList, hPtr->hostName) == NULL)
    {
      ls_syslog (LOG_ERR, "\
%s: Got load from client-only host %s.  Kill LIM on %s", __func__, sockAdd2Str_ (from), sockAdd2Str_ (from));
      return;
    }
...
%s: Received load update from host %s", __func__, hPtr->hostName);
  ...
  if (loadVector->seqNo - hPtr->lastSeqNo > 2
    ...
    ls_syslog (LOG_ERR, "\
%s: host %s lastSeqNo=%d seqNo=%d. Packets being dropped?", __func__, hPtr->hostName, hPtr->lastSeqNo, loadVector->seqNo);
  hPtr->lastSeqNo = loadVector->seqNo;

  copyResValues (*loadVector, hPtr);
  copyIndices (loadVector->li,
	       loadVector->numIndx, loadVector->numUsrIndx, hPtr);
...
copyIndices (float *lindx, int numIndx, int numUsrIndx, struct hostNode *hPtr)
{
  ...
  for (i = 0; (i < numUsrIndx) && (i < allInfo.numUsrIndx); i++)
    {
