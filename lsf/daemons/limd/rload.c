...
putLastActiveTime ()
{
  ...
  if (putEnv (ENV_LAST_ACTIVE_TIME, lsfLastActiveTime) != 0)
    {
      ls_syslog (LOG_WARNING, I18N (5902, "putLastActiveTime: %s, failed."),	/* catgets 5902 */
		 lsfLastActiveTime);
    }
}
...
saveIndx (char *name, float value)
{
  ...
  if (indx < 0)
      ...
      for (i = NBUILTINDEX; names[i] && i < allInfo.numIndx; i++)
	{
      ...
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5920, "%s: Unknown index name %s from ELIM"),	/* catgets 5920 */
		 fname, name);
      if (names[i])
	{
	  FREEUP (names[i]);
  ...
  if (indx < NBUILTINDEX)
      ...
      if (!names[indx])
	{
	  ...
	  ls_syslog (LOG_WARNING, _i18n_msg_get (ls_catd, NL_SETN, 5921, "%s: ELIM over-riding value of index %s"),	/* catgets 5921 */
		     fname, name);
	}
      overRide[indx] = value;
    }
...
getSharedResBitPos (char *resName)
{
  ...
  for (tmpSharedRes = sharedResourceHead, bitPos = 0;
       tmpSharedRes; tmpSharedRes = tmpSharedRes->nextPtr, bitPos++)
    {
...
getusr (void)
{
  ...
  if ((elim_pid < 0) && (time (0) - lastStart > 90))
      ...
      if (ELIMrestarts < 0 || ELIMrestarts > 0)
	{
	  ...
	  ls_syslog (LOG_INFO, (_i18n_msg_get (ls_catd, NL_SETN, 5930, "%s: Started ELIM %s pid %d")), fname,	/* catgets 5930 */
	  ...
	  if (!fromELIM)
	    {
	      ...
	      ls_syslog (LOG_ERR, I18N (5935, "%s:Received from ELIM: <out of memory to record contents>"),	/* catgets 5935 */
			 fname);
...
%s: Signal mask has been changed, all are signals blocked now", fname);
      ...
      if (ELIMblocktime >= 0)
	{
      ...
      if (cc != 1)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5924, "%s: Protocol error numIndx not read (cc=%d): %m"), fname, cc);	/* catgets 5924 */
	  lim_pclose (fp);
	  fp = NULL;
      ...
      if (numIndx < 0)
	{
	  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5925, "%s: Protocol error numIndx=%d"), fname, numIndx);	/* catgets 5925 */
	  setUnkwnValues ();
	  lim_pclose (fp);
	  fp = NULL;
	  unblockSigs_ (&oldMask);
      ...
      while (i)
	{
	  ...
	  if (cc == -1)
	      ...
	      if (scanerrno != EAGAIN || scc <= 0)
		{
		  ...
		  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5926, "%s: Protocol error, expected %d more tokens (cc=%d): %m"),	/* catgets 5926 */
			     fname, i, cc);


		  ls_syslog (LOG_ERR, I18N (5904, "Received from ELIM: %s."),	/* catgets 5904 */
			     fromELIM);
      ...
      if (ELIMdebug)
	{
	  ...
	  ls_syslog (LOG_WARNING, I18N (5903, "ELIM: %s."),	/* catgets 5903 */
		     fromELIM);
	}
    }
}
...
saveSBValue (char *name, char *value)
{
  ...
  for (i = 0; i < myHostPtr->numInstances; i++)
	...
	ls_syslog (LOG_DEBUG3,
		   "saveSBValue: i = %d, resName=%s, value=%s, newValue=%s, updHost=%s",
...
initConfInfo (void)
{
  ...
  if (myHostPtr->statInfo.maxCpus <= 0)
    {
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5928, "%s: Invalid num of CPUs %d. Default to 1"),	/* catgets 5928 */
		 fname, myHostPtr->statInfo.maxCpus);
