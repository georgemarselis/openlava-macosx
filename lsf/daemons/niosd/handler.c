...
int
ls_nioselect (int nfds, fd_set * readfds, fd_set * writefds,
  ...
  for (;;)
      ...
      if (writeBuf.empty)
	{
	  for (notice = (taskNotice_t *) notifyList->forw;
	       notice != (taskNotice_t *) notifyList; notice = nextNotice)
	    {
		  ...
		  switch (msgHdr.opCode)
		      ...
		      if (retVal < 0 && lserrno == LSE_MALLOC)
			{
			  ...
			  add_list (&readyTaskList, conn[i].rpid, NIO_IOERR,
				    NULL);
			  add_list (&readyTaskList, conn[i].rpid, NIO_EOF,
				    NULL);
			  naborted++;
			  DISCONNECT (i);
			  lserrno = retVal;
  ...
  for (i = 0; i < lastConn; i++)
	...
	continue;
      ...
      for (task = (rtaskInfo_t *) conn[i].taskList;
	   task != (rtaskInfo_t *) conn[i].taskList; task = nextTask)
	{
	  nextTask = task->forw;
      ...
      for (i = 0; i < lastConn; i++)
	{
	  if (C_CONNECTED (conn[i]) && FD_ISSET (i, &writeBuf.socks)
		...
		empty = FALSE;
	      ...
	      if ((cc < 0) && (errno != EPIPE) && BAD_IO_ERR (errno))
		{
		  memset ((void *) &status, 0, sizeof (LS_WAIT_T));
		  SETTERMSIG (status, STATUS_IOERR);
		  if (bury_task (status, 0, conn[i].rpid) == -1)
...
do_newconnect (int s)
{
  ...
  while (!LIST_IS_EMPTY (conn[i].taskList))
    removeTask (conn[i].taskList, (rtaskInfo_t *) conn[i].taskList->forw);
  if (conn[i].rbuf != NULL)
    FREEUP (conn[i].rbuf);
  conn[i].rbuf = (RelayLineBuf *) malloc (sizeof (RelayLineBuf));
  if (conn[i].rbuf == NULL)
...
bury_task (LS_WAIT_T status, struct rusage *ru, int rpid)
{
  ...
  if (ru)
    {
      memcpy ((char *) &(prpid->ru), (char *) ru, sizeof (struct rusage));
    }
  else
    {
  ...
  for (i = 0; i < lastConn; i++)
      ...
      if (conn[i].sock.fd > 0)
	{
	  if (callRes_ (conn[i].sock.fd, NIOS2RES_SETTTY, (char *) &tty,
			buf, MSGSIZE, xdr_resStty, 0, 0, NULL) == -1)
	    {
...
%s: Nios job has exited from LSF system exitStatus<%d>", __func__, WEXITSTATUS (wStatus));
	  ...
	  else
	    {
	      ...
	      fprintf (stderr, I18N (901, "<<<Job gone from LSF system>>>\r\n"));	/* catgets 901 */
	      kill_self (0, -11);
	    }
	}
...
%s: Nios pending job timeout %dmin  expired, killing the job\n", __func__, pendJobTimeout / 60);
		...
		{
		  execl ("/bin/sh",
			 "/bin/sh", "-c", getenv ("LSF_NIOS_DIE_CMD"), NULL);
		  perror ("/bin/sh");
		  exit (-10);
		}
	    }
	  ...
	  if (msgInterval > 0 && (now - lastMsgCheck) >= msgInterval)
	    {
	      JobStateInfo (jobId);
	      lastMsgCheck = now;
	    }
	      ...
	      struct jobInfoHead ** jobHead)
{
  ...
  jobInfoHead = lsb_openjobinfo_a (jid, NULL, "all", NULL, NULL, ALL_JOB);
  if (jobInfoHead == NULL)
    {
