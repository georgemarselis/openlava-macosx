...
main (int argc, char **argv)
{
  ...
  if (niosParams[LSF_NIOS_RES_HEARTBEAT].paramValue)
      ...
      if (isint_ (niosParams[LSF_NIOS_RES_HEARTBEAT].paramValue))
	{
	  ...
	  heartbeatInterval =
	    atoi (niosParams[LSF_NIOS_RES_HEARTBEAT].paramValue) * 60;
  ...
  if (niosParams[LSF_NIOS_JOBSTATUS_INTERVAL].paramValue)
      ...
      if (isint_ (niosParams[LSF_NIOS_JOBSTATUS_INTERVAL].paramValue))
	{
	  ...
	  jobStatusInterval =
	    atoi (niosParams[LSF_NIOS_JOBSTATUS_INTERVAL].paramValue) * 60;
  ...
  if (argc == 3)
      ...
      if (!strcmp (argv[1], "-p"))
	{
	  ...
	  if ((asock = TcpCreate_ (TRUE, atoi (argv[2]))) < 0)
	    {
	      ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_M, __func__, "tcpCreate", "-p");
	      exit (-1);
	    }
      ...
      else if (!strcmp (argv[1], "-N"))
	{
	  ...
	  if (niosDebug)
	    {
	      ls_syslog (LOG_DEBUG, "%s: Nios running in sbdMode mode", __func__);
	    }
	  ...
	  if (getenv ("BSUB_BLOCK") != NULL)
	  ...
	  else
	    {
		  ...
		  strcasecmp (niosParams[LSB_INTERACT_MSG_ENH].paramValue,
			      "y") == 0)
		{
		  msgEnabled = TRUE;
		  ...
		  if (niosParams[LSB_INTERACT_MSG_INTVAL].paramValue)
		    {
		      if (isint_
			  (niosParams[LSB_INTERACT_MSG_INTVAL].paramValue))
			{
		  ...
		  if ((sp = getenv ("LSB_JOBID")) == NULL)
		    {
		    	/* catgets 5803 */
		      ls_syslog (LOG_ERR, I18N (5803, "%s: LSB_JOBID is not defined %M"),  __func__);
		      exit (-1);
		    }
	  ...
	  if (standalone)
	    {
	      if (niosDebug)
		{
		  ls_syslog (LOG_DEBUG, "%s: Nios ls_niotasks returned 0, got_eof=%d", __func__, got_eof);
		  ls_syslog (LOG_DEBUG, "%s: Nios exit_sig=%d exit_status=%d. exiting", __func__, exit_sig, exit_status);
		}
	      PassSig (SIGKILL);
	      die ();
	    }
	  ...
	  for (i = 0; i < tasks->num; i++)
		...
		case NIO_STDERR:
		  if (niosDebug)
			...
			{
			  ls_syslog (LOG_DEBUG, "%s: Nios Got stderr from connection <%d>", __func__, tasks->ioTask[i].tid);
			}
		      else
			{
			  ls_syslog (LOG_DEBUG, "%s: Nios Got stdout from connection <%d>", __func__, tasks->ioTask[i].tid);
			}
		    }
		  ...
		  if (usepty || standalone)
		    {
		      if (usepty && tasks->ioTask[i].type == NIO_STDERR)
			{
			  dumpOption = 2;
			}
		      else
		  ...
		  if (ls_niodump (outputFd, tasks->ioTask[i].tid, dumpOption, taggingFormat) < 0)
		    {
		      ls_syslog (LOG_ERR, I18N_FUNC_FAIL_M, __func__, "ls_niodump");
		      ls_niokill (SIGKILL);
		      die ();
		    }
		  break;
		case NIO_EOF:
		  if (niosDebug)
		    ls_syslog (LOG_DEBUG, "%s: Nios got EOF from remote task standalone=%d got_status=%d", __func__, standalone, got_status);
		...
		case NIO_STATUS:
		  if (niosDebug)
		    ls_syslog (LOG_DEBUG, "%s: Nios Got status <%#x> from task remote", __func__, tasks->ioTask[i].status);
		  ...
		  {
		    LS_WAIT_T status =
		      *((LS_WAIT_T *) & tasks->ioTask[i].status);
			...
			if ((niosSbdMode == TRUE) && (msgEnabled == TRUE))
			  {
			    ...
			    if (LS_WSTOPSIG (status) == SIGCONT)
			      {
				prtLine (I18N (801, "Starting after being resumed"));	/* catgets 801 */
				isResumed = 1;
				...
				while ((oldState == JOB_STAT_RUN)
				       && (tryTimes >= 0));

			      }
			  }

			emusig (tasks->ioTask[i].tid,
				tasks->ioTask[i].status);
		...
		case NIO_IOERR:
		  if (standalone)
		    {
		      ls_syslog (LOG_ERR, I18N (5806, "%s: Nios IO_ERR while reading from remote task"),	/* catgets 5806 */
				 __func__);
		...
		case NIO_REQUEUE:
		  if (requeued && niosSbdMode && standalone)
		    {
		      if (niosDebug)
			ls_syslog (LOG_DEBUG, "%s: Nios got REQUEUE from remote task standalone=%d got_status=%d", __func__, standalone, got_status);
      ...
      if (FD_ISSET (STDIN_FD, &rmask) && (readCount == 0))
	{
	  ...
	  if (readCount > 0)
	      ...
	      if (usepty && directecho)
		{
	      ...
	      if (niosDebug)
		{
		  ls_syslog (LOG_DEBUG, "%s: Nios NIOS2RES_STDIN <%d> bytes to remote task", __func__, readCount);
		}
		...
		if (niosDebug)
		  {
		    ls_syslog (LOG_DEBUG, "%s: Nios got <EOF> NIOS2RES_EOF for all current remote tasks", __func__);
		  }
		ls_nioclose ();
...
{
  char __func__] = "kill_self()";

  if (niosDebug)
    {
      ls_syslog (LOG_DEBUG, "%s: Nios kill_self, exit_sig=%d exit_stat=%d", __func__, exit_sig, exit_stat);
    }
  ...
  if (exit_sig)
...
#if defined(SIGXCPU)
	case SIGXCPU:
	  ls_syslog (LOG_ERR, I18N (5807, "%s: Nios receives signal SIGXCPU, exit\n"),	/* catgets 5807 */
		     __func__);
	  exit (exit_stat);
	  break;
#endif
#if defined(SIGXFSZ)
	case SIGXFSZ:
	  ls_syslog (LOG_ERR, I18N (5808, "%s: Nios receives signal SIGXFSZ, exit"),	/* catgets 5808 */
		     __func__);
	  exit (exit_stat);
	  break;
#endif
	...
	default:
	  ...
	  ls_syslog (LOG_ERR, I18N (5809, "%s: Nios does not die at sig %d: errno %d"),	/* catgets 5809 */
...
do_newtask (void)
{
  ...
  switch (hdr.opCode)
    ...
    default:
      ls_syslog (LOG_ERR, I18N (5812, "%s: No such service provided by NIOS code = %d"),	/* catgets 5812 */
		 __func__, hdr.opCode);
    }
}
...
emusig (int tid, int st)
{
  ...
  if (LS_WIFSTOPPED (status))
      ...
      if (standalone)
	{
	  ...
	  if (niosDebug)
	    {
	      ls_syslog (LOG_DEBUG, "%s: Nios remote signaled exit_sig=<%d> exit_status=<%d>", __func__, exit_sig, exit_status);
	    }
      ...
      else
	{
	  ...
	  if (niosDebug)
	    {
	      ls_syslog (LOG_DEBUG, "%s: Nios signaled exit_sig=<%d> sending SIGUSR1 to oparent", __func__, LS_WTERMSIG (status));
	    }
...
PassSig (int signo)
{
  char __func__] = "PassSig()";
  sigset_t omask, nmask;

  if (niosDebug)
    {
      ls_syslog (LOG_DEBUG, "%s: Nios NIOS2RES_SIGNAL delivering signal = <%d> to remote tasks.", __func__, signo);
    }
...
{
  char __func__] = "nios/acceptCallBack()";
  int sock;
  char *sp;
  ...
  if ((sp = getenv ("LSB_JOBID")) == NULL)
    {
      ls_syslog (LOG_ERR, "%s: LSB_JOBID is not defined", __func__);
      exit (-1);
    }
  ...
  for (;;)
    {
	  ...
	  && ((jobStatusInterval > 0)
	      || (pendJobTimeout > 0) || (msgInterval > 0)))
	{
		  ...
		  if (LS_WIFSIGNALED (status) || LS_WIFEXITED (status))
		      ...
		      if (connReq.terWhiPendStatus == 1)
			{
			  if (!getenv ("BSUB_QUIET2"))
			    {
			      fprintf (stderr, I18N (804, "<<Terminated while pending>>\r\n"));	/* catgets 804 */
			    }
		      ...
		      else
			{
			  if (!getenv ("BSUB_QUIET2"))
			    {
			      fprintf (stderr, I18N (805, "<<Job is finished>>\r\n"));	/* catgets 805 */
			    }
			  exit (out_status);
			}
		    }
  ...
  if (verbose)
      ...
      else
	{
	  ...
	  hp = Gethostbyaddr_ (&sin.sin_addr.s_addr, sizeof (in_addr_t), AF_INET);
	  if (hp == NULL)
	    {
	      ls_syslog (LOG_ERR, "%s: gethostbyaddr() failed for %s", __func__, sockAdd2Str_ (&sin));

	    }
	  else
	    {
	      fprintf (stderr, "<<Starting on %s>>\r\n", hp->h_name);
	    }
	}
    }
  ...
  if (isatty (0))
    ...
    {
    	/* catgets 5816 */
      ls_syslog (LOG_ERR, I18N (5816, "%s: usepty specified but TTY not detected\r\n"),	__func__);
      exit (-1);
    }
...
JobExitInfo (void)
{
  ...
  if (standaloneTaskDone)
    {
      ...
      sprintf (msg, I18N (808, "Job <%s> is done successfully."),	/* catgets 808 */
	       lsb_jobid2str (jobId));
      prtLine (msg);
      return;
    }
  ...
  if (niosParams[LSB_INTERACT_MSG_EXITTIME].paramValue)
	...
	{
	  niosExitTimeOut =
	    atoi (niosParams[LSB_INTERACT_MSG_EXITTIME].paramValue);
	}
    }
  ...
  while (1)
    {
	  ...
	  || (jStatus == JOB_STAT_SSUSP) || (jStatus == JOB_STAT_USUSP))
	{
...
prtJobStateMsg (struct jobInfoEnt *job, struct jobInfoHead *jInfoH)
{
  char prline[MSGSIZE] = "";
  time_t doneTime;
  struct loadIndexLog *loadIndex = NULL;
  ...
  switch (job->status)
    {
    case JOB_STAT_DONE:
      sprintf (prline, I18N (811, "Job <%s> is done successfully."),	/* catgets 811 */
	...
	{
	  sprintf (prline, I18N (812, "<Termination request issued; the job will be killed once the host is ok>"));	/* catgets 812  */
	  prtLine (prline);
	  break;
	}
      ...
      else
	{
	  ...
	  if (job->exitStatus)
		...
		{
		  sprintf (prline, I18N (813, "Job <%s> exited with exit code %d."),	/* catgets  813  */
			   lsb_jobid2str (job->jobId), WEXITSTATUS (wStatus));
		}
	      else
		{
		  sprintf (prline, I18N (814, "Job <%s> has been terminated by user or administrator:\n Exited by signal %d"),	/* catgets  814  */
			   lsb_jobid2str (job->jobId), WTERMSIG (wStatus));
		}
	    }
	  else
	    {
	      sprintf (prline, I18N (815, "Job <%s> has been terminated by user or administrator"),	/* catgets 815 */
		       lsb_jobid2str (job->jobId));
	    }
	}
      prtLine (prline);
      ...
      if (job->reasons)
	{
	  if ((job->reasons == SUSP_USER_STOP)
	    ...
	    {
	      sprintf (prline, I18N (816, "The job was suspended by user or administrator;"));	/* catgets 816 */
	    }
	  else
	    {
...
initLoadIndex (void)
{
  char __func__] = "initLoadIndex()";
  int i;
  struct lsInfo *lsInfo;
  struct loadIndexLog loadIndex;
  char *defNames[] = { "r15s", "r1m", "r15m", "ut", "pg", "io", "ls",
    "it", "swp", "mem", "tmp"
  };
  char **names;
