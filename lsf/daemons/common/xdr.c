...
xdr_jobSpecs (XDR * xdrs, struct jobSpecs *jobSpecs, struct LSFHeader *hdr)
{
  ...
  if (!xdr_int (xdrs, &nLimits))
    {
      ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S, fname,
		 lsb_jobid2str (tmpJobId), "xdr_int", "nLimits");
      return (FALSE);
    }
  ...
  for (i = 0; i < nLimits && i < LSF_RLIM_NLIMITS; i++)
      ...
      if (!xdr_lsfLimit (xdrs, &jobSpecs->lsfLimits[i], hdr))
	{
	  ls_syslog (LOG_ERR, I18N_JOB_FAIL_S, fname,
		     lsb_jobid2str (tmpJobId), "xdr_lsfLimit");
	  return (FALSE);
	}
    }
  ...
  if (!(xdr_string (xdrs, &sp[0], MAXFILENAMELEN) &&
    ...
    {
      ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S, fname,
		 lsb_jobid2str (tmpJobId), "xdr_int", "jobFile");
      return (FALSE);
    }
  if (xdrs->x_op == XDR_DECODE)
    sp[11][0] = '\0';
  ...
  for (i = 0; i < jobSpecs->numToHosts; i++)
	...
	{
	  ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S, fname,
		     lsb_jobid2str (tmpJobId), "xdr_var_string", "toHosts");
	  return (FALSE);
	}
    }
  ...
  if (!xdr_thresholds (xdrs, jobSpecs))
    ls_syslog (LOG_ERR, I18N_JOB_FAIL_S, fname,
	       lsb_jobid2str (tmpJobId), "xdr_thresholds");


  if (!xdr_int (xdrs, &jobSpecs->nxf))
    {
      ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S, fname,
		 lsb_jobid2str (tmpJobId), "xdr_int", "nxf");
      return (FALSE);
    }
  ...
  for (i = 0; i < jobSpecs->nxf; i++)
	...
	{
	  ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S, fname,
		     lsb_jobid2str (tmpJobId), "xdr_arrayElement", "xf");
	  return (FALSE);
	}
    }
...
bool_t
xdr_statusReq (XDR * xdrs, struct statusReq * statusReq,
...
bool_t
xdr_sbdPackage (XDR * xdrs, struct sbdPackage * sbdPackage,
		struct LSFHeader * hdr)
{
...
bool_t
xdr_sbdPackage1 (XDR * xdrs, struct sbdPackage * sbdPackage,
		 struct LSFHeader * hdr)
{
