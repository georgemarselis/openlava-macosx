...
sbdChildCloseChan (int exceptChan)
{
    ...
    if (logclass & LC_TRACE) {
        ls_syslog (LOG_DEBUG1, "%s: Entering...", __func__ );
    }
...
mysetLimits (struct jobSpecs *jobSpecsPtr)
{
    ...
    if( ( lsbJobCpuLimit != 1 ) ||
        ...
        rlimitDecode_ (&jobSpecsPtr->lsfLimits[LSF_RLIMIT_CPU], &rlimit, LSF_RLIMIT_CPU);
...
send_results (struct jobCard *jp)
{
    ...
    else
    {
        ...
        fflush (errout);
        fflush (notif);
        memset( ofileHost, '\0', strlen (ofileHost) );
        ...
        for ( unsigned int i = 0; i < jp->jobSpecs.nxf; i++) {
            ...
            if (jp->jobSpecs.xf[i].options & XF_OP_EXEC2SUB) {
                ...
                if (ofileHost[0] != '\0' && ofIdx == -1) {
                    ...
                    if ((host = ls_getmnthost (jp->jobSpecs.xf[i].execFn)) && ostat.st_dev == st.st_dev && ostat.st_ino == st.st_ino && !strcmp (host, ofileHost)) {
                        ofIdx = i;
                        continue;
                    }
                }
...
renewJobStat (struct jobCard *jp)
{
    ...
    if (jp->jobSpecs.actPid) {
        ...
        if (mbdJobStatus & JOB_STAT_EXIT) {
            ...
            if ((mbdReasons & EXIT_RESTART) || ((mbdReasons == EXIT_ZOMBIE) && !(jp->jobSpecs.options & SUB_CHKPNTABLE) && (jp->jobSpecs.options & SUB_RERUNNABLE))) {
            ...
            if (sbdLogFound == 0) {
                ...
                switch (MASK_STATUS (jp->jobSpecs.jStatus)) {
                    ...
                    case JOB_STAT_USUSP: {
                        ...
                        k = sbdStartupStopJob (jp, jp->jobSpecs.reasons, jp->jobSpecs.subreasons);
                    ...
                    break;
                    default: {
                        ls_syslog( LOG_ERR, "%s: switch( MASK_STATUS( jp->jobSpecs.jStatus ) ) hit default. You are not supposed to be here!", __func__ );
                    }
                    break;
                }
            }
                ...
                switch (MASK_STATUS (mbdJobStatus)) {
                    ...
                    break;
                    default: {
                        ls_syslog( LOG_ERR, "%s: switch( MASK_STATUS( mbdJobStatus ) ) hit default. You are not supposed to be here!", __func__ );
                    }
                    break;
                }
            }
...
refreshJob (struct jobSpecs *jobSpecs)
{
    ...
    for (jp = jobQueHead->forw; jp != jobQueHead; jp = jp->forw) {
        ...
        if (!(jp->postJobStarted) && (jp->jobSpecs.jStatus != jobSpecs->jStatus)) {
            jp->jobSpecs.jStatus = (jobSpecs->jStatus & ~JOB_STAT_MIG) | (jp->jobSpecs.jStatus & JOB_STAT_MIG);
        ...
        for ( size_t i = 0; i < LSB_SIG_NUM; i++) {
        ...
        for (size_t i = 0; i < LSF_RLIM_NLIMITS; i++) {
            memcpy ((char *) &jp->jobSpecs.lsfLimits[ i ], (char *) &jobSpecs->lsfLimits[j], sizeof (struct lsfLimit));
        }
        ...
        if ((strcmp (jp->jobSpecs.windows, jobSpecs->windows)) != 0)
        {
            ...
            while ((word = getNextWord_ (&cp)) != NULL) {
                ...
                if (addWindow (word, jp->week, "refreshJobs jobSpecs") < 0) {
                    ...
                    ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "addWindow", word);
                    /* catgets 458 */
        ...
        if (!(daemonParams[LSB_RENICE_NEVER_AT_RESTART].paramValue)) {
            if (reniceJob (jp) < 0) {
                ls_syslog (LOG_DEBUG, "refreshJob: reniceJob job <%s> failed", lsb_jobid2str (jp->jobSpecs.jobId));
            }
        }
    }
...
setIds (struct jobCard *jobCardPtr)
{
    ...
    if (!debug) {
        ...
        if ((tGname = getenv ("LSB_UNIXGROUP"))) { // FIXME FIXME FIXME FIXME "LSB_UNIXGROUP" should be put into configure.ac

            if (logclass & (LC_TRACE | LC_EXEC)) {
                ls_syslog (LOG_DEBUG, "got the LSB_UNIXGROUP=%s", tGname); // FIXME FIXME FIXME FIXME "LSB_UNIXGROUP" should be put into configure.ac
            }
                    ...
                    while (memp != NULL && *memp != NULL) {
                        if (strcmp (*memp, jobCardPtr->execUsername) == 0) {
                            ...
                            if (setgid (grEntry->gr_gid) < 0) {
                                ls_syslog (LOG_DEBUG, "setIds: Job <%s> setgid(%d) for LSB_UNIXGROUP=%s  failed: %m", lsb_jobid2str (jobSpecsPtr->jobId), (int) grEntry->gr_gid, sGname);
                            }
                            else {
...
deallocJobCard (struct jobCard *jobCard)
{
    ...
    sprintf (fileBuf, "%s/.%s.%s.fail", LSTMPDIR, jobCard->jobSpecs.jobFile, lsb_jobidinstr (jobCard->jobSpecs.jobId));  // FIXME FIXME FIXME FIXME ".fail" should be put into configure.ac
    ...
    sprintf (fileBuf, "%s/.%s.%s.chk", LSTMPDIR, jobCard->jobSpecs.jobFile, lsb_jobidinstr (jobCard->jobSpecs.jobId)); // FIXME FIXME FIXME FIXME ".chk" should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT)
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);

    sprintf (fileBuf, "%s/.%s.%s.resume", LSTMPDIR, jobCard->jobSpecs.jobFile, lsb_jobidinstr (jobCard->jobSpecs.jobId));  // FIXME FIXME FIXME FIXME ".resume" should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT) {
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__,lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);
    }
    ...
    sprintf (fileBuf, "%s/.%s.%s.terminate", LSTMPDIR, jobCard->jobSpecs.jobFile, lsb_jobidinstr (jobCard->jobSpecs.jobId)); // FIXME FIXME FIXME FIXME ".terminate" should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT) {
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);
    }

    sprintf (fileBuf, "%s/.%s.acct", LSTMPDIR, jobCard->jobSpecs.jobFile); // FIXME FIXME FIXME FIXME ".acct" should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT) {
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);
    }

    sprintf (fileBuf, "%s/.%s.%d.stt", LSTMPDIR, lsb_jobidinstr (jobCard->jobSpecs.jobId), jobCard->jobSpecs.jobPid); // FIXME FIXME FIXME FIXME ".stt" should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT) {
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);
    }

    sprintf (fileBuf, "%s/.%s.sbd/jobstatus.%s", LSTMPDIR, clusterName, lsb_jobidinstr (jobCard->jobSpecs.jobId)); // FIXME FIXME FIXME FIXME "sbd/jobstatus." should be put into configure.ac
    if (unlink (fileBuf) < 0 && errno != ENOENT) {
        ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), "unlink", fileBuf);
    }

    sprintf (fileBuf, "/tmp/.sbd/%s.rusage", jobCard->jobSpecs.jobFile); // FIXME FIXME FIXME FIXME "tmp/.sbd/%s.rusage." should be put into configure.ac
    if (logclass & LC_MPI) { // FIXME FIXME FIXME FIXME how and where does LC_MPI gets set?
        ls_syslog (LOG_DEBUG3, "deallocJobCard: unlink(%s) for MPI job <%s>: %m", fileBuf,lsb_jobid2str (jobCard->jobSpecs.jobId));
    }
...
freeToHostsEtc (struct jobSpecs *jobSpecs)
{
    ...
    for( size_t i = 0; i < jobSpecs->numToHosts; i++) {
    ...
    if (jobSpecs->numEnv > 0) {
        for ( size_t i = 0; i < jobSpecs->numEnv; i++) {
...
{
    // int i;
    jobSpecs->toHosts = my_calloc (specs->numToHosts, sizeof (char *), __func__); // FIXME FIXME FIXME replace my_calloc with a regular calloc call

    for( size_t i = 0; i < specs->numToHosts; i++) {
    ...
    for (i = 0; i < specs->nxf; i++) {
        memcpy ((char *) &(jobSpecs->xf[i]), (char *) &specs->xf[i], ssizeof (struct xFile));
    }

    jobSpecs->nxf = specs->nxf;

    if (specs->numEnv > 0) {
        jobSpecs->env = (char **) my_calloc (specs->numEnv, sizeof (char *), __func__);  // FIXME FIXME FIXME replace my_calloc with a regular calloc call
...
getLoginShell (char *jfData, char *jobFile, struct hostent *hp, int readFile)
{
    ...
    if( first_time ) {
        memset( shellPath, '\0', MAX_FILENAME_LEN );
        first_time = 0;
    }
...
createTmpJobFile (struct jobSpecs *jobSpecsPtr, struct hostent *hp, char *stdinFile)
{
    ...
    {
        sprintf (cmdBuf, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jobSpecsPtr->jobId), "myopen_", path); // FIXME FIXME FIXME FIXME "TMPDIR" should be put into configure.ac
        // goto Error;
        if (logclass & LC_EXEC) {
    ...
    size = 2 * (strlen (jobSpecsPtr->jobFile) + strlen (stdinFile)) + strlen (cmdBuf) + 25;

    for( unsigned int i = 0; execArgv[i]; i++) {
...
acctMapTo (struct jobCard *jobCard)
{
    ...
    if (jobCard->jobSpecs.userId == PC_LSF_CUGID)  {
        // goto trySubUser;
        if ((pw = my_getpwnam (jobCard->jobSpecs.userName, "acctMapTo/trySubUser")) == NULL) { // FIXME FIXME FIXME what is this constant string about?
            /* catgets 5441 */
    ...
    for( size_t j = 0; j < jobCard->jobSpecs.numEnv; i++) {
        if (logclass & LC_EXEC) {
    ...
    if (!sp) { // FIXME FIXME collapse everything into a single if
        if (logclass & LC_EXEC) {
            ls_syslog (LOG_DEBUG1, "%s: Job <%s>, no user level account mapping. Trying submission user", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId)); // FIXME FIXME FIXME what is this constant string about?
        }
        // goto trySubUser;
    ...
    while ((num = sscanf (sp, "%s %s%n", clusorhost, user, &ccount)) == 2) {
        ...
        if (strcmp (clusterName, clusorhost) == 0 || strcmp ("+", clusorhost) == 0) {
            mycluster = 1;
        }

        if (!mycluster) {
            if (((hp = Gethostbyname_ (clusorhost)) != NULL) && equalHost_ (hp->h_name, hostname)) {
                myhost = TRUE;
            }
            else {
                myhost = FALSE;
...
acctMapOk (struct jobCard *jobCard)
{
    ...
    strcat (hostfn, "/.lsfhosts"); // FIXME FIXME FIXME FIXME "/.lsfhosts" should be put into configure.ac
...
runQPre (struct jobCard *jp)
{
    ...
    if ((pid = fork ()) == 0) {
      /* child */
        ...
        for ( unsigned int j = 1; j < NSIG; j++) { // FIXME FIXME FIXME signal NSIG does not exist in linux
            Signal_ (j, SIG_DFL);
        }
    ...
    collectPreStatus (jp, pid, "runQPre"); // FIXME FIXME FIXME FIXME "runQPre" should be put into configure.ac
...
postJobSetup (struct jobCard *jp)
{
    ...
    if( NULL == jp->jobSpecs.execUsername )  {
    ...
    if (initPaths (jp, hp, NULL) < 0) {
        if (!(jp->jobSpecs.jStatus & JOB_STAT_PRE_EXEC) && jp->jobSpecs.reasons != PEND_QUEUE_PRE_FAIL) {
...
runUPre (struct jobCard *jp)
{
    ...
    if (pid == 0) {
        ...
        for( unsigned int i = 1; i < NSIG; i++) { // FIXME FIXME FIXME FIXME 
            Signal_ (i, SIG_DFL);
        }
...
void
collectPreStatus (struct jobCard *jp, pid_t pid, char *context)
{
    pid_t id = -1;
    LS_WAIT_T status;
    char errMsg[MAXLINELEN];
...
requeueJob (struct jobCard *jp)
{
    ...
    if (!WIFEXITED (status) || !jp->jobSpecs.requeueEValues || jp->jobSpecs.requeueEValues[0] == '\0') {
        return FALSE; 
    }
...
reniceJob (struct jobCard *jp)
{
    ...
    if (jp->runRusage.npgids == 0) {
        ...
        which = PRIO_PGRP;
        for( unsigned int i = 0; i < jp->runRusage.npgids; i++) {
...
updateRUsageFromSuper (struct jobCard *jp, char *mbuf)
{
    ...
    if (ret != 5) { // FIXME FIXME FIXME wtf is the context of 5? what is 4? 3? or 1?
        // FIXME FIXME FIXME FIXME "TMPDIR" should be put into configure.ac
        /* catgets 5462 */
    ...
    if( jusage.npids > 0 ) {
        ...
        for ( unsigned int i = 0; i < jusage.npids; i++) {
            if ((ret = sscanf (mbuf, "%d%d%d%d%n", &jusage.pidInfo[i].pid, &jusage.pidInfo[i].ppid, &jusage.pidInfo[i].pgid, &jusage.pidInfo[i].jobid, &cnt)) != 4) {
                /* catgets 5463 */
    ...
    if (jusage.npgids > 0) {
        ...
        for ( unsigned int i = 0; i < jusage.npgids; i++) {
...
updateJUsage (struct jobCard *jPtr, const struct jRusage *jRusage)
{
    ...
    if (logclass & LC_EXEC) {
        ls_syslog (LOG_DEBUG, "%s: current rusage of job %d utime=%d stime=%d mem=%d swap=%d", __func__, jPtr->jobSpecs.jobId, jPtr->runRusage.utime, jPtr->runRusage.stime, jPtr->runRusage.mem, jPtr->runRusage.swap);
    }
...
copyPidInfo (struct jobCard *jPtr, const struct jRusage *jRusage)
{
    ...
    if (jRusage->npids > 0) {
        ...
        if (jPtr->runRusage.pidInfo == NULL) {
            /* catgets 5500 */
            ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5500, "%s: failed to malloc (%d) bytes.\n"), __func__, (jRusage->npids) * (sizeof (struct pidInfo)));
            return;
        }

        jPtr->runRusage.npids = jRusage->npids;
        memcpy (jPtr->runRusage.pidInfo, jRusage->pidInfo, (jRusage->npids) * (sizeof (struct pidInfo)));
    }

    if (jRusage->npgids > 0) {

        jPtr->runRusage.pgid = (int *) my_calloc (jRusage->npgids, sizeof (int), __func__); // FIXME FIXME FIXME FIXME remove my_calloc /// FIXME FIXME FIXME (int *)
...
writePidInfoFile (const struct jobCard *jPtr, const struct jRusage *jRusage)
{
    ...
    if (jPtr->jobSpecs.jobFile[0] == '/') {
        sprintf (buf, "%s/.%s.pidInfo", LSTMPDIR, strrchr (jPtr->jobSpecs.jobFile, '/') + 1); // FIXME FIXME FIXME FIXME "pidInfo" should be put into configure.ac
    }
    else {
        sprintf (buf, "%s/.%s.pidInfo", LSTMPDIR, jPtr->jobSpecs.jobFile); // FIXME FIXME FIXME FIXME "pidInfo" should be put into configure.ac
    }
    ...
    for( unsigned int i = 0; i < jRusage->npids; i++) {
        fprintf (fp, "%d %d %d\n", jRusage->pidInfo[i].pid, jRusage->pidInfo[i].ppid, jRusage->pidInfo[i].pgid);
    }
...
jobFinishRusage (struct jobCard *jp)
{
    FILE *fp = NULL;
    int lineNum = 0;
    struct lsfAcctRec *rec = NULL;
    char rufn[MAX_FILENAME_LEN];
    char rufn30[MAX_FILENAME_LEN];
    char tmpDirName[MAX_FILENAME_LEN];

    memset( rufn,       '\0', MAX_FILENAME_LEN );
    memset( rufn30,     '\0', MAX_FILENAME_LEN );
    memset( tmpDirName, '\0', MAX_FILENAME_LEN );
    ...
    if ((fp = fopen (rufn, "r")) == NULL && (fp = fopen (rufn30, "r")) == NULL) {
        ; // FIXME FIXME FIXME FIXME refactor this part
    }
    ...
    if (fp == NULL) {
        ...
        if ((rec = ls_getacctrec (fp, &lineNum)) == NULL) {
            ls_syslog (LOG_ERR, I18N_FUNC_S_FAIL_M, __func__, "ls_getacctrec", rufn);
        }
        else {
            if (logclass & LC_EXEC) {
                LS_WAIT_T w_status;
                ...
                /*catgets 5495 */
                ls_syslog (LOG_DEBUG, I18N (5495, "%s: Job <%s> status <%d> exitcode <%d>"), __func__, lsb_jobid2str (jp->jobSpecs.jobId),
...
initJobCard (struct jobCard *jp, struct jobSpecs *jobSpecs, int *reply)
{
    ...
    if (jobSpecs->resumeCond && jobSpecs->resumeCond != NULL ) {
        if ((jp->resumeCondVal = checkThresholdCond (jobSpecs->resumeCond)) == NULL) {
    ...
    if (jobSpecs->stopCond && jobSpecs->stopCond != NULL ) {
        if ((jp->stopCondVal = checkThresholdCond (jobSpecs->stopCond)) == NULL) {
...
saveThresholds (struct jobSpecs *jobSpecs, struct thresholds *thresholds)
{
    ...
    jobSpecs->thresholds.loadSched = (float **) my_calloc (thresholds->nThresholds, sizeof (float *), __func__); // FIXME FIXME FIXME FIXME remove my_calloc
    jobSpecs->thresholds.loadStop  = (float **) my_calloc (thresholds->nThresholds, sizeof (float *), __func__); // FIXME FIXME FIXME FIXME remove my_calloc

    for( unsigned int i = 0; i < thresholds->nThresholds; i++) {
        jobSpecs->thresholds.loadSched[i] = (float *) my_calloc (thresholds->nIdx, sizeof (float), __func__); // FIXME FIXME FIXME FIXME remove my_calloc
        jobSpecs->thresholds.loadStop[i]  = (float *) my_calloc (thresholds->nIdx, sizeof (float), __func__); // FIXME FIXME FIXME FIXME remove my_calloc
    }

    for( unsigned int i = 0; i < thresholds->nThresholds; i++) {
        for( unsigned int j = 0; j < thresholds->nIdx; j++) {
...
getJobVersion (struct jobSpecs *jobSpecs)
{
    ...
    if (jobSpecs->numEnv <= 0) {
        return 2;  // FIXME FIXME FIXME FIXME what does 2 represent?
    }

    for( unsigned int i = 0; i < jobSpecs->numEnv; i++)  {
        if ((sp = strstr (jobSpecs->env[i], "LSF_VERSION=")) == NULL) {// FIXME FIXME FIXME FIXME "TMPDIR" should be put into configure.ac
            continue;
        }
        ...
        if (version <= 0) {
            return -1; // FIXME FIXME FIXME FIXME what does 3 represent?
        }
        return version; 
    }

    return 3; // FIXME FIXME FIXME FIXME what does 3 represent?
}
...
lockHosts (struct jobCard *jp)
{
    ...
    for (unsigned int i = 0; i < jp->jobSpecs.numToHosts; i++) {
        ...
        if (lockHost_ (0, jp->jobSpecs.toHosts[i]) < 0 && lserrno != LSE_LIM_ALOCKED) {
            ...
            unlockHosts (jp, i);
            return -1; // FIXME FIXME FIXME FIXME what does -1 represent?
        }
    }

    return 0; // FIXME FIXME FIXME FIXME what does 0 represent?
}
...
{
    FILE *fp = NULL;
    char str[256]; // FIXME FIXME FIXME FIXME why is str length only 256?
    char cmd[MAX_FILENAME_LEN + 32]; // FIXME FIXME FIXME FIXME why is str length only MAX_FILENAME_LEN + 32?

    memset( str, '\0', 256); // FIXME FIXME FIXME replace number with variable
    memset( cmd, '\0', MAX_FILENAME_LEN + 32); // FIXME FIXME FIXME replace number with variable
