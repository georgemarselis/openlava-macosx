...
sendNotification (struct jobCard *jobCardPtr)
{
  ...
  fprintf (mail, "\n");
  fprintf (mail, _i18n_msg_get (ls_catd, NL_SETN, 402, "Job <%s> was submitted from host <%s> by user <%s>. "),	/* catgets 402 */
	   jobSpecsPtr->jobName, jobSpecsPtr->fromHost,
	   jobSpecsPtr->userName);
  temp1 = putstr_ (_i18n_msg_get (ls_catd, NL_SETN, 404, "unknown time in the past"));	/* catgets 404 */
  fprintf (mail, _i18n_msg_get (ls_catd, NL_SETN, 405, "Started at %sunder user <%s>, "),	/* catgets 405 */
	   jobSpecsPtr->startTime ? _i18n_ctime (ls_catd,
						 CTIME_FORMAT_DEFAULT,
						 &jobSpecsPtr->
						 startTime) : temp1,
  ...
  now = time (0);
  fprintf (mail, _i18n_msg_get (ls_catd, NL_SETN, 408, "Notification reported at %s"),	/* catgets 408 */
...
createJobTmpDir (struct jobCard *jobCardPtr)
{
  ...
  if ((mkdir (tmpDirName, 0700) == -1) && errno != EEXIST)
    {
      ls_syslog (LOG_ERR, "\
%s: Unable to create job tmp directory %s for job %s", __func__, tmpDirName, lsb_jobid2str (jobCardPtr->jobSpecs.jobId));
      tmpDirName[0] = 0;
    }
...
execJob (struct jobCard *jobCardPtr, int chfd)
{
  ...
  if (rcvJobFile (chfd, &jf) == -1)
    {
      ls_syslog (LOG_ERR, "\
%s: failed receiving job file job %s", __func__, lsb_jobid2str (jobSpecsPtr->jobId));
...
setJobEnv (struct jobCard *jp)
{
  ...
  if (strlen (jp->jobSpecs.chkpntDir) == 0)
      ...
      if (jp->jobSpecs.jobSpoolDir[0] == '\0')
	{
	  sprintf (shellFile, "%s/.lsbatch/%s",
		   jp->jobSpecs.execHome, jp->jobSpecs.jobFile);
	}
      else
	{
	  sprintf (shellFile, "%s/%s",
		   jp->jobSpecs.jobSpoolDir, jp->jobSpecs.jobFile);
	}
    }
  else
    {
  ...
  if (strlen (jp->jobSpecs.chkpntDir) > 0)
      ...
      else
	{
	  sprintf (shellFile, "%s/%s", jp->jobSpecs.chkpntDir,
		   jp->jobSpecs.jobFile);
	  ls_syslog (LOG_ERR, I18N (430, "Failed to set LSB_CHKPNT_DIR, error in the chkpnt directory: %s"),	/* catgets 430 */
		     jp->jobSpecs.chkpntDir);
	}
...
#define MAX_LSB_HOSTS_LEN 4096
  if (jp->jobSpecs.numToHosts <= (MAX_LSB_HOSTS_LEN / 2))
    {
      hosts =
	(char *) my_malloc (jp->jobSpecs.numToHosts * MAXHOSTNAMELEN,
			    "setJobEnv");
      hosts[0] = '\0';
...

  {
    ...
    putEnv ("LSB_MCPU_HOSTS", lsb_printNameList (hostList, PRINT_MCPU_HOSTS));
  }
	  ...
	  if (envpath != NULL)
	    {
	      ...
	      cc =
		strncmp (envpath, daemonParams[LSF_BINDIR].paramValue, len);
	      if (cc != 0)
  ...
  if (runQPost (jobCard) != 0)
    {
      hasError = -1;
      ls_syslog (LOG_ERR, "\
%s: failed to run queue post for job %s", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId));
    }
...
status_report (void)
{
  ...
  for (jp = jobQueHead->back; (jp != jobQueHead); jp = next)
	...
	ls_syslog (LOG_DEBUG3,
		   "status_report: checking job %s notReproted=%d missing=%d needReportRU=%d now=%d startTime=%d status=%x",
		   lsb_jobid2str (jp->jobSpecs.jobId), jp->notReported,
		   jp->missing, jp->needReportRU, now, jp->jobSpecs.startTime,
		   jp->jobSpecs.jStatus);
...
send_results (struct jobCard *jp)
{
  ...
  fprintf (notif, _i18n_msg_get (ls_catd, NL_SETN, 426, "Started at %s"), sp);	/* catgets 426 */
  free (sp);
  now = time (0);
  fprintf (notif, _i18n_msg_get (ls_catd, NL_SETN, 487, "Results reported at %s"),	/* catgets 487 */
	   _i18n_ctime (ls_catd, CTIME_FORMAT_DEFAULT, &now));
  ...
  if ((fp = myfopen_ (fileName, "r", hp)) == NULL)
      ...
      else
	{
	  ...
	  fputs (_i18n_printf ("\n%s\n", _i18n_msg_get (ls_catd, NL_SETN, 429, "Your job looked like:")),	/* catgets 429 */
		 notif);
	  fputs
	    ("\n------------------------------------------------------------\n",
	     notif);
	  ...
	  while (fgets (line, MAXLINELEN, fp) != NULL)
	      ...
	      else
		{
		  ...
		  if ((k == 1)
		    ...
		    {
		      char *pSep = NULL;
		      if ((pSep = strchr (jp->jobSpecs.command, ';')) == NULL)
			{
...
Read file <%s> for stdout output of this job.\n", jp->jobSpecs.outFile);
  ...
  if (!(jp->jobSpecs.options & SUB_ERR_FILE))
  ...
  else if (strcmp (jp->jobSpecs.errFile, "/dev/null"))
    {
      ...
      sprintf (fileName, "%s.err", jp->jobSpecs.jobFile);
      if (copyStderrFromLsbatch
	  && (fp = myfopen_ (fileName, "r", hp)) == NULL)
	{
	...
	{
	  if (errFileError)
	    fprintf (errout, "\n\n%s\n\n", _i18n_msg_get (ls_catd, NL_SETN, 449, "PS: The stderr output (if any) follows:"));	/* catgets 449 */
	  else
	    {
  ...
  for (i = 0; i < jp->jobSpecs.nxf; i++)
      ...
      if (jp->jobSpecs.xf[i].options & XF_OP_EXEC2SUB)
	{
	  ...
	  sprintf (line, _i18n_msg_get (ls_catd, NL_SETN, 452, "Unable to copy file <%s> to <%s> on submission host <%s>: %s.\n"),	/* catgets 452 */
		   jp->jobSpecs.xf[i].execFn,
		   jp->jobSpecs.xf[i].subFn, jp->jobSpecs.fromHost, rcpMsg);
	  hasError = -1;
	  strcat (ps, line);
	  withps = TRUE;
	}
  ...
  if (!errorOpeningOutputFile && (output == notif) && ferror (output))
    {
      ...
      sprintf (line,
	       ...
	       jp->jobSpecs.outFile,
	       lsb_jobid2str (jp->jobSpecs.jobId), strerror (errno));

      if (jp->jobSpecs.options & SUB_MAIL_USER)
	{
	  merr_user (jp->jobSpecs.mailUser,
		     jp->jobSpecs.fromHost, line, I18N_Warning);
	}
      else
	{
	  merr_user (jp->jobSpecs.userName,
  ...
  if (ofIdx >= 0)
		   ...
		   jp->jobSpecs.fromHost, XF_OP_EXEC2SUB, rcpMsg) < 0)
	{
	  sprintf (ps, _i18n_msg_get (ls_catd, NL_SETN, 454, "We are unable to copy your output file <%s> to <%s> on submission host <%s> for job <%s>: %s.\n\n"),	/* catgets 454 */
		   ...
		   jp->jobSpecs.fromHost,
		   lsb_jobid2str (jp->jobSpecs.jobId), rcpMsg);
	  hasError = -1;

	  if (jp->jobSpecs.options & SUB_MAIL_USER)
	    merr_user (jp->jobSpecs.mailUser,
		       jp->jobSpecs.fromHost, ps, I18N_Warning);
	  else
	    merr_user (jp->jobSpecs.userName,
		       jp->jobSpecs.fromHost, ps, I18N_Warning);
	}
...
sbdStartupStopJob (struct jobCard *jp, int reasons, int subReasons)
{
  ...
  return (jRunSuspendAct (jp, sigValue,
			  ((reasons & SUSP_USER_STOP) ? JOB_STAT_USUSP :
			   JOB_STAT_SSUSP), reasons, subReasons, NO_SIGLOG));
		  ...
		  while (memp != NULL && *memp != NULL)
		      ...
		      if (strcmp (*memp, jobCardPtr->execUsername) == 0)
			{
			  ...
			  if (setgid (grEntry->gr_gid) < 0)
			    {
			      ls_syslog (LOG_DEBUG,
					 "setIds: Job <%s> setgid(%d) for LSB_UNIXGROUP=%s  failed: %m",
					 lsb_jobid2str (jobSpecsPtr->jobId),
...
deallocJobCard (struct jobCard *jobCard)
{
  ...
  if (logclass & LC_MPI)
    {
      ls_syslog (LOG_DEBUG3,
		 "deallocJobCard: unlink(%s) for MPI job <%s>: %m", fileBuf,
		 lsb_jobid2str (jobCard->jobSpecs.jobId));
    }
  if (unlink (fileBuf) < 0 && errno != ENOENT)
    ls_syslog (LOG_ERR, I18N_JOB_FAIL_S_S_M, fname,
...
trySubUser:
  ...
  if ((pw =
       my_getpwnam (jobCard->jobSpecs.userName,
		    "acctMapTo/trySubUser")) == NULL)
    {
...
acctMapOk (struct jobCard *jobCard)
{
  ...
  if ((pw = getpwuid (jobCard->jobSpecs.execUid)) == NULL)
    {
      sprintf (errMsg, "\
%s: Job %s getpwuid %d failed: %s", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), jobCard->jobSpecs.execUid, strerror (errno));
  ...
  if ((fp = fopen (hostfn, "r")) == NULL)
    {
      sprintf (errMsg, "\
%s: Job %s fopen() for file %s failed: %s", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), hostfn, strerror (errno));
  ...
  if ((fstat (fileno (fp), &statbuf) < 0) ||
    ...
    {
      sprintf (errMsg, "\
%s: Job %s file %s is not owned or writable only by the user, file uid %d, file mode %03o, userId %d", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), hostfn, (int) statbuf.st_uid, (unsigned int) statbuf.st_mode, (int) pw->pw_uid);
      sbdSyslog (LOG_ERR, errMsg);
      FCLOSEUP (&fp);
      goto error;
    }
  ...
  while ((line = getNextLine_ (fp, TRUE)) != NULL)
    {
      ...
      if (num < 2
	  || (num == 3
	      && (strcmp (dir, "send") == 0 || strcmp (dir, "recv") != 0)))
	continue;
  ...
  sprintf (errMsg, "\
%s: Job %s no authorization for user name %s from cluster/host %s/%s in file %s", __func__, lsb_jobid2str (jobCard->jobSpecs.jobId), jobCard->jobSpecs.userName, clusterName, jobCard->jobSpecs.fromHost, hostfn);
  sbdSyslog (LOG_ERR, errMsg);
  ...
  if (logclass & LC_TRACE)
    {
      chuser (batchId);
      ls_syslog (LOG_DEBUG, "\
%s: queue's post-command %s pre-command %s execJobFlag %x for job %d status %x", __func__, jp->jobSpecs.postCmd ? jp->jobSpecs.postCmd : "NULL", jp->jobSpecs.preCmd ? jp->jobSpecs.preCmd : "NULL", jp->execJobFlag, lsb_jobid2str (jp->jobSpecs.jobId), jp->w_status);
  ...
  if (jp->jobSpecs.preCmd && jp->jobSpecs.preCmd[0] != '\0')
      ...
      if (!(jp->execJobFlag & JOB_EXEC_QPRE_KNOWN))
	{
	  ...
	  chuser (batchId);
	  ls_syslog (LOG_WARNING, "\
%s: qpost is not run for job %d because status of qpre unknown", __func__, jp->jobSpecs.jobId);
	  return -1;
	}
      ...
      if (!(jp->execJobFlag & JOB_EXEC_QPRE_OK))
	{
	  ...
	  chuser (batchId);
	  ls_syslog (LOG_WARNING, "\
%s: qpost is not run for job % because qpre failed", __func__, lsb_jobid2str (jp->jobSpecs.jobId), chuser (jp->jobSpecs.execUid));
	  return -1;
	}
    }
  ...
  if ((pid = fork ()) == -1)
    {
      chuser (batchId);
      ls_syslog (LOG_ERR, "\
%s: fork() failed for job %d: %m", __func__, jp->jobSpecs.jobId);
      chuser (jp->jobSpecs.execUid);
      return -1;
    }
  ...
  if (pid)
      ...
      if (waitpid (pid, NULL, 0) < 0)
	{
	  chuser (batchId);
	  ls_syslog (LOG_ERR, "\
%s: waitpid(%d) failed for qpre for job <%d>: %m", __func__, pid, jp->jobSpecs.jobId);
	  chuser (jp->jobSpecs.execUid);
	}
      return -1;
    }
  ...
  if (chPrePostUser (jp) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: queue's post-exec chPrePostUser failed for job <%d>", __func__, jp->jobSpecs.jobId);
      exit (-1);
    }
  ...
  sprintf (val, "\
%s: queue's post-exec command %s failed for job %s: %s", __func__, jp->jobSpecs.postCmd, lsb_jobid2str (jp->jobSpecs.jobId), strerror (errno));
...
chPrePostUser (struct jobCard *jp)
{
  ...
  if (initgroups (jp->execUsername, jp->execGid) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: initgroups() failed for user %s uid %d gid %d %m", __func__, jp->execUsername, jp->jobSpecs.execUid, jp->execGid);
    }

  if (setgid (jp->execGid) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: setgid() failed for user %s uid %d gid %d %m", __func__, jp->execUsername, jp->jobSpecs.execUid, jp->execGid);
    }
  ...
  if (lsfSetUid (prepostUid) < 0)
    {
      ls_syslog (LOG_ERR, "\
%s: lsfSetUid() failed for uid %d gid %d %m", __func__, prepostUid, jp->execGid);
      return -1;
    }
...
postJobSetup (struct jobCard *jp)
{
  ...
  if ((hp = Gethostbyname_ (jp->jobSpecs.fromHost)) == NULL)
    ls_syslog (LOG_ERR, "\
%s: Gethostbyname_() failed %s job %s", __func__, jp->jobSpecs.fromHost, lsb_jobid2str (jp->jobSpecs.jobId));
  ...
  if (jRusage->npgids > 0)
      ...
      if (jPtr->runRusage.pgid == NULL)
	{
      ...
      memcpy (jPtr->runRusage.pgid,
	      jRusage->pgid, (jRusage->npgids) * (sizeof (int)));
    }
...
getJobVersion (struct jobSpecs *jobSpecs)
{
  ...
  for (i = 0; i < jobSpecs->numEnv; i++)
	...
	continue;
      ...
      if (logclass & LC_EXEC)
	ls_syslog (LOG_DEBUG3,
		   "getJobVersion: LSF_VERSION of job <%s> is <%d>",
		   lsb_jobid2str (jobSpecs->jobId), version);
      if (version <= 0)
	return (-1);
      return (version);
    }
