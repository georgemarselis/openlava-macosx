...
shout_err (struct jobCard *jobPtr, char *msg)
{
    ...
    if (jobPtr->jobSpecs.options & SUB_MAIL_USER) {
        merr_user (jobPtr->jobSpecs.mailUser, jobPtr->jobSpecs.fromHost, buf, I18N_error);
    }
    else {
    ...
    while( ( pid = waitpid( -1, &status, WNOHANG ) ) > 0)  { // while ((pid = wait3 (&status, WNOHANG, &rusage)) > 0)  {
        ...
        if (pid == mbdPid) {
            ...
            if (mbdExitVal) {
                ...
                if (WCOREDUMP (status)) {
                    /* catgets 5601 */
                    ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5601, "mbatchd core dumped")); 
                }
                ...
                if (mbdExitVal == MASTER_RECONFIG) {
                    /* catgets 5602 */
                    ls_syslog (LOG_NOTICE, _i18n_msg_get (ls_catd, NL_SETN, 5602, "mbatchd resigned for reconfiguration"));
                    start_master ();
                }
                else {
                    /* catgets 5603 */
        ...
        for (jobCard = jobQueHead->forw; (jobCard != jobQueHead); jobCard = jobCard->forw) {
            ...
            if (jobCard->exitPid == pid) {
                ...
                if (logclass & LC_EXEC) {
                    /*catgets 5604 */
                    ls_syslog (LOG_DEBUG, I18N (5604, "child_handler: Job <%s> exitPid <%d> status <%d> exitcode <%d>"), lsb_jobid2str (jobCard->jobSpecs.jobId), pid, jobCard->w_status, WEXITSTATUS (status));
                }
            }
            ...
            if (jobCard->jobSpecs.jobPid == pid) {
                ...
                jobCard->w_status = LS_STATUS (status);
                jobCard->exitPid = -1;
                memcpy ((char *) &jobCard->lsfRusage, (char *) &lsfRusage, sizeof (struct lsfRusage));
                jobCard->notReported++;
                ...
                if (logclass & LC_EXEC) {
                    /*catgets 5605 */
                    ls_syslog (LOG_DEBUG, I18N (5605, "child_handler: Job <%s> Pid <%d> status <%d> exitcode <%d>"), lsb_jobid2str (jobCard->jobSpecs.jobId), pid, jobCard->w_status, WEXITSTATUS (status));
                }
...
fcp (char *file1, char *file2, struct hostent *hp)
{
    ...
    buf = malloc( BSIZE * sizeof( char )  + 1 );
    ...
    for (;;) { // FIXME FIXME FIXME FIXME FIXME remove infiinite loop
...
getManagerId (struct sbdPackage *sbdPackage)
{
    ...
    for ( int i = 0; i < sbdPackage->nAdmins; i++) {
