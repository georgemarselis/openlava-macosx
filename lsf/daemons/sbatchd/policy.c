...
job_checking (void)
{
  ...
  for (jobCard = jobQueHead->forw; (jobCard != jobQueHead); jobCard = nextJob)
    {
	  ...
	  && !(jobCard->jobSpecs.jAttrib & JOB_URGENT_NOSTOP))
	{
	  if (!(jobCard->jobSpecs.options & SUB_WINDOW_SIG)
		  ...
		  && now - jobCard->windWarnTime >= WARN_TIME))
	    {
	      ...
	      jobSuspendAction (jobCard, SIG_SUSP_WINDOW, SUSP_QUEUE_WINDOW,
				0);
	      continue;
	  ...
	  if (shouldResume (myload, jobCard, 1))
	    {
	      ...
	      if (jobResumeAction (jobCard, SIG_RESUME_LOAD, LOAD_REASONS) <
		  0)
		continue;
	      else
		return;
  ...
  for (jobCard = jobQueHead->forw; jobCard != jobQueHead; jobCard = next)
      ...
      if (jobCard->jobSpecs.numToHosts == 1)
	{
	  ...
	  if ((jobCard->jobSpecs.jStatus & JOB_STAT_RUN)
	      ...
	      && shouldStop (myload, jobCard, &reasons, &subreasons, 1,
			     &stopmore))
	    {
      ...
      else
	{
	  ...
	  if ((jobCard->jobSpecs.jStatus & JOB_STAT_RUN)
	    ...
	    {
	      if (shouldStop
		  (load, jobCard, &reasons, &subreasons, numh, &stopmore))
		{

		  jobSuspendAction (jobCard, SIG_SUSP_LOAD, reasons,
				    subreasons);
		  if (stopmore)
		    break;
		  else
  ...
  for (i = 0; i < jobCard->jobSpecs.numToHosts && (*reasons) == 0; i++)
      ...
      else if (LS_ISLOCKEDM (load->status))
	{
	  ...
	  *stopmore = TRUE;
	       ...
	       jobCard->jobSpecs.thresholds.loadStop[numLoad][IT]
  ...
  if (numHosts >= 0)
    {
      ...
      resume = checkResumeByLoad (jp->jobSpecs.jobId, numHosts,
				  jp->jobSpecs.thresholds, loads,
				  &jp->jobSpecs.reasons,
				  &jp->jobSpecs.subreasons,
				  jp->jobSpecs.jAttrib, jp->resumeCondVal,
				  tclHostData);
...
job_resume (struct jobCard *jp)
{
  ...
  rep = status_job (BATCH_STATUS_JOB, jp, jp->jobSpecs.jStatus, ERR_NO_ERROR);
  if (rep < 0)
    jp->notReported++;
  else
    {
...
sigActEnd (struct jobCard *jobCard)
{
  ...
  switch (jobCard->jobSpecs.actValue)
    ...
    case SIG_TERM_MEMLIMIT:
      ...
      break;
    default:
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5708, "sigActEnd: unknown sigValue <%d> for job <%s> at the job status <%d> with actPid <%d>"),	/* catgets 5708 */
		 jobCard->jobSpecs.actValue,
		 lsb_jobid2str (jobCard->jobSpecs.jobId),
		 jobCard->jobSpecs.jStatus, jobCard->jobSpecs.actPid);
...
chkpntEnd (struct jobCard *jobCard, int w_status, bool_t * freed)
{
  ...
  if (status_job (BATCH_STATUS_JOB, jobCard, jobCard->jobSpecs.jStatus,
		  w_status == 0 ? ERR_NO_ERROR : ERR_SYSACT_FAIL) < 0)
    {
...
suspendActEnd (struct jobCard *jobCard, int w_status)
{
  ...
  if (logclass & (LC_TRACE | LC_SIGNAL))
    ls_syslog (LOG_DEBUG,
	       "suspendActEnd: Suspend job %s; reasons=%x, subresons=%d",
	       lsb_jobid2str (jobCard->jobSpecs.jobId), jobCard->actReasons,
	       jobCard->actReasons);
...
ruLimits (struct jobCard *jobCard)
{
  ...
  if ((lsbJobMemLimit == 1) || (lsbJobMemLimit != 0 && lsbMemEnforce == TRUE))
    {
