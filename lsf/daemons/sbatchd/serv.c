...
do_sigjob (XDR * xdrs, int chfd, struct LSFHeader *reqHdr)
{
  ...
  if (!JOB_STARTED (jp))
	...
	{
	  if ((cc =
	       jobSigStart (jp, sigValue, jobSig.actFlags, jobSig.chkPeriod,
			    NO_SIGLOG)) < 0)
	    reply = ERR_SIG_RETRY;
	  else
	    reply = ERR_NO_ERROR;
  ...
  if ((cc = jobSigStart (jp,
			 sigValue,
			 jobSig.actFlags, jobSig.chkPeriod, NO_SIGLOG)) < 0)
    reply = ERR_SIG_RETRY;
  else
    reply = ERR_NO_ERROR;
...
do_reboot (XDR * xdrs, int chfd, struct LSFHeader *reqHdr)
{
  ...
  ls_syslog (LOG_NOTICE, _i18n_msg_get (ls_catd, NL_SETN, 5828, "Slave batch daemon reboot command received"));	/* catgets 5828 */
  relife ();
  ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5829, "Unable to relife during rebooting: %m"));	/* catgets 5829 */
  xdr_destroy (&xdrs2);
...
do_shutdown (XDR * xdrs, int chfd, struct LSFHeader *reqHdr)
{
  ...
  xdr_destroy (&xdrs2);
  ls_syslog (LOG_NOTICE, _i18n_msg_get (ls_catd, NL_SETN, 5836, "Slave batch daemon shutdown command received"));	/* catgets 5836 */
  die (SLAVE_SHUTDOWN);
