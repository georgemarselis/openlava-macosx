...
// #define MAX_ARGS        16
const unsigned int MAX_ARGS = 16;
...
int
jobSigStart (struct jobCard *jp, int sigValue, int actFlags, int actPeriod, logType logFlag)
{
	...
	if (logclass & (LC_TRACE | LC_SIGNAL)) {
		ls_syslog (LOG_DEBUG, "%s: Signal %s to job %s with actFlags=%d; reasons=%x, subresons=%d, sigValue=%d", __func__, getLsbSigSymbol (sig_decode (sigValue)), lsb_jobid2str (jp->jobSpecs.jobId), actFlags, jp->jobSpecs.reasons, jp->jobSpecs.subreasons, sigValue);
	}

	defSigValue = sigValue;

	if (!(jp->jobSpecs.jStatus & JOB_STAT_ZOMBIE) && IS_FINISH (jp->jobSpecs.jStatus)) {
    	return 0;
	}
  ...
  if (!JOB_STARTED (jp))
	...
	case SIG_TERM_LOAD:
	  return (terminateAct
		  (jp, sigValue, jp->actReasons, jp->actSubReasons, logFlag));
	...
	case SIG_TERM_RUNLIMIT:
	  return (terminateAct
		  (jp, sigValue, SUSP_RES_LIMIT, SUB_REASON_RUNLIMIT,
		   logFlag));
	case SIG_TERM_DEADLINE:
	  return (terminateAct
		  (jp, sigValue, SUSP_RES_LIMIT, SUB_REASON_DEADLINE,
		   logFlag));
	case SIG_TERM_PROCESSLIMIT:
	  return (terminateAct
		  (jp, sigValue, SUSP_RES_LIMIT, SUB_REASON_PROCESSLIMIT,
		   logFlag));
	case SIG_TERM_CPULIMIT:
	  return (terminateAct
		  (jp, sigValue, SUSP_RES_LIMIT, SUB_REASON_CPULIMIT,
		   logFlag));
	case SIG_TERM_MEMLIMIT:
	  return (terminateAct
		  (jp, sigValue, SUSP_RES_LIMIT, SUB_REASON_MEMLIMIT,
		   logFlag));
	default:
	  return -1;
	}
  ...
  if ((jp->jobSpecs.jStatus & JOB_STAT_RUN) && jp->postJobStarted)
    {
  ...
  if (jp->jobSpecs.jStatus & JOB_STAT_RUN)
	...
	case SIG_SUSP_USER:
	  return (jRunSuspendAct (jp, sigValue,
				  JOB_STAT_USUSP, SUSP_USER_STOP, 0,
				  logFlag));
	case SIG_SUSP_LOAD:
	  return (jRunSuspendAct (jp, sigValue,
				  JOB_STAT_SSUSP, jp->actReasons,
				  jp->actSubReasons, logFlag));
	case SIG_SUSP_WINDOW:
	  return (jRunSuspendAct (jp, sigValue,
				  JOB_STAT_SSUSP, SUSP_QUEUE_WINDOW, 0,
				  logFlag));
	case SIG_SUSP_OTHER:
	  return (jRunSuspendAct (jp, sigValue,
				  JOB_STAT_SSUSP, jp->actReasons,
		...
		logType logFlag)
{
  ...
  if (logclass & (LC_TRACE | LC_SIGNAL))
    ls_syslog (LOG_DEBUG1,
	       "jSuspResumeAct: job %s jobSpecs.reasons=%x sigValue=%d suspendReasons=%x",
  ...
  if (jp->jobSpecs.reasons & SUSP_MBD_LOCK)
	...
	{
	  if (jp->
	      actReasons & ~(SUSP_MBD_LOCK | SUSP_USER_STOP | SUSP_USER_RESUME
			     | SUSP_SBD_STARTUP))
	    {

	      jp->jobSpecs.reasons =
		jp->actReasons & ~(SUSP_USER_STOP | SUSP_USER_RESUME);
...
mykillpg (struct jobCard *jp, int sig)
{
  ...
  if (jp->regOpFlag & REG_RUSAGE)
    ...
    {
      TIMEIT (0, jru =
	      getJInfo_ (npgid, pgid, 0, jp->jobSpecs.jobPGid),
...
mykillpg: job file name = %s\n", jobFileName);
  ...
  if (jru)
      ...
      if (fileStatus == 0)
	{
	  ...
	  if (status == 0)
	      ...
	      if (logclass & LC_MPI)
		{
		  ...
		  ls_syslog (LOG_DEBUG3, "mykillpg: utime  = <%d>",
			     jru->utime);
		  ls_syslog (LOG_DEBUG3, "mykillpg: stime  = <%d>",
			     jru->stime);
		}
	    }
	}
	...
	{
	  ls_syslog (LOG_DEBUG,
		     "mykillpg(): Job <%s> pgid %d mem %d swap %d utime %d stime %d rusageUpdatePercent %d current mem %d swap %d rusageUpdateRate %d changed %d needReportRU %d lastStatusMbdTime %d sbdSleepTime %d",
	...
	int forkSig)
{
  ...
  sprintf (exitFile, "%s/.%s.%s.%s", LSTMPDIR,
	   jp->jobSpecs.jobFile, lsb_jobidinstr (jp->jobSpecs.jobId),
	   exitFileSuffix (actValue));
...
Error:
  ...
  sprintf (msg, "\
We are unable to take the action %s on job %s <%s>.\nThe error is: %s", actCmd, lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.command, errMsg);
...
execRestart (struct jobCard *jobCardPtr, struct hostent *hp)
{
  ...
  if (pid > 0)
    ...
    again:
      while (((i = read (fds[0], sp, MSGSIZE)) < 0) && (errno == EINTR));
	...
	{
	  sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 908, "Unable to rename chkpnt directory %s to %s: %s."),	/* catgets 908 */
  ...
  if ((erestartPath = getEchkpntDir ("/erestart")) == NULL)
    {
      sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 909, "Unable to find the erestart executable"));	/* catgets 909 */
      exit (-1);
    }
...
exeChkpnt (struct jobCard *jp, int chkFlags, char *exitFile)
{
  ...
  if (jp->regOpFlag & REG_CHKPNT)
      ...
      if (!(jp->jobSpecs.options & SUB_CHKPNTABLE))
	{
	  if (jobsig (jp, SIGKILL, FALSE) == 0)
	    {
		  ...
		  || FCLOSEUP (&fp) == -1)
		{
		  sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 910, "Unable to write migration status into %s file: %s"),	/* catgets 910 */
	  ...
	  sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 911, "Unable to send signal to job: %s"),	/* catgets 911 */
		   strerror (errno));
	  goto Error;
	}
	...
	{
	  sprintf (errMsg, "\
%s: gethostbyname() %s failed job %s", __func__, lsb_jobid2str (jp->jobSpecs.jobId), jp->jobSpecs.fromHost);
	  goto Error;
	}
      ...
      while ((i = read (fds[0], sp, len)) > 0)
	{
      ...
      *sp = '\0';
      ...
      if ((strlen (errMsg) > 0)
	  && (strstr (errMsg, "Checkpoint done") == NULL))
	{
	  goto ChildError;
	}
...
Error:
  ...
  sprintf (msg, "\
We are unable to checkpoint your job %s <%s>.\nThe error is: %s", lsb_jobid2str (jp->jobSpecs.jobId), jp->jobSpecs.command, errMsg);


  if (jp->jobSpecs.options & SUB_MAIL_USER)
    merr_user (jp->jobSpecs.mailUser, jp->jobSpecs.fromHost, msg, I18N_error);
  else
    merr_user (jp->jobSpecs.userName, jp->jobSpecs.fromHost, msg, I18N_error);
...
getEchkpntDir (char *name)
{
  ...
  if ((echkpnt_dir = getenv ("LSF_ECHKPNTDIR")) == NULL)
    if ((echkpnt_dir = daemonParams[LSF_SERVERDIR].paramValue) == NULL)
      {
	perror (_i18n_msg_get (ls_catd, NL_SETN, 927, "Can't access echkpnt()"));	/* catgets 927 */
	return NULL;
      }
  ...
  if (access (echpnt_path, X_OK) < 0)
    {
      perror (_i18n_msg_get (ls_catd, NL_SETN, 928, "Can't access echkpnt()/erestart()"));	/* catgets 928 */
      return NULL;
    }
  return echpnt_path;
...
sbdlog_newstatus (struct jobCard *jp)
{
  ...
  sprintf (logFile, "%s/.%s.sbd/jobstatus.%s", LSTMPDIR, clusterName,
...
sbdread_jobstatus (struct jobCard *jp)
{
  ...
  if ((fp = fopen (logFile, "r")) == NULL)
    {
      ...
      sprintf (logFile, "%s/.%s.sbd/jobstatus.%d", LSTMPDIR, clusterName,
	       jobid32);
      if ((fp = fopen (logFile, "r")) == NULL)
	{
	  return -1;
  ...
  job->jobId = LSB_JOBID (jobStatusLog->jobId, jobStatusLog->idx);
