...
// static int unlinkBufFiles (char *, char *, struct jobCard *,
//             struct hostent *);
...
//              char *ext, struct jobSpecs *jobSpecsPtr);

int
rcpFile (struct jobSpecs *jp, struct xFile *xf, const char *host, int op, char *errMsg)
{
    ...
    if (pipe (p) < 0)
        {
            sprintf (errMsg, "pipe(): %s", strerror (errno));
            return -1;
        }
    ...
    if (pid == 0)
        {
            ...
            for (unsigned int i = 3; i < maxfds; i++)
    close (i);
    ...
    sprintf (rcpCmd, "lsrcp");
            ...
            if (logclass & LC_FILE)
    {
        sprintf (debugMsg, "rcpFile: Job %s rcpCmd <%s> subFn <%s> execFn <%s> rcpArg <%s> options %x XF_OP_SUB2EXEC %x XF_OP_EXEC2SUB_APPEND %x op %x", lsb_jobidinstr (jp->jobId), rcpCmd, xf->subFn, xf->execFn, rcpArg, xf->options, XF_OP_SUB2EXEC, XF_OP_EXEC2SUB_APPEND, op);
    ...
    if (LS_STATUS (status))
        {
            if (errMsg[0] == '\0')
    sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 305, "lsrcp exited with non-zero status %x"), /* catgets 305 */
         LS_STATUS (status));
            return -1;
        }
...
rmJobBufFiles (struct jobCard *jp)
{
    ...
    if (logclass & LC_EXEC)
        {
            sprintf (errMsg, "rmJobBufFiles: Enter ... job <%s> jobfile <%s>", lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.jobFile);
    ...
    if ((hp = Gethostbyname_ (jp->jobSpecs.fromHost)) == NULL)
        {
            sprintf (errMsg, "%s: gethostbyname() %s failed %s", __func__, jp->jobSpecs.fromHost, lsb_jobid2str (jp->jobSpecs.jobId));
            sbdSyslog (LOG_ERR, errMsg);
            return -1;
        }


    if ((jp->jobSpecs.options2 & SUB2_JOB_CMD_SPOOL)  && (jp->spooledExec != NULL))
        {

            for (p = jp->spooledExec + strlen (jp->spooledExec) - 1; (*p != '/') && (p > jp->spooledExec); p--) { ; }
            *p = '\0';
            ...
            if (retVal < 0)
    {
        ls_syslog (LOG_ERR, I18N (5300, "Can not remove spooled executable directory <%s>\n please remove it manually"), jp->spooledExec);  /* catgets 5300 */
    }
...
int
unlinkBufFiles ( const char *lsbDir, const char *jobFile, struct jobCard *jp, struct hostent *hp)
{
    ...
    if (logclass & LC_EXEC)
        {
            sprintf (errMsg, "%s: Enter ... job <%s> lsbDir <%s> jobFile <%s>", __func__, lsb_jobidinstr (jp->jobSpecs.jobId), lsbDir, jobFile);
            sbdSyslog (LOG_DEBUG, errMsg);
        }
    ...
    if (errCode < 0 && errno != ENOENT)
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "myunlink", fileBuf);
            sbdSyslog (LOG_ERR, errMsg);
            error = TRUE;
        }
    else if (errCode == 1)
    ...
    if (!(jp->jobSpecs.jAttrib & JOB_SAVE_OUTPUT))
        {
            sprintf (fileBuf, "%s.out", jfPath);  // FIXME FIXME FIXME FIXME add to configure.ac
    ...
    sprintf (fileBuf, "%s.err", jfPath);  // FIXME FIXME FIXME FIXME add to configure.ac
    if (myunlink_ (fileBuf, hp, doMount) < 0 && errno != ENOENT)
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "myunlink", fileBuf);
            sbdSyslog (LOG_ERR, errMsg);
    ...
    sprintf (fileBuf, "%s.in", jfPath);  // FIXME FIXME FIXME FIXME add to configure.ac
    if (myunlink_ (fileBuf, hp, doMount) < 0 && errno != ENOENT)
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "myunlink", fileBuf);
            sbdSyslog (LOG_ERR, errMsg);
    ...
    sprintf (fileBuf, "%s.tmp", jfPath);  // FIXME FIXME FIXME FIXME add to configure.ac
    if (myunlink_ (fileBuf, hp, doMount) < 0 && errno != ENOENT)
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "myunlink", fileBuf);
            sbdSyslog (LOG_ERR, errMsg);
    ...
    sprintf (fileBuf, "%s.shell", jfPath);  // FIXME FIXME FIXME FIXME add to configure.ac
    if (myunlink_ (fileBuf, hp, doMount) < 0 && errno != ENOENT)
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "myunlink", fileBuf);
            sbdSyslog (LOG_ERR, errMsg);
...
initPaths (struct jobCard *jp, struct hostent *fromHp, struct lenData *jf)
{
            ...
            if (!isAbsolutePathSub (jp, jp->jobSpecs.xf[i].subFn))
    {
        if (isAbsolutePathSub (jp, jp->jobSpecs.cwd)) {
            sprintf (fileBuf, "%s/%s", jp->jobSpecs.cwd, jp->jobSpecs.xf[i].subFn);
        }
        else {
            sprintf (fileBuf, "%s/%s/%s", jp->jobSpecs.subHomeDir, jp->jobSpecs.cwd, jp->jobSpecs.xf[i].subFn);
        }
    ...
    if (jp->jobSpecs.execCwd[0] == '\0')
        {
            if (getenv ("WINDIR") != NULL || getenv ("windir") != NULL)  // FIXME FIXME FIXME FIXME add to configure.ac
    {
        if ((sp = getenv ("HOMESHARE")) != NULL)  // FIXME FIXME FIXME FIXME add to configure.ac
            {
                i = strlen (sp);
            ...
            if (cwdJob (jp, cwd, fromHp) == -1)
    {
        ...
        return -1;
            ...
            putEnv ("PWD", cwd);  // FIXME FIXME FIXME FIXME add to configure.ac
        }
    else
        {
            ...
            if (chdir (jp->jobSpecs.execCwd) < 0)
    {
        ...
        sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 314, "%s: Job <%s> chdir(%s) failed, errno=<%s>. Use <%s> as execCwd"),  __func__, lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.execCwd, strerror (errno), LSTMPDIR); /* catgets 314 */
        sbdSyslog (LOG_INFO, errMsg);
        chdir (LSTMPDIR);
        strcpy (jp->jobSpecs.execCwd, LSTMPDIR);
    }
            putEnv ("PWD", jp->jobSpecs.execCwd); // FIXME FIXME FIXME FIXME add to configure.ac
        }
            ...
            if (!isAbsolutePathExec (jp->jobSpecs.jobFile))
    {
        if (goodSpoolDir)
            {
                sprintf (fileBuf, "%s/%s%s", jp->jobSpecs.jobSpoolDir, jp->jobSpecs.jobFile, JOBFILEEXT);
                if (access (fileBuf, R_OK) != 0)
        {
            struct stat st;
            if (stat (fileBuf, &st) < 0)

                sprintf (fileBuf, "%s/.lsbatch/%s", jp->jobSpecs.execHome, jp->jobSpecs.jobFile);  // FIXME FIXME FIXME FIXME add to configure.ac
            else
                sprintf (fileBuf, "%s/%s", jp->jobSpecs.jobSpoolDir, jp->jobSpecs.jobFile);
        }
                else
        ...
        if (logclass & LC_EXEC)
            {
                sprintf (errMsg, "%s: job <%s> jp->jobSpecs.jobFile is %s goodspooldir is %d", __func__, lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.jobFile, goodSpoolDir);
                sbdSyslog (LOG_DEBUG, errMsg);
            }

    }

            putEnv ("HOME", jp->jobSpecs.execHome);  // FIXME FIXME FIXME FIXME add to configure.ac
            putEnv ("LSB_JOBFILENAME", jp->jobSpecs.jobFile);  // FIXME FIXME FIXME FIXME add to configure.ac
            ...
            else
    {
        ...
        chkDirEnv = getenv ("LSB_CHKPNT_DIR");  // FIXME FIXME FIXME FIXME add to configure.ac
        if (chkDirEnv != NULL)
            {
            ...
            putEnv ("LSB_CHKFILENAME", shellFile);  // FIXME FIXME FIXME FIXME add to configure.ac
            return 0;
        }

    if (UID_MAPPED (jp) || !isAbsolutePathSub (jp, jp->jobSpecs.subHomeDir) || (jp->jobSpecs.options & SUB_LOGIN_SHELL))
        {
    ...
    if (logclass & LC_EXEC)
        {
            if (jp->jobSpecs.jobSpoolDir[0] == '\0')
    {
        sprintf (errMsg, "%s: job <%s>, lsbDir is %s, jp->jobSpecs.jobFile is <%s>, goodspooldir %d", __func__, lsb_jobidinstr (jp->jobSpecs.jobId), lsbDir, jp->jobSpecs.jobFile, goodSpoolDir);
        sbdSyslog (LOG_DEBUG, errMsg);
    }
            else
    {
        sprintf (errMsg, "%s: job <%s>, spooldir is %s, lsbDir is %s, jp->jobSpecs.jobFile is %s, goodSpoolDir %d", __func__, lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.jobSpoolDir, lsbDir, jp->jobSpecs.jobFile, goodSpoolDir);
        sbdSyslog (LOG_DEBUG, errMsg);
    }
        }
...
cwdJob (struct jobCard *jp, char *cwd, struct hostent *fromHp)
{
            ...
            if (mychdir_ (cwd, fromHp) == 0)
    {
        ...
        return 0;
            ...
            if (logclass & LC_EXEC)
    {
        sprintf (errMsg, "cwdJob: mychdir_(%s) failed for job <%s>: %s", chosenPath, lsb_jobidinstr (jp->jobSpecs.jobId), strerror (errno));
        sbdSyslog (LOG_DEBUG, errMsg);
    }


            if ((pw = getpwdirlsfuser_ (jp->execUsername)) && pw->pw_dir && isAbsolutePathExec (pw->pw_dir))
    {
    ...
    return 0;
            ...
            if (logclass & LC_EXEC)
    {
        sprintf (errMsg, "cwdJob: chdir(%s) failed for job <%s>: %s", cwd, lsb_jobidinstr (jp->jobSpecs.jobId), strerror (errno));
        sbdSyslog (LOG_DEBUG, errMsg);
    }
        }
    ...
    if (chdir (cwd) == -1)
        {
            sprintf (errMsg, "cwdJob: chdir tmp (%s) failed for job <%s>: %s", cwd, lsb_jobidinstr (jp->jobSpecs.jobId), strerror (errno));
            sbdSyslog (LOG_DEBUG, errMsg);
            return -1;
        }
...
int
lsbatchDir( const char *lsbDir, struct jobCard *jp, struct hostent *fromHp, struct lenData *jf)
{
    ...
    if (jp->jobSpecs.jobSpoolDir[0] != '\0')
        {
            sprintf (errMsg, "%s: Enter this function... job <%s> spool dir <%s> is %d", __func__, lsb_jobidinstr (jp->jobSpecs.jobId), jp->jobSpecs.jobSpoolDir, goodSpoolDir);
            sbdSyslog (LOG_DEBUG, errMsg);
        }
    ...
    sprintf (lsbDir, "%s/.lsbatch", jp->jobSpecs.subHomeDir);  // FIXME FIXME FIXME FIXME add to configure.ac
            ...
            if (lsbDirOk (lsbDir, jp, fromHp, jf) == 0)
    {
        ...
        *sp = '\0';
                ...
                if (goodSpoolDir)
        {
            putEnv ("HOME", jp->jobSpecs.subHomeDir);  // FIXME FIXME FIXME FIXME add to configure.ac
        }
                else
        {

            putEnv ("HOME", lsbDir);  // FIXME FIXME FIXME FIXME add to configure.ac
        }
    ...
    sprintf (lsbDir, "%s/.lsbatch", pw->pw_dir); // FIXME FIXME FIXME FIXME add to configure.ac
            if (lsbDirOk (lsbDir, jp, NULL, jf) == 0)
    {

        putEnv ("HOME", pw->pw_dir);  // FIXME FIXME FIXME FIXME add to configure.ac
        return 0;
    }
        }
    ...
    if (mkdir (lsbDir, 0700) == -1 && errno != EEXIST)  // FIXME FIXME FIXME FIXME add to configure.ac // FIXME FIXME FIXME 0700 has to be made into a label
        {
            sprintf (errMsg, I18N_JOB_FAIL_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "mkdir");
            sbdSyslog (LOG_ERR, errMsg);
    ...
    strcat (lsbDir, "/.lsbatch");  // FIXME FIXME FIXME FIXME add to configure.ac
...
int
lsbDirOk (char *lsbDir, struct jobCard *jp, struct hostent *fromHp, struct lenData *jf)
{
    ...
    if ((lsbDir != NULL) && (chosenPath != NULL) && (logclass & LC_EXEC))
        {
            sprintf (errMsg, "Enter %s: lsbDir is %s, chosenPath is %s", __func__, lsbDir, chosenPath);
            sbdSyslog (LOG_DEBUG, errMsg);
        }
            ...
            if ((jp->jobSpecs.options & SUB_CHKPNTABLE) == 0)
    {
        ...
        jobSetupStatus (JOB_STAT_PEND, PEND_JOB_NO_FILE, jp);
                ls_syslog (LOG_DEBUG, "CreateJobfile for job <%s> fail", lsb_jobid2str (jp->jobSpecs.jobId));
                return -1;
            }
            ...
            else
    {
        ...
        {
            unlinkBufFiles (chkpntDir, jp->jobSpecs.jobFile, jp,
                    fromHp);
            jobSetupStatus (JOB_STAT_PEND, PEND_JOB_RESTART_FILE, jp);
            return -1;
        }
    ...
    if ((lsbDir != NULL) && (strlen (lsbDir) > 0))
        {
            sprintf (jobFile, "%s/%s%s", lsbDir, jp->jobSpecs.jobFile, JOBFILEEXT);
        }
    else
        {
            sprintf (jobFile, "%s%s", jp->jobSpecs.jobFile, JOBFILEEXT);
        }
    if (stat (jobFile, &st) == -1)
        {
            ...
            if (errno != ENOENT)
    {
        sprintf (errMsg, I18N_JOB_FAIL_S_M, __func__, lsb_jobid2str (jp->jobSpecs.jobId), "stat");
        sbdSyslog (LOG_ERR, errMsg);
    }
            return -1;
        }
...
int
createChkpntJobFile (char *lsbDir, struct jobCard *jp, struct lenData *jf, struct hostent *fromHp, const char *chkpntDir, const char *restartDir)
{
    // static char __func__] = "createChkpntJobFile()";
    char errMsg[MAX_LINE_LEN];
    char chkDir[MAX_FILENAME_LEN];
    char restartDirBak[MAX_FILENAME_LEN];
    char oldJobId[20];
    char newJobId[20];
    char *strPtr = NULL;
    char *p = NULL;

    if (((strPtr = strrchr (jp->jobSpecs.chkpntDir, '/')) != NULL) && (islongint_ (strPtr + 1)))
        {
            ...
            else
    {
        if (*jp->jobSpecs.execCwd == '\0')
            {
                ...
                if (cwdJob (jp, cwd, fromHp) == -1)
        {
            ls_syslog (LOG_ERR, "%s: cannot find execCwd for job <%s>", __func__, lsb_jobid2str (jp->jobSpecs.jobId));
            return -1;
        }
                strcpy (jp->jobSpecs.execCwd, cwd);
            }
        ...
        *strPtr = '/';
        ...
        {
            sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 316, "Error in the chkpnt directory: %s"), jp->jobSpecs.chkpntDir);  /* catgets 316 */
            sbdSyslog (LOG_ERR, errMsg);
        ...
        p != &(jp->jobSpecs.chkpntDir[0]))
    {
        *p = '/';
        ...
        if ((mymkdir_ (chkDir, 0777, fromHp) < 0 && errno != EEXIST) || (mymkdir_ (chkpntDir, 0700, fromHp) < 0 && errno != EEXIST))
            {
                sprintf (errMsg, I18N_FUNC_S_FAIL_EMSG_S, __func__, "mymkdir_", jp->jobSpecs.chkpntDir, strerror (errno));
                *p = '/';
                sbdSyslog (LOG_ERR, errMsg);
                umask (mode);
                jobSetupStatus (JOB_STAT_PEND, PEND_CHKPNT_DIR, jp);
    ...
    {
        if ((mkdir (chkDir, 0777) < 0 && errno != EEXIST) || (mkdir (chkpntDir, 0700) < 0 && errno != EEXIST))
            {
                sprintf (errMsg, I18N_FUNC_S_FAIL_EMSG_S, __func__, "mkdir", jp->jobSpecs.chkpntDir, strerror (errno));
                sbdSyslog (LOG_ERR, errMsg);
                umask (mode);
                jobSetupStatus (JOB_STAT_PEND, PEND_CHKPNT_DIR, jp);
                return -1;
            ...
            if (strcmp (oldJobId, newJobId) != 0)
    {
        if (((dirp1 = opendir (chkpntDir)) == NULL) && (errno == ENOENT))
            ...
            && (errno == ENOENT))
        {
            sprintf (errMsg, I18N_FUNC_S_FAIL_EMSG_S, __func__, "opendir", chkpntDir, strerror (errno));
            sbdSyslog (LOG_ERR, errMsg);
            return -1;
        }
                else
        {
            if (dirp2)
                ...
                {
                    sprintf (errMsg, "Chkpnt directory %s isn't accessable: %s.", chkpntDir, strerror (errno));
                }
        }
            }
        else
...
myRename (char *fromDir, char *toDir)
{
    ...
    if (rename (fromDir, toDir) == -1) {
        sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 320, "Unable to rename checkpoint directory %s to %s: %s."), fromDir, toDir, strerror (errno)); /* catgets 320 */
        sbdSyslog (LOG_ERR, errMsg);
        return -1;
    }
...
#define STDOUT_DIRECT  1 // FIXME FIXME FIXME FIXME turn to const
#define STDERR_DIRECT  2 // FIXME FIXME FIXME FIXEM turn to const


void
determineFilebufStdoutDirect (char *filebuf, struct jobSpecs *jobSpecsPtr, int flag)
{
    ...
    if (flag == STDOUT_DIRECT) {
        ext = "out";  // FIXME FIXME FIXME FIXME add to configure.ac
        usersFile = jobSpecsPtr->outFile;
    }
    if (flag == STDERR_DIRECT) {
        ext = "err";  // FIXME FIXME FIXME FIXME add to configure.ac
    ...
    if (outputIsDirectory && lastSlash) {
            sprintf (filebuf, "%s%s.%s", usersFile, lsb_jobidinstr (jobSpecsPtr->jobId), ext);
    }
...
int
openStdFiles( const char *lsbDir, const char *chkpntDir, struct jobCard *jobCardPtr,  struct hostent *hp)
{
    ...
    if (logclass & LC_EXEC)
        {
            ...
            sbdSyslog (LOG_DEBUG, errMsg);
            sprintf (errMsg, "%s, the chkpntDir is <%s>", __func__, chkpntDir ? chkpntDir : "NULL");  // FIXME FIXME FIXME FIXME add to configure.ac
            sbdSyslog (LOG_DEBUG, errMsg);
            ...
            else
    {
        sprintf (jobFile, "%s", jobSpecsPtr->jobFile);
            ...
            if (logclass & LC_EXEC)
    {
        sprintf (errMsg, "%s, jobFile is <%s> not chkpntDir", __func__, jobFile);
        sbdSyslog (LOG_DEBUG, errMsg);
    }
        }
    else
            ...
            if ((rc < 0) && (errno == ENOENT))
    {
        ...
        if (rcpFile (jobSpecsPtr, &xf,
                     spoolingHost, XF_OP_SUB2EXEC, rcpMsg) < 0)
            {
        ...
        if (jobCardPtr->spooledExec != NULL)
            ...
            {
                ls_syslog (LOG_ERR, I18N (5305, "Can not allocate memory to save  spooled executable file name <%s>\n please: remove it manually"), spooledExecName);   /* catgets 5305 */
            }
    }
            else
    {
        ...
        if (!S_ISREG (statbuf.st_mode))
            {
                ls_syslog (LOG_ERR, I18N (5306, "spooled command file <%s> is not a regular file"), /* catgets 5306 */
             spooledExecName);
                return -1;
            }
    }
            ...
            if (jobSpecsPtr->xf[i].options & XF_OP_SUB2EXEC)
    {
        ...
        if (rcpFile (jobSpecsPtr, jobSpecsPtr->xf + i,
                     ...
                     rcpMsg) < 0)
            {
                sprintf (xMsg, _i18n_msg_get (ls_catd, NL_SETN, 322, "%sLSF: Failed to copy file <%s> from submission host <%s> to file <%s> on execution host: %s\n"), /* catgets 322 */
            ...
            {
                if (rcpMsg[0] != '\0')
        sprintf (xMsg, _i18n_msg_get (ls_catd, NL_SETN, 323, "%sLSF: Copy file <%s> from submission host <%s> to file <%s> on execution host: %s\n"),   /* catgets 323 */
...
OpenStdin:
        ...
        != NULL)
    {
        strcpy (spoolingHost, pStrTmp);
        memcpy (&spoolHostEnt,
            Gethostbyname_ (spoolingHost), sizeof (spoolHostEnt));
            ...
            else
    {
        ...
        if (i == 0)
            {
                if (rcpMsg[0] != '\0')
        sprintf (xMsg, _i18n_msg_get (ls_catd, NL_SETN, 325, "%sLSF: Copy stdin file <%s> on submissi on host <%s> to <%s> on execution host: %s\n"),   /* catgets 325 */
             xMsg,
             xf.subFn, jobSpecsPtr->fromHost, xf.execFn, rcpMsg);
                goto OpenStdin;
            }

        sprintf (xMsg, _i18n_msg_get (ls_catd, NL_SETN, 326, "%sLSF: Unable to copy stdin file <%s> on submission host <%s> to <%s> on execution host: %s\n"),  /* catgets 326 */
                 ...
                 struct lenData *jf, struct hostent *hp)
{
    ...
    if (chkpntDir == NULL)
        ...
        {
            sprintf (jobFile, "%s/%s%s", chkpntDir, jp->jobSpecs.jobFile,
                 JOBFILEEXT);
    ...
    if ((fd = myopen_ (jobFile, O_CREAT | O_TRUNC | O_WRONLY, 0700, hp)) == -1)
        {
            sprintf (errMsg,
                 "createJobFile: Job <%s> open jobfile %s failed: lsbDir is %s, jp->jobSpecs.jobFile is %s,  %s",
                 lsb_jobidinstr (jobSpecsPtr->jobId), jobFile, lsbDir,
                 jp->jobSpecs.jobFile, strerror (errno));
            sbdSyslog (LOG_ERR, errMsg);
            return -1;
        }
    ...
    if (logclass & LC_TRACE)
        {
            sprintf (errMsg,
                 "createJobFile: Job <%s> open jobfile %s success: lsbDir is %s, jp->jobSpecs.jobFile is %s",
                 lsb_jobidinstr (jobSpecsPtr->jobId), jobFile, lsbDir,
                 jp->jobSpecs.jobFile);
            sbdSyslog (LOG_DEBUG, errMsg);
        }
    shellLine = SHELLLINE;
    if ((cc = write (fd, shellLine, strlen (shellLine))) != strlen (shellLine))
        {
            sprintf (errMsg, "createJobFile:\
 Job <%s> write jobfile %s len=%lu cc=%d failed: %s", lsb_jobidinstr (jobSpecsPtr->jobId), jobFile, strlen (shellLine) + 1, cc, strerror (errno));
            sbdSyslog (LOG_DEBUG, errMsg);
            close (fd);
            return -1;
        }
    ...
    if ((sp = strstr (jf->data, TRAPSIGCMD)) == NULL)
        {
            sprintf (errMsg,
                 "createJobFile: No TRAPSIGCMD <$LSB_TRAPSIGS...> is found in jobfile data for job <%s>",
                 lsb_jobidinstr (jobSpecsPtr->jobId));
            sbdSyslog (LOG_DEBUG, errMsg);
            sp = jf->data;
        }
    ...
    {
        sprintf (errMsg,
             "createJobFile:Job <%s> Unable to creat a symbolic link from <%s> to <%s>: %s",
             lsb_jobidinstr (jobSpecsPtr->jobId), jobFileLink, jobFile,
             strerror (errno));
        sbdSyslog (LOG_DEBUG, errMsg);
        return -1;
    }
    ...
    {
        sprintf (errMsg,
             "createJobFile:Job <%s> Unable to creat shellfile symbolic link from <%s> to <%s>: %s",
             lsb_jobidinstr (jobSpecsPtr->jobId), shellFileLink,
             shellFile, strerror (errno));
        sbdSyslog (LOG_DEBUG, errMsg);
        return -1;
    }
        }
...
jobFileExitStatus (struct jobCard *jobCard)
{
    ...
    if ((hp = Gethostbyname_ (jobCard->jobSpecs.fromHost)) == NULL)
        {
            ls_syslog (LOG_ERR, "\
%s: gethostbyname() %s failed job %s", __func__, jobCard->jobSpecs.fromHost, lsb_jobid2str (jobCard->jobSpecs.jobId));
            exit (jobCard->w_status);
        }
...
static int
restartFiles (char *lsbDir, char *restartDir, struct jobCard *jp,
                struct hostent *hp)
{
    int returnCode = 0;
