	 ...
	 char *errMsg)
{
  ...
  if (pid == 0)
	...
	{
	  sprintf (debugMsg,
		   "rcpFile: Job %s rcpCmd <%s> subFn <%s> execFn <%s> rcpArg <%s> options %x XF_OP_SUB2EXEC %x XF_OP_EXEC2SUB_APPEND %x op %x",
		   lsb_jobidinstr (jp->jobId), rcpCmd, xf->subFn, xf->execFn,
		   rcpArg, xf->options, XF_OP_SUB2EXEC, XF_OP_EXEC2SUB_APPEND,
		   op);
...
rmJobBufFiles (struct jobCard *jp)
{
  ...
  if ((hp = Gethostbyname_ (jp->jobSpecs.fromHost)) == NULL)
    {
      sprintf (errMsg, "\
%s: gethostbyname() %s failed %s", __func__, jp->jobSpecs.fromHost, lsb_jobid2str (jp->jobSpecs.jobId));
  ...
  if (isAbsolutePathSub (jp, jp->jobSpecs.cwd))
	  ...
	  pw->pw_dir && isAbsolutePathExec (pw->pw_dir))
	{
	  if (chdir (pw->pw_dir) == 0)
	      ...
	      if (getcwd (homePath, sizeof (homePath)))
		{
		  ...
		  for (i = 0;
		       homePath[i] != '\0' && subCwd[i] == homePath[i]; i++);
		  if ((i != 0) && (homePath[i] == '\0'))
		    {
		      if (subCwd[i] == '\0')
			{
      ...
      if (chdir (cwd) == -1)
	{
	  if (logclass & LC_EXEC)
	    {
	      sprintf (errMsg,
		       "cwdJob: chdir(%s) failed after mychdir_(%s) failed for job <%s>: %s",
...
static int
createChkpntJobFile (char *lsbDir, struct jobCard *jp, struct lenData *jf,
		     ...
		     char *restartDir)
{
  ...
  if (((strPtr = strrchr (jp->jobSpecs.chkpntDir, '/')) != NULL)
      ...
      else
	{
	  ...
	  sprintf (chkpntDir, "%s/%s", jp->jobSpecs.execCwd,
		   jp->jobSpecs.chkpntDir);
	  *strPtr = '\0';
	  sprintf (chkDir, "%s/%s", jp->jobSpecs.execCwd,
		   jp->jobSpecs.chkpntDir);
    ...
    {
      sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 316, "Error in the chkpnt directory: %s"),	/* catgets 316 */
	       jp->jobSpecs.chkpntDir);
      sbdSyslog (LOG_ERR, errMsg);
	  ...
	  if (((dirp1 = opendir (chkpntDir)) == NULL) && (errno == ENOENT))
		  ...
		  && (errno == ENOENT))
		{
		  if (myRename (chkpntDir, restartDir) == -1)
		    return (-1);
		  if ((symlink (restartDir, chkpntDir) == -1)
		      && (errno != EEXIST))
		    return (-1);
		}
	      else
...
myRename (char *fromDir, char *toDir)
{
  ...
  if (rename (fromDir, toDir) == -1)
    {
      sprintf (errMsg, _i18n_msg_get (ls_catd, NL_SETN, 320, "Unable to rename checkpoint directory %s to %s: %s."),	/* catgets 320 */
