...
main (int argc, char **argv)
{
  ...
  if (resParams[LSF_SERVERDIR].paramValue == NULL)
    {
      ls_openlog ("res", resParams[LSF_LOGDIR].paramValue, (debug > 1),
		  resParams[LSF_LOG_MASK].paramValue);
      ls_syslog (LOG_ERR, _i18n_msg_get (ls_catd, NL_SETN, 5001, "LSF_SERVERDIR not defined in %s/lsf.conf: %M; res exiting"),	/* catgets 5001 */
		 pathname);
      resExit_ (-1);
    }
  ...
  ls_syslog (LOG_DEBUG, "%s: LSF_SERVERDIR=%s", fname,
	     resParams[LSF_SERVERDIR].paramValue);
  ...
  for (;;)
	...
	{
	  ls_syslog (LOG_DEBUG, "\
%s: %s Res child_res=<%d> child_go=<%d> child_cnt=<%d> client_cnt=<%d>", fname, ((child_res) ? "Application" : "Root"), child_res, child_go, child_cnt, client_cnt);
	  ...
	  && FD_IS_VALID (children[i]->std_out.fd))
	{
	  if (children[i]->std_out.retry)
		...
		children[i]->std_out.retry = 0;
	      ...
	      if (logclass & LC_TRACE)
		ls_syslog (LOG_DEBUG, "\
%s: Trying to flush child=<%d> stdout, std_out.retry=<%d>", fname, i, children[i]->std_out.retry);
	  ...
	  && FD_IS_VALID (children[i]->std_err.fd))
	{
	  if (children[i]->std_err.retry)
		...
		children[i]->std_err.retry = 0;
	      ...
	      if (logclass & LC_TRACE)
		ls_syslog (LOG_DEBUG, "\
%s: Trying to flush child=<%d> stderr, std_err.retry=<%d>", fname, i, children[i]->std_err.retry);
