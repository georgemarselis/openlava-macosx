...
struct chanData
{
	...
	int handle; // file handle. due to historical reasons, they have to be an int
...
int            chanAccept_      ( unsigned int chfd, struct sockaddr_in *from);
void           chanInactivate_  ( unsigned int chfd);
void           chanActivate_    ( unsigned int chfd);
int            chanConnect_     ( unsigned int chfd, struct sockaddr_in *peer, time_t timeout);
int            chanSendDgram_   ( unsigned int chfd, char *buf, size_t len, struct sockaddr_in *peer);
int            chanRcvDgram_    ( unsigned int chfd, char *buf, size_t len, struct sockaddr_in *peer, time_t timeout );
...
void           chanCloseAllBut_ ( unsigned int  chfd );
int            chanSelect_      ( struct Masks *sockmask, struct Masks *chanmask, struct timeval *timeout);
int            chanEnqueue_     ( unsigned int  chfd, struct Buffer *msg);
int            chanDequeue_     ( unsigned int  chfd, struct Buffer **buf);
long           chanReadNonBlock_( unsigned int  chfd, char *buf, size_t len, time_t timeout);
long           chanRead_        ( unsigned int  chfd, char *buf, size_t len);
long           chanWrite_       ( unsigned int  chfd, char *buf, size_t len);
int            chanRpc_         ( unsigned int  chfd, struct Buffer *in, struct Buffer *out, struct LSFHeader *outhdr, time_t timeout);

int            chanSock_          ( unsigned int chfd );
int            chanSetMode_       ( unsigned int chfd, mode_t mode );
void           doread             ( unsigned int chfd, struct Masks *chanmask );
void           dowrite            ( unsigned int chfd, struct Masks *chanmask );
struct Buffer *newBuf             ( void );
int            chanAllocBuf_      ( struct Buffer **buf, size_t size);
