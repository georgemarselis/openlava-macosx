...
bool expandWordAll(size_t *size, unsigned int *num, struct inNames **inTable, const char *ptr_level);
bool readHvalues_conf(struct keymap *keyList, const char *linep, struct lsConf *conf, const char *lsfile, size_t *lineNumber, int exact, const char *section);
...
bool addUser(const char *username, size_t maxjobs, float pJobLimit, const char *filename, int override);
bool isInGrp( const char *word, struct groupInfoEnt *gp, int grouptype, int checkAll);
...
bool do_Queues(struct lsConf *conf, const char *filename, size_t *lineNum, struct lsInfo *info, int options);
void initQueueInfo(struct queueInfoEnt *qp);
void freeQueueInfo(struct queueInfoEnt *qp);
bool checkRequeEValues(struct queueInfoEnt *qp, const char *word, const char *filename, size_t *lineNum);
bool addQueue(struct queueInfoEnt *qp, const char *filename, size_t lineNum);
...
bool checkCpuLimit(char **hostSpec, float **cpuFactor, int useSysDefault, const char *filename, size_t *lineNumber, const char *pname, struct lsInfo *info, int options);
bool parseCpuAndRunLimit(struct keymap *keylist, struct queueInfoEnt *qp, const char *filename, size_t *lineNum, const char *pname, struct lsInfo *info, int options);
bool parseProcLimit( const char *word, struct queueInfoEnt *qp, const char *filename, size_t *lineNum, const char *pname);
bool parseLimitAndSpec( const char *word, float *limit, char **spec, const char *hostSpec, const char *param, struct queueInfoEnt *qp, const char *filename, size_t *lineNum, const char *pname);
float *getModelFactor( const char *hostModel, struct lsInfo *info);
float *getHostFactor( const char *host);
char *parseAdmins( const char *admins, int options, const char *filename, size_t *lineNum);
char *putIntoList( char **list, unsigned int *length, const char *string, const char *listName);
int setDefaultHost(struct lsInfo *info);
unsigned int setDefaultUser(void);
bool handleUserMem(void);
int handleHostMem(void);
int parseSigActCmd(struct queueInfoEnt *qp, const char *linep, const char *filename, size_t *lineNum, const char *section);
bool terminateWhen(struct queueInfoEnt *qp, const char *linep, const char *filename, size_t *lineNum, const char *section);
int checkAllOthers( const char *word, int *hasAllOthers);
int getReserve(char *reserve, struct queueInfoEnt *qp, const char *filename, size_t lineNum);
...
int parseDefAndMaxLimits(struct keymap *key, float *defaultVal, float *maxVal, const char *filename, size_t *lineNum, const char *pname);
