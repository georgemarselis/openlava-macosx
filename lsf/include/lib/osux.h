...
procStart_ (char *argv[],
	    char *envp[],
	    int procStartFlags,
	    LS_PROC_START_T * startInfo, LS_PROCESS_T * procInfo)
{
  int ii;
  pid_t idPro;
  pid_t idCrnt;
...
procSendSignal_ (LS_ID_T idProc,
		 int sig, int jobTerminateInterval, int options)
{
  if (idProc < 0)
    {
      return ERR_BAD_ARG;
...
thrdStart_ (void (*thread_func) (void *),
	    void *params,
	    int thrdStartFlags,
	    LS_THRD_START_T * startInfo, LS_THREAD_T * thrdInfo)
{
  pid_t idPro;
  pid_t idCrnt;
  int tmpPrio;
...
unsigned long
getEnvVar_ (char *name, char *buf, unsigned long size)
{
  char *varStr;
  unsigned long len;

  if (NULL == (varStr = getenv (name)))
    return 0;
