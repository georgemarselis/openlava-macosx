...
void freeSA(const char **list, unsigned int num);
unsigned int fillCell_(struct inNames **table, const char *name, const char *level);
...
int parseDefAndMaxLimits(struct keymap key, int *defaultVal, int *maxVal, const char *filename, size_t *lineNum, const char *pname);
int parseQFirstHost(char *myWord, int *haveFirst, const char *pname, size_t *lineNum, const char *filename, const char *section);
int chkFirstHost(const char *host, int *needChk);
