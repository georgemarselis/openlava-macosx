...
struct lsRequest *lsReqHandCreate_ ( unsigned long taskid, unsigned int seqno, int connfd, void *extra, requestCompletionHandler replyHandler,  appCompletionHandler appHandler, void *appExtra);
int ackAsyncReturnCode_(int s, struct LSFHeader *replyHdr);
int enqueueTaskMsg_(int s, unsigned long taskID, struct LSFHeader *msgHdr);
...
struct lsRequest *lsIRGetRusage_(pid_t rtaskid, struct jRusage *ru, appCompletionHandler appHandler, void *appExtra, int options);
int lsGetRProcRusage(const char *host, unsigned long rtaskid, struct jRusage *ru, int options);
struct lsRequest *lsGetIRProcRusage_(const char *host, unsigned long taskid, pid_t pid, struct jRusage *ru, appCompletionHandler appHandler, void *appExtra);
...
int ls_rkill( unsigned long rtaskid, int sig);
int lsMsgRdy_(unsigned long taskid, size_t *msgLen);
void tMsgDestroy_(void *extra);
long lsMsgRcv_(unsigned long taskid, const char *buffer, size_t length, int options);

int lsMsgSnd2_( int *socket,  unsigned short opcode, const char *buffer, size_t length, int options);
int lsMsgSnd_ ( unsigned long taskid,                const char *buffer, size_t length, int options);

int lsMsgWait_(unsigned long inTidCnt, unsigned long *tidArray, unsigned long *rdyTidCnt, unsigned int inFdCnt, int *fdArray, unsigned long *rdyFdCnt, int *outFdArray, struct timeval *timeout, int options);
