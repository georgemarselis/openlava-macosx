...
getEchkpntVar (const char *pVariableName)
{
  ...
  for (pTableItem = m_pTableHeader; pTableItem != NULL;
       pTableItem = pTableItem->m_pNextItem)
    {

      if ((pTableItem->m_pVarPair != NULL)
	  && (pTableItem->m_pVarPair->m_pVariable != NULL)
	  && (strcmp (pTableItem->m_pVarPair->m_pVariable, pVariableName) ==
	      0))
	{
...
updateVarToFile_ ()
{
  ...
  for (pTableItem = m_pTableHeader; pTableItem != NULL;
       pTableItem = pTableItem->m_pNextItem)
    {
      ...
      if ((pVarPair != NULL)
	  && (pVarPair->m_pVariable != NULL) && (pVarPair->m_pValue != NULL))
	{
