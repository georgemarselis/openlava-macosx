  ...
  // static char __func__] = "main()";
    ...
    while ((cc = getopt (argc, argv, "cfksd:Vhx")) != EOF)  {
        switch (cc) {
            ...
            case 'f': // T
            case 'k': // F
            case 'x': // are all these switches supposed to be doing
            case 's': // 
                {
                    1; // I don't know man, don't ask me
                }
            break;
            case 'V': {
                fprintf( STDERR, _LS_VERSION_, stderr);
                exit (0); 
            }
            case 'd':
                pChkpntDir = optarg;
            ...
            default: {
                usage (argv[0]);
                exit (-1); // FIXME FIXME FIXME FIXME replace negative number with positive, but no idea what it is supposed to mean
            }
            break;
        }
    }
    ...
    if (pChkpntDir == NULL) {
        fprintf( stderr, "No checkpoint directory defined.");
        usage (argv[0]);
        exit (-1);
    }
    ...
    if (iReValue == 1) {
        ...
        jf = getenv ("LSB_JOBFILENAME"); // FIXME FIXME FIXME FIXME put this in configure.ac
        if (jf == NULL) {
            fprintf (stderr, "%s : getenv of LSB_JOBFILENAME failed", __func__);
        }
        else {
            jfn = strchr (jf, '/'); // FIXME FIXME FIXME FIXME put this in configure.ac
            if (jfn) {
                sprintf (resAcctFN, "%s/.%s.acct", LSTMPDIR, jfn + 1); // FIXME FIXME FIXME FIXME put this in configure.ac
        ...
        fprintf (stderr, "%s : the .echkpnt file content occurs error: %s\n", __func__, strerror (errno)); // FIXME FIXME FIXME FIXME put  .echkpnt  in configure.ac
        exit (-1); // FIXME FIXME FIXME FIXME replace negative number with positive indicating the .echkpnt file content occurs error
    }

    if( ( pIsOutput != NULL ) && ( ( strcmp (pIsOutput, ECHKPNT_OPEN_OUTPUT) == 0 ) || ( strcmp (pIsOutput, ECHKPNT_OPEN_OUTPUT_L) == 0 ) ) ) {
        initLog ("Echkpnt"); // FIXME FIXME FIXME FIXME put this in configure.ac
    }

#ifdef DEBUG // FIXME FIXME FIXME FIXME replace with run-time debug
    ...
    if(  strcmp( pMethodName, ECHKPNT_DEFAULT_METHOD ) == 0 ) {
        cargv[0] = "echkpnt.default"; // FIXME FIXME FIXME FIXME put .echkpnt.default in configure.ac
        freeTable_ ();
...
#endif
        closeLog ();
        execv (echkpntProgPath, cargv); // we are not supposed to come back from here
        sprintf (logMesgBuf, "%s : execute the echkpnt.default fail\n%s\n",  __func__, (errno ? strerror (errno) : "")); // FIXME FIXME FIXME FIXME put .echkpnt.default in configure.ac
        fprintf (stderr, "%s", logMesgBuf);
        exit (-1); // FIXME FIXME FIXME FIXME replace negative value with value indicating failure to execv
    }
    ...
    if (iChildPid < 0) {
        ...
        freeTable_ ();
        closeLog ();
        exit (-1); // FIXME FIXME FIXME FIXME replace negative value with value indicating failure to fork
    }
    else if (iChildPid == 0) {
        ...
        memset( progName, '\0', MAX_FILENAME_LEN );

        sprintf (logMesgBuf, "erestart.%s", pMethodName); // FIXME FIXME FIXME FIXME put .echkpnt.default in configure.ac
        setMesgHeader (logMesgBuf);


        if ((pIsOutput == NULL) || ((strcmp (pIsOutput, ECHKPNT_OPEN_OUTPUT) != 0)  && (strcmp (pIsOutput, ECHKPNT_OPEN_OUTPUT_L) != 0))) {
            if (redirectFd (ECHKPNT_DEFAULT_OUTPUT_FILE, 1) == -1) { // FIXME FIXME FIXME what is this 1? put an enum in
                sprintf (logMesgBuf, "%s : redirect stdout to %s file\n%s\n", __func__, ECHKPNT_DEFAULT_OUTPUT_FILE, errno ? strerror (errno) : "");
                // goto Error;
                fprintf (stderr, "%s", logMesgBuf);
                logMesg (logMesgBuf);
                freeTable_ ();
                closeLog ();
                exit (-1); // FIXME FIXME FIXME FIXME replace negative value with value indicating failure to redirectFd 1
            }
            if (redirectFd (ECHKPNT_DEFAULT_OUTPUT_FILE, 2) == -1) { // FIXME FIXME FIXME what is this 2? put an enum in
                ...
                freeTable_ ();
                closeLog ();
                exit (-1); // FIXME FIXME FIXME FIXME replace negative value with value indicating failure to redirectFd 2
            }
        }
        else {
            char aFileName[MAX_PATH_LEN];
            if( ( getChkpntDirFile( aFileName, ECHKPNT_STDOUT_FILE ) == -1 ) || ( redirectFd ( aFileName, 1 ) == -1 ) ) { // FIXME FIXME FIXME what is this redirectFd 1? put an enum in
                sprintf (logMesgBuf, "%s : redirect the stdout to %s fail\n", __func__, ECHKPNT_STDOUT_FILE); 
                logMesg (logMesgBuf);

                if( redirectFd( ECHKPNT_DEFAULT_OUTPUT_FILE, 1 ) == -1 ) { // FIXME FIXME FIXME what is this redirectFd 1? put an enum in
            ...
            if ((getChkpntDirFile (aFileName, ECHKPNT_STDERR_FILE) == -1) || (redirectFd (aFileName, 2) == -1)) {
        ...
        for( unsigned int ii = 3; ii < lMaxfds; ii++) {
            close (ii);
        }
    ...
    if (iChildPid < 0) {
        ...
        if (WEXITSTATUS (iStatus) != 0) {
            sprintf (logMesgBuf, "%s : the echkpnt.%s fail,the exit value is %d\n", __func__, pMethodName, WEXITSTATUS (iStatus)); // the english is fail, too
            fprintf (stderr, "%s", logMesgBuf);
            logMesg (logMesgBuf);
            freeTable_ ();
            closeLog ();
