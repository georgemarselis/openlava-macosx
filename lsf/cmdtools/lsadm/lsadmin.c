...
static int opCodeList[] =
  { LIM_CMD_REBOOT, -1, LIM_CMD_REBOOT, -1, LIM_CMD_SHUTDOWN, 0,
...
lsadminDebug (int argc, char **argv, int opCode)
{
  ...
  while ((c = getopt (argc, argv, opt)) != EOF)
	...
	case 'f':
	  if (strstr (optarg, "/") && strstr (optarg, "\\"))
	    {
	      fprintf (stderr, I18N (6549, "Command denied. Invalid file name\n"));	/*  catgets 6549 */
	      return (-1);
	    }
	  fileOption = TRUE;
	  ...
	  if (debug.logFileName[strlen (debug.logFileName) - 1] == '/' ||
	    ...
	    {
	      fprintf (stderr, I18N (6565, "Command denied. File name is needed after the path\n"));	/*  catgets 6565 */
	      return (-1);
	    }
	  break;
  ...
  if (checkPerm ())
	...
	{
	  fprintf (stderr, I18N (6546, "Operation permission denied by LIM\n"));	/* catgets 6546 */
	}
      else
	{
	  fprintf (stderr, I18N (6550, "Operation permission denied by RES\n"));	/* catgets 6550 */
	}
      return (-1);
    }
  if (opCode == LIM_DEBUG || opCode == LIM_TIMING)
  ...
  if (pid == 0)
      ...
      for (i = 0; i < numHost; i++)
	{
	  if (hosts[i][0] != '\0')
		...
		{
		  sprintf (strBuf, "Host %s set RES debug failed: ",
			   hosts[i]);
