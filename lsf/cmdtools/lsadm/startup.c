...
getLSFenv (void)
{
  ...
  if ((myClusterConf =
       findMyCluster (mySharedConf->clusterName, mySharedConf)))
    {
      return (0);
    }
  else
...
findMyCluster (char *CName, struct sharedConf *mySharedConf)
{
  ...
  ls_strcat (lsfClusterFile, sizeof (lsfClusterFile),
	     myParamList[LSF_CONFDIR].paramValue);
...
startupRemoteHost (char *host, int opCode, int ask)
{
  ...
  if ((envDir = getenv ("LSF_ENVDIR")))
	...
	case LSADM_LIMSTARTUP:
	  if (symbolic == TRUE)
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; $LSF_BINDIR/lsadmin limstartup'");
	    }
	  else
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; . $LSF_CONFDIR/profile.lsf; $LSF_BINDIR/lsadmin limstartup'");
	    }
	  break;
	case LSADM_RESSTARTUP:
	  if (symbolic == TRUE)
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; $LSF_BINDIR/lsadmin resstartup'");
	    }
	  else
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; . $LSF_CONFDIR/profile.lsf; $LSF_BINDIR/lsadmin resstartup'");
	    }
	  break;
	case BADMIN_HSTARTUP:
	  if (symbolic == TRUE)
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; $LSF_BINDIR/badmin hstartup'");
	    }
	  else
	    {
	      ls_strcat (msg, sizeof (msg),
			 "; export LSF_ENVDIR; . $LSF_ENVDIR/lsf.conf; . $LSF_CONFDIR/profile.lsf; $LSF_BINDIR/badmin hstartup'");
	    }
	  break;
	default:
	...
	case LSADM_LIMSTARTUP:
	  if (symbolic == TRUE)
	    {
	      myargv[cc++] =
		"'. /etc/lsf.conf; $LSF_BINDIR/lsadmin limstartup'";
	    }
	  else
	    {
	      myargv[cc++] =
		"'. /etc/lsf.conf; . $LSF_CONFDIR/profile.lsf;  lsadmin limstartup'";
	    }
	  break;
	case LSADM_RESSTARTUP:
	  if (symbolic == TRUE)
	    {
	      myargv[cc++] =
		"'. /etc/lsf.conf; $LSF_BINDIR/lsadmin resstartup'";
	    }
	  else
	    {
	      myargv[cc++] =
		"'. /etc/lsf.conf; . $LSF_CONFDIR/profile.lsf; lsadmin resstartup'";
	    }
	  break;
	case BADMIN_HSTARTUP:
	  if (symbolic == TRUE)
	    ...
	    {
	      myargv[cc++] =
		"'. /etc/lsf.conf; . $LSF_CONFDIR/profile.lsf; badmin hstartup'";
	    }
	  break;
	default:
	  fprintf (stderr, "startUpHostR: %s %d\n",
