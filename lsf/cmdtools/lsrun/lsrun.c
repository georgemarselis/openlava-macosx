...
usage (void)
{
    fprintf (stderr, "lsrun: [-h] [-V] [-P] [-v] [-p] [-R] [-m hosts...]\n");
...
main (int argc, char **argv, char **environ)
{
    ...
    case 'm':
        ...
        if (gethostsbyname (P->hosts, &err) < 0) {
                fprintf (stderr, "lsrun: cannot resolve %s hostname", err);
                free (err);
                return -1;
            }
        break;
            ...
            if (P->verbose)
    {
        printf ("Running task on host %s\n", hlist[cc]);
            ...
            taskid = ls_rtaske (hlist[cc], P->cmd, P->pty ? REXF_USEPTY : 0, environ);
            if (taskid < 0)
    {
        fprintf (stderr, "\
    ...
    while (num > 0)
        {
            ...
            // taskid = ls_rwait (&stat, 0, &ru);
            taskid = rwait_ (0, &stat, 0, &ru);
            if (taskid < 0)
    {
        ls_perror ("ls_rwait()");
        break;
