...
{
  fprintf (stderr,
	   "%s: %s [-h] [-V] [-L] [-R res_req] [-n needed] [-w wanted]\n               [host_name ... ]\n",
	   I18N_Usage, cmd);
  exit (-1);
}
  ...
  for (; optind < argc; optind++)
    {
      if (cc >= MAXLISTSIZE)
	{
	  fprintf (stderr, _i18n_msg_get (ls_catd, NL_SETN, 2201, "%s: too many hosts specified (max %d)\n"),	/* catgets  2201  */
		   argv[0], MAXLISTSIZE);
	  exit (-1);
	}
  ...
  if (!desthosts)
    {
	  ...
	  lserrno == LSE_UNKWN_RESNAME || lserrno == LSE_UNKWN_RESVALUE)
	exit (-1);
      else
	exit (1);
    }
