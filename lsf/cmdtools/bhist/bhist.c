...
static void printEvent (struct bhistReq *, struct jobRecord *,
			struct jobInfoEnt *, struct eventRecord *, time_t,
			char *, int);
...
extern struct eventRec *lsbGetNextJobEvent (struct eventLogHandle *, int *,
					    int, LS_LONG_INT *,
					    struct jobIdIndexS *);
...
usage (char *cmd)
{
  ...
  fprintf (stderr,
	   "       %s [-h] [-V] -t [-f logfile_name] [-T time0,time1]\n",
	   cmd);
