...
void bhist_usage (char *);
void fileNumbers (char *, struct bhistReq *);
int do_options (int, char **, struct bhistReq *);
void displayhist (struct bhistReq *);
int skip_jobRecord (struct jobRecord *, struct bhistReq *);
char logfile_check (struct bhistReq *);
char *getEventStatus (struct eventRecord *);
void initLoadIndexNames (void);
void prtParameters (struct jobInfoEnt *, struct bhistReq *, char *);
char *getUserName (int);
int dispChkpnt (struct eventRecord *, struct jobRecord *);
void readEventFromHead (char *, struct bhistReq *);
void printEvent (struct bhistReq *, struct jobRecord *, struct jobInfoEnt *, struct eventRecord *, time_t, char *, int);
void printChronicleEventLog (struct eventRec *, struct bhistReq *);
char *lowFirstChar (char *);

void prtModifiedJob (struct jobModLog *, struct bhistReq *, char *);
int initJobIdIndexS (struct jobIdIndexS *indexS, char *fileName);
...
bhist_usage (char *cmd)
{
    ...
    exit (-1); // FIXME FIXME FIXME why return a negative exit status?
}



void
fileNumbers (char *inputstr, struct bhistReq *Req)
{
...
do_options (int argc, char **argv, struct bhistReq *bhistReq)
{
  ...
  while ((cc =
      getopt (argc, argv,
          "VaAhwlbdepstrf:q:m:u:C:D:S:N:J:P:T:n:")) != EOF)
    {
      switch (cc)
    {
    case 'A':
  ...
  if (bhistReq->options & OPT_CHRONICLE)
    ...
    usage (argv[0]);
      ...
      if (bhistReq->options &
      ~OPT_CHRONICLE & ~OPT_TIME_INTERVAL & ~OPT_ELOGFILE)
    usage (argv[0]);
  ...
  if (!(bhistReq->options & OPT_TIME_INTERVAL))
      ...
      (bhistReq->options & OPT_DISPATCH))
    {
      ...
      !((defaultTime[0] != 0) && (bhistReq->options & OPT_LONGFORMAT)))
    {
      ...
      if ((options & OPT_DFTSTATUS) != OPT_DFTSTATUS)
    {
      if (!(((options & OPT_DONE) &&
        ...
        || ((options & OPT_EXIT) &&
            (jobRecord->currentStatus & JOB_STAT_EXIT))
        || ((options & OPT_PEND)
            && (IS_PEND (jobRecord->currentStatus)))
        || ((options & OPT_SUSP)
            && (jobRecord->
  ...
  while (jobRecord != jobRecordList)
      ...
      while (event)
    {
      ...
      if (matchJobId (bhistReq, LSB_JOBID ((int) job->jobId, event->idx)))
        {
          printEvent (bhistReq,
              jobRecord, job, event, timeStamp, timestamp, 0);
      ...
      if (nextevent == NULL)
          ...
          else
        {
          ...
          if ((bhistReq->searchTime[1] < runningTime)
              && (bhistReq->searchTime[1] != -1))
            {
              event->jStatus = JOB_STAT_EXIT;
            }
        ...
        {
          timeinterval =
        (double) nextevent->timeStamp - (double) timeStamp;
      ...
      if (!matchJobId
          (bhistReq, LSB_JOBID ((int) job->jobId, lastevent->idx))
      ...
      if ((event->kind != EVENT_JOB_SWITCH))
        ...
        case JOB_STAT_DONE:
          if (event->reasons & FINISH_PEND
              || (nextevent
              && !(nextevent->jStatus & JOB_STAT_PDONE)))
            {
              pendTime += timeinterval;
            }
          break;
    ...
    End:if ((bhistReq->options & OPT_CHRONICLE) ==
          OPT_CHRONICLE)
      ...
      if (bhistReq->options & OPT_WIDEFORMAT)
          ...
          if ((pos = strchr (jobName, '[')) && LSB_ARRAY_IDX (job->jobId))
        {
          *pos = '\0';
          ...
          printf
        ("%-7s %-7s %-9s %-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-10.0f\n",
         lsb_jobid2str (job->jobId), jobRecord->job->user, jobName,
                  ...
                  MAXLINELEN) != 0)
        {
          ...
          printf
        ("%-7.7s %-7.7s %-9.9s %-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-8.0f%-10.0f\n",
         lsb_jobid2str (job->jobId), osUserName, jobName,
         NegtoZero (pendTime), NegtoZero (pendSuspTime),
         NegtoZero (runTime), NegtoZero (usrSuspTime),
         NegtoZero (sysSuspTime), NegtoZero (unknownTime),
         NegtoZero (pendTime) + NegtoZero (pendSuspTime) +
...
%s/logdir/lsb.events.%d", bhistParams[LSB_SHAREDIR].paramValue, ++maxEventFile);
    }
  ...
  if (Req->options & OPT_CHRONICLE)
      ...
      while (TRUE)
    {
      ...
      ls_syslog (LOG_ERR, I18N (3203, "File %s at line %d: %s\n"),  /* catgets 3203 */
             eLogPtr->openEventFile, lineNum, lsb_sysmsg ());
    }
  ...
  while (TRUE)
    {
      ...
      if ((log = lsbGetNextJobEvent (eLogPtr,
                     &lineNum, Req->numJobs, Req->jobIds,
                     jobIdIndexSPtr)) != NULL)
    {
...
void
prtModifiedJob (struct jobModLog *jobModLog, struct bhistReq *bhistReq,
        char *timestamp)
{
  char prline[MSGSIZE];
  int i;
...
#define PRT_FMTSTR(i18nstr)                     \
  ...
  if (jobModLog->options & SUB_IN_FILE)
    {
      sprintf (prline,
           I18N (3360, "Input file change to : %s") /* catgets 3360 */ ,
           jobModLog->inFile);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_OUT_FILE)
    {
      sprintf (prline,
           I18N (3361, "Output file change to : %s") /* catgets 3361 */ ,
           jobModLog->outFile);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_ERR_FILE)
    {
      sprintf (prline,
           I18N (3362, "Error file change to : %s") /* catgets 3362 */ ,
           jobModLog->errFile);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_CHKPNT_PERIOD)
    {
      sprintf (prline, "%s: %d",
           I18N (3367,
             "Checkpoint period changes to") /* catgets 3367 */ ,
  ...
  if (jobModLog->options & SUB_HOST_SPEC)
    {
      sprintf (prline, "%s: %s", I18N (3370, "Host specification change to"),   /* catgets 3370 */
           jobModLog->hostSpec);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_DEPEND_COND)
    {
      sprintf (prline, "%s: %s", I18N (3371, "Dependances condition changes to"),   /* catgets 3371 */
  ...
  if (jobModLog->options & SUB_RES_REQ)
    {
      sprintf (prline, "%s: %s",
           I18N (3372,
             "Resource requirement changes to") /* catgets 3372 */ ,
           jobModLog->resReq);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_OTHER_FILES)
    {
      sprintf (prline, "%s: %d",
           I18N (3373,
             "Number of Copied files change to") /* catgets 3373 */ ,
           jobModLog->nxf);
      PRT_FMTSTR (prline);
      for (i = 0; i < jobModLog->nxf; i++)
    {
      sprintf (prline,
           I18N (3374,
             "%3d : subfile name is %s") /* catgets 3374 */ , i,
           jobModLog->xf[i].subFn);
      PRT_FMTSTR (prline);
      sprintf (prline,
           I18N (3375,
             "      execfile name is %s") /* catgets 3375 */ ,
           jobModLog->xf[i].execFn);
      PRT_FMTSTR (prline);
      sprintf (prline,
           I18N (3376, "      file options %d") /* catgets 3376 */ ,
           jobModLog->xf[i].options);
      PRT_FMTSTR (prline);
    }
    }
  ...
  if (jobModLog->options & SUB_PRE_EXEC)
    {
      sprintf (prline,
           I18N (3377,
             "Pre exec command changes to : %s") /* catgets 3377 */ ,
           jobModLog->preExecCmd);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_LOGIN_SHELL)
    {
      sprintf (prline,
           I18N (3378, "Log shell changes to : %s") /* catgets 3378 */ ,
           jobModLog->loginShell);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_MAIL_USER)
    {
      sprintf (prline,
           I18N (3379, "Mail user changes to : %s") /* catgets 3379 */ ,
           jobModLog->mailUser);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->options & SUB_PROJECT_NAME)
    {
      sprintf (prline,
           I18N (3380,
             "Project name changes to : %s") /* catgets 3380 */ ,
           jobModLog->projectName);
      PRT_FMTSTR (prline);
    }
  ...
  for (i = 0; i < LSF_RLIM_NLIMITS; i++)
    ...
    switch (i)
      ...
      case LSF_RLIMIT_CPU:
        sprintf (prline,
             I18N (3381,
               "CPU limit changes to : %d (minutes)")
             /* catgets 3381 */ ,
      ...
      case LSF_RLIMIT_RUN:
        sprintf (prline,
             I18N (3433,
               "run limit changes to : %d (minutes)")
             /* catgets 3433 */ ,
      ...
      case LSF_RLIMIT_PROCESS:
        sprintf (prline,
             I18N (3388,
               "Process limit changes to : %d") /* catgets 3388 */
             ,
  ...
  if (jobModLog->options2 & SUB2_MODIFY_CMD)
    {
      sprintf (prline,
           I18N (3390, "Job command changes to : %s") /* catgets 3390 */ ,
           jobModLog->command);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->numProcessors != DEFAULT_NUMPRO &&
      jobModLog->numProcessors != DEL_NUMPRO)
    {
      sprintf (prline,
           I18N (3391,
             "New minimal num of processors is : %d")
           /* catgets 3391 */ , jobModLog->numProcessors);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->maxNumProcessors != DEFAULT_NUMPRO &&
      jobModLog->maxNumProcessors != DEL_NUMPRO)
    {
      sprintf (prline,
           I18N (3392,
             "New max num of processors is : %d") /* catgets 3392 */ ,
           jobModLog->maxNumProcessors);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->beginTime > 0)
    {
      ...
      sprintf (prline,
           I18N (3393, "Job new begin time is : %s") /* catgets 3393 */ ,
           sp);
      PRT_FMTSTR (prline);
      free (sp);
    }
  ...
  if (jobModLog->termTime > 0)
    {
      sp =
    putstr_ (_i18n_ctime
         (ls_catd, CTIME_FORMAT_a_b_d_T_Y,
          (time_t *) & jobModLog->termTime));
      sprintf (prline,
           I18N (3394, "Job new end time is : %s") /* catgets 3394 */ ,
           sp);
      PRT_FMTSTR (prline);
      free (sp);
    }
  ...
  if (jobModLog->options2 & SUB2_JOB_PRIORITY)
    {
      sprintf (prline,
           I18N (3174, "New job priority is: %d") /* catgets 3174 */ ,
           jobModLog->userPriority);
      PRT_FMTSTR (prline);
    }
  ...
  if (jobModLog->delOptions & SUB_QUEUE)
    {
      sprintf (prline,
           I18N (3395,
             "Job queue changes back to default") /* catgets 3395 */
           );
      PRT_FMTSTR (prline);
    }
  ...
  if (bhistReq->options & OPT_LONGFORMAT)
    {
      ...
      prtSubDetails (params, params->submit.hostSpec, params->cpuFactor);
    }
  else
    prtLine (";\n");
}
        ...
        time_t timeStamp, char *timeStampStr, int option)
{
  ...
  if ((bhistReq->options & OPT_CHRONICLE))
    {
      if (event->idx > 0)
    sprintf (tBuff, " %s <%d[%d]>",
         I18N_Job, (int) job->jobId, event->idx);
      else
    sprintf (tBuff, " %s <%d>", I18N_Job, (int) job->jobId);
    }
  ...
  switch (event->kind)
      ...
      if (job->exHosts && job->numExHosts > 0)
    {
      ...
      job->exHosts = calloc (job->numExHosts, sizeof (char *));
      for (i = 0; i < job->numExHosts; i++)
    {
    ...
    case EVENT_JOB_MOVE:
      if (tBuff[0] == '\0')
    sprintf (prline, (_i18n_msg_get (ls_catd, NL_SETN, 3427, "%-12.19s: Job moved to position %d relative to <%s> by user or administrator <%s>")), /* catgets 3427 */
         timeStampStr,
         event->position, topOrBot (event->base), event->userName);
      else
    sprintf (prline, (_i18n_msg_get (ls_catd, NL_SETN, 3428, "%-12.19s:%s moved to position %d relative to <%s> by user or administrator <%s>")),   /* catgets 3428 */
         timeStampStr,
         tBuff,
         event->position, topOrBot (event->base), event->userName);
      prtLine (prline);
    ...
    *cp1 = '\000';
      ...
      while (cp1 != NULL)
    {
      ...
      if ((event->jStatus & JOB_STAT_DONE) ||
      ((event->jStatus & JOB_STAT_EXIT) &&
       !(event->reasons & (EXIT_ZOMBIE | EXIT_RERUN | EXIT_KILL_ZOMBIE))))
    {
      LS_WAIT_T wStatus;
    ...
    case EVENT_JOB_MODIFY2:
      {
    prtModifiedJob (&(event->eventRecUnion.jobModLog), bhistReq,
            timeStampStr);
  ...
  switch (log->type)
    ...
    case EVENT_CHKPNT:
      ...
      sprintf (prline, (_i18n_msg_get (ls_catd, NL_SETN, 3314, "%-12.19s:%s checkpoint info with period<%ld> pid<%d> ok<%d> flags<%d>")),   /* catgets  3314  */
           ...
           log->eventLog.chkpntLog.ok, log->eventLog.chkpntLog.flags);
      prtLine (prline);
      prtLine (";\n");
      break;
    case EVENT_MIG:
    ...
    if ((strcmp (sigactLog->signalSymbol, "SIG_CHKPNT") == 0)
    ...
    else
      {
        ...
        sprintf (prline, "%-12.19s:%s %s %s (actpid %d)",
             ...
             I18N (3262, "initiated"), sigactLog->pid);
        break;
          case ACT_DONE:
          case ACT_FAIL:
    ...
    case EVENT_JOB_CLEAN:
      ...
      sprintf (prline, "%-12.19s:%s %s",
           timeStampStr,
           tBuff, I18N (3349, "has been cleaned") /* catgets 3349 */ );
      prtLine (prline);
      prtLine (";\n");
      break;
