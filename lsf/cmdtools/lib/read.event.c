...
read_newjob (struct eventRec *log)
{
  ...
  if (submitPtr->options & SUB_JOB_NAME)
    ...
    {
      STRNCPY (submitPtr->jobName, submitPtr->command, MAX_LSB_NAME_LEN);
    }
...
copyJobModLog (struct jobModLog *des, struct jobModLog *src)
{
  ...
  if (des->numAskedHosts > 0)
    {
      des->askedHosts =
	(char **) calloc (des->numAskedHosts, sizeof (char *));
...
freeJobRecord (struct jobRecord *jobRecord)
{
  ...
  while (nextEvent)
	...
	currentEvent->chronback->chronforw = NULL;
      if ((currentEvent->chronback == NULL)
	  && (currentEvent->chronforw != NULL))
  ...
  switch (log->type)
      ...
      if (job != NULL)
	{
	  ...
	  if (Req->options & OPT_JOBNAME)
		...
		{
		  Req->jobIds =
		    (LS_LONG_INT *) realloc (Req->jobIds,
					     Req->numJobs *
					     sizeof (LS_LONG_INT));
		  if (Req->jobIds == NULL)
		    {
		  ...
		  for (i = oldjobnum; i < Req->numJobs; i++)
		    {
		      Req->jobIds[i] =
			LSB_JOBID (LSB_ARRAY_JOBID (job->jobId),
				   idxList[i - oldjobnum]);
		    }
		}
	      FREEUP (idxList);
	  ...
	  if (check_queue (Req, submitPtr->queue))
	      ...
	      for (i = 0; i < numEles; i++)
		{
		  ...
		  newjobRecord->hostFactor =
		    log->eventLog.jobNewLog.hostFactor;
		  ...
		  memset ((struct eventRecord *) event, 0,
	...
	sscanf (log->eventLog.jobModLog.jobIdStr, "%d[%d]", &array_jobId,
		&array_ele);
	  ...
	  if (LSB_ARRAY_IDX (Req->jobIds[i]) > 0)
	    {
		  ...
		  ((LSB_ARRAY_JOBID (jobId) ==
		    LSB_ARRAY_JOBID (Req->jobIds[i]))
...
insertModEvent (struct eventRec *log, hEnt * ent)
{
  ...
  if (!(event = (struct eventRecord *) malloc (sizeof (struct eventRecord))))
    {
      perror ("malloc");
      exit (-1);
    }
  ...
  if (log->type == EVENT_JOB_MODIFY2)
    {
      copyJobModLog (&(event->eventRecUnion.jobModLog),
		     &(log->eventLog.jobModLog));
    }
