...
prtBETime (struct submit req)
{
    ...
    char sp[60]; // FIXME FIXME FIXME FIXME FIXME why 60?
...
if (operate == CMD_BMODIFY)
{
  ...
  if ((index + 1) < argc)
  {
      ...
      {
          fprintf (stderr, "%s %s\n", (_i18n_msg_get (ls_catd, NL_SETN, 1580, "Checkpoint method cannot be changed with bmod:")), argv[index + 1]); /* catgets  1580  */
          return -1;
      }
  }
}
...
if (fstat (0, &statBuf) == 0)
    ...
    sprintf (chkDir, "%s/%s", argv[optind],
     lsb_jobidinstr (arrayJobId));
...
else if (argc >= optind + 1)
{
  ...
  if (embedArgc > 1 && operate == CMD_BSUB)
  {
    ...
    if (req->options2 & SUB2_JOB_CMD_SPOOL)
    {
      ...
      fprintf (stderr, (_i18n_msg_get (ls_catd, NL_SETN, 1562, "-Zs is not supported for embeded job command")));   /* catgets  1562  */
      return -1;
  }
}
...
if (operate == CMD_BSUB)
{
  ...
  {
      fprintf (stderr, I18N (1581, "Set job mac label failed."));   /* catgets 1581 */
      return -1;
  }
}
...
parseScript (FILE * from, int *embedArgc, char ***embedArgv, int option)
{
  // static char __func__] = "parseScript";
    ...
    "saveExit=$?\n"
        "/bin/rm -f $LSB_JOBFILENAME.shell\n" "(exit $saveExit)\n"; // FIXME FIXME FIXME FIXME put into configurea.ac!
...
if (select)
{
 ...
 && rusage != req && filter != req && span != req && same != req)
{
